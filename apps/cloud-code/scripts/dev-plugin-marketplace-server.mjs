...
export async function startPluginMarketplaceServer(options = {}) {
  const pluginsRoot = resolve(
    options.pluginsRoot ?? process.env.CLOUD_CODE_PLUGIN_MARKETPLACE_DEV_ROOT ?? DEFAULT_PLUGINS_ROOT,
  );
  const host = options.host ?? process.env.CLOUD_CODE_PLUGIN_MARKETPLACE_DEV_HOST ?? '127.0.0.1';
  const port = Number(options.port ?? process.env.CLOUD_CODE_PLUGIN_MARKETPLACE_DEV_PORT ?? 0);
