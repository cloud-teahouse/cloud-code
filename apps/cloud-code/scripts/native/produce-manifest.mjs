 ...
 *
 * Input dir must contain files matching: cloud-code-<target>.zip.sha256
...
// Tag 格式 `cloudcode-cli@x.y.z`（旧格式 `@cloud-code/cli@x.y.z`）或 `vx.y.z` 或 `x.y.z`，都归一化到 x.y.z
const version = tag.replace(/^@cloud-code\/cli@/, '').replace(/^cloudcode-cli@/, '').replace(/^v/, '');

const entries = await readdir(inputDir);
const sumFiles = entries.filter((f) => /^cloud-code-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No cloud-code-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // cloud-code-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^cloud-code-/, '').replace(/\.zip$/, '');
