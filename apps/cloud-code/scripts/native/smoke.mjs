...
async function runCloudCode(args) {
  try {
...
async function runCloudCodeWithEnv(args, env) {
  try {
    const { stdout, stderr } = await execFileAsync(executablePath, args, {
      cwd: appRoot,
      env: { ...process.env, ...env },
...
const versionOutput = await runCloudCode(['--version']);
assertIncludes(versionOutput, expectedVersion, '--version');

const helpOutput = await runCloudCode(['--help']);
assertIncludes(helpOutput, 'Usage: cloud-code', '--help');

const exportHelpOutput = await runCloudCode(['export', '--help']);
assertIncludes(exportHelpOutput, 'Usage: cloud-code export', 'export --help');

const nativeAssetOutput = await runCloudCodeWithEnv(['--version'], {
  CLOUD_CODE_HOME: smokeHome,
  CLOUD_CODE_NATIVE_ASSET_SMOKE: '1',
});
