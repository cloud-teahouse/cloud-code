...
// the whole repo instead of just apps/cloud-code.
const REPO_ROOT = resolve(APP_ROOT, '../..');
// Runtime variable the CLI reads to locate the marketplace JSON.
const MARKETPLACE_ENV = 'CLOUD_CODE_PLUGIN_MARKETPLACE_URL';
// Opt-in for dev: point this run at an external marketplace instead of a local one.
const EXTERNAL_MARKETPLACE_ENV = 'CLOUD_CODE_DEV_MARKETPLACE_URL';
...
child.on('error', async (error) => {
  console.error(`Failed to start Cloud Code CLI dev: ${error.message}`);
