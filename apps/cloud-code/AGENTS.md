# apps/cloud-code Development Guide

This file only contains rules local to `apps/cloud-code`. For cross-repo rules, see the root `AGENTS.md`.

> **Writing or modifying the TUI?** Read `docs/tui-design.md` first — it is the written visual spec (box dialects, dividers, padding, token usage, tables, narrow-terminal policy) and its §9 tracks which legacy implementations still await migration. This file keeps only the map, boundaries, and hard constraints.

## TUI File Layout

`apps/cloud-code` is the terminal UI / CLI app. The entry chain is:

`src/main.ts` -> `src/cli/commands.ts` -> `src/cli/run-shell.ts` -> SDK `CloudCodeHarness` -> `src/tui/cloud-code-tui.ts`
...
- `src/tui/cloud-code-tui.ts`: the `CloudCodeTUI` coordinator — wires state, layout, editor, session, SDK events, and dialogs together, and dispatches slash-command handlers. Heavy logic is delegated to `controllers/`, not accumulated here.
...
- `src/tui/i18n/`: UI internationalization — `t()` / `setLocalePreference()` singleton, system-locale detection, en + zh-CN dictionaries (en keys are the compile-time source of truth), `resolveDescription()` (key-or-passthrough for builtin registry/shortcut/tip constants), and `padEndVisible()` (display-width-aware padding — never use `.padEnd()` for column alignment).
...
- `src/tui/components/primitives/`: the layout primitives of the visual system (`renderBox`/`renderRow`/`columnWidth`/`renderDivider`/`fitText`/`padStartVisible`/`renderTable`) — pure line builders, not components; the dialect rules live in `docs/tui-design.md`.
...
- `CloudCodeTUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `controllers`, `commands`, `components`, `reverse-rpc`, or `utils` first.
- `controllers` own the heavy, independently-testable slices (event routing, streaming render, session replay, tasks browser, editor keyboard, auth). Event-routing and rendering logic belong here, not on the `CloudCodeTUI` class.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `CloudCodeTUI`, but complex logic should continue to sink downward.
...
- `apps/cloud-code` may only use core capabilities through `@cloud-code/sdk`. Do not import `@cloud-code/agent-core` directly in app code.
...
- Functions with no state / UI side effects do not belong as private methods on the `CloudCodeTUI` class; put them in external utils.
...
Theme token semantics and per-token consumers are documented in `src/tui/theme/colors.ts`; token usage rules (which semantic gets which token) live in `docs/tui-design.md`. The following rules are hard and guard-enforced:
