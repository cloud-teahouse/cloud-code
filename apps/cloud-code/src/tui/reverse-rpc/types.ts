...
import type { PermissionMode, QuestionAnswerMethod } from '@cloud-code/sdk';
...
export interface ApprovalPanelChoice {
  label: string;
  response: 'approved' | 'approved_for_session' | 'approved_always' | 'rejected' | 'cancelled';
  selected_label?: string | undefined;
  requires_feedback?: boolean | undefined;
  // Optional permission-mode switch applied when this choice approves (plan
  // review "approve and switch to Auto" variant). Echoed on the response so
  // the resolving policy can apply the mode change with the decision.
...
export interface ApprovalPanelData {
  ...
  /**
   * A4 worker badge: present when the ask was bridged from a teammate
   * through the leader's approval queue — shown under the panel title so the
   * user sees which teammate is asking.
   */
  requester?: { readonly name: string; readonly teamName?: string | undefined } | undefined;
}
