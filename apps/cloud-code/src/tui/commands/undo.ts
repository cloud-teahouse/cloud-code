...
import type { ContextMessage } from '@cloud-code/sdk';
...
// ---------------------------------------------------------------------------
// Shared with /rewind: the count-based anchor model, availability probe,
// choice construction, and the conversation-truncation path are identical —
// /rewind only adds a file-rollback side and a mode pick on top.
// ---------------------------------------------------------------------------
...
export async function handleUndoCommand(
  ...
  if (host.state.appState.streamingPhase !== 'idle') {
    host.showError(t('commands.undo.busyStreaming'));
    return;
  }
  ...
  if (count === undefined) {
    host.showError(t('commands.undo.usage'));
    return;
  }
...
// undoHistory is destructive and the RPC is not idempotent, so concurrent
// calls must never overlap: the selector latches its own Enter key, but a
// typed `/undo <count>` can be submitted twice inside the RPC window and
// both calls would resolve the same anchor and truncate the server-side
// history twice. A reentrant call is dropped, mirroring the selector latch.
let undoInFlight = false;

export async function undoByCount(host: SlashCommandHost, count: number): Promise<boolean> {
  if (undoInFlight) return false;
  undoInFlight = true;
  try {
    return await performUndoByCount(host, count);
  } finally {
    undoInFlight = false;
  }
}

async function performUndoByCount(host: SlashCommandHost, count: number): Promise<boolean> {
  const session = host.session;
  if (session === undefined) {
    host.showError(resolveDescription(NO_ACTIVE_SESSION_MESSAGE));
    return false;
  ...
  if (lastUserIndex === undefined) {
    showUndoLimitStatus(host, t('commands.undo.nothing'));
    return false;
  }
  ...
  try {
    ...
    host.showError(t('commands.undo.failed', { error: formatErrorMessage(error) }));
    return false;
  }
...
async function showUndoSelector(host: SlashCommandHost): Promise<void> {
  ...
  // Latch against a double Enter: the selector stays mounted while
  // undoByCount awaits the server-side undoHistory RPC, and a re-entrant
  // select would truncate the history twice (2N messages gone server-side,
  // N locally — a silent, unrecoverable fork). Same pattern as the session
  // picker's sessionPickerSelectInFlight. Released on failure so the
  // still-open selector stays usable.
...
function formatUndoChoiceLabel(
  ...
  if (entry.kind === 'skill_activation') {
    ...
    if (name.length === 0) return t('commands.undo.unknownSkill');
    return args.length > 0 ? `/${name} ${args}` : `/${name}`;
  }
  if (entry.kind === 'plugin_command' && entry.pluginCommandData !== undefined) {
    return formatPluginCommandSlash(entry.pluginCommandData) ?? t('commands.undo.userMessage');
  }
  ...
  if (imageCount > 0) {
    return t(
      imageCount === 1
        ? 'commands.undo.userMessageImages.one'
        : 'commands.undo.userMessageImages.other',
      { count: imageCount },
    );
  }
  return t('commands.undo.userMessage');
}
...
function formatUndoLimitMessage(
  ...
  const reason = availability.stoppedAtCompaction ? t('commands.undo.limitReasonCompaction') : '';
  const requested = formatPromptCount(requestedCount);
  const max = formatPromptCount(availability.maxCount);
  return t('commands.undo.limit', { requested, max, reason });
}

function formatNothingToUndoMessage(availability: UndoAvailability): string {
  if (availability.stoppedAtCompaction) {
    return t('commands.undo.nothingAfterCompaction');
  }
  return t('commands.undo.nothing');
}
