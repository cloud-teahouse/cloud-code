...
import { resolveDescription } from '../i18n';

/**
 * A completable token (subcommand or flag) for a slash command's argument
 * position. Generic across commands — any `CloudCodeSlashCommand` can build a
...
export interface ArgCompletionSpec {
  ...
  /** Short description shown in the autocomplete menu — an i18n key for
   * builtin specs (resolved at completion time), plain text otherwise. */
...
export function completeLeadingArg(
  ...
  const items = specs
    ...
    .map((spec) => ({
      value: spec.value,
      label: spec.value,
      // Resolve i18n keys here — completions are built per keystroke, so the
      // menu follows a mid-session language switch for free.
      description: resolveDescription(spec.description),
    }));
