import type { PluginCommandDef } from '@cloud-code/sdk';

import type { CloudCodeSlashCommand } from './types';

export interface PluginSlashCommands {
  readonly commands: readonly CloudCodeSlashCommand[];
...
export function buildPluginSlashCommands(defs: readonly PluginCommandDef[]): PluginSlashCommands {
  ...
  const commands = defs.map((def) => {
    ...
    } satisfies CloudCodeSlashCommand;
  });
