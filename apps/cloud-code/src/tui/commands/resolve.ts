...
export function resolveSlashCommandInput(options: ResolveSlashCommandInput): SlashCommandIntent {
  ...
  if (
    command !== undefined &&
    isExperimentalFlagEnabled((command as CloudCodeSlashCommand).experimentalFlag)
  ) {
...
export function slashBusyMessage(
  ...
  if (reason === 'streaming') {
    return t('commands.dispatch.busyStreaming', { name: commandName });
  }
  return t('commands.dispatch.busyCompacting', { name: commandName });
}
