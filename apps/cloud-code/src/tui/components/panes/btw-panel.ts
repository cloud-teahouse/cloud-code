import type { Component, MarkdownTheme, MouseEvent } from '@cloud-code/pi-tui';
import {
  Markdown,
  Text,
  truncateToWidth,
...
export class BtwPanelComponent implements Component {
  ...
  private renderTopBorder(width: number, truncated: boolean): string {
    ...
    const hint = truncated && this.options.canUseScrollKeys()
      ? t('panels.btw.hintCloseScroll')
      : t('panels.btw.hintClose');
    const title =
  ...
  private renderBody(width: number): BtwBodyRender {
    ...
    if (this.turns.length === 0) {
      lines.push(chalk.hex(currentTheme.palette.textDim)(t('panels.btw.ready')));
    }
  ...
  private renderTurn(turn: BtwTurn, width: number): string[] {
    ...
    if (answer.length > 0) {
      ...
      lines.push(chalk.hex(currentTheme.palette.textDim)(t('panels.btw.waiting')));
    }
  ...
  /**
   * Hover-to-scroll: the wheel pans the conversation three rows per tick with
   * the same follow-tail semantics as the scroll keys (landing on the tail
   * re-engages following).
   */
  handleMouse(event: MouseEvent): void {
    if (event.type !== 'wheel' || this.maxScrollTop <= 0) return;
    const delta = event.button === 64 ? -3 : event.button === 65 ? 3 : 0;
    if (delta === 0) return;
    const current = this.followTail ? this.maxScrollTop : this.scrollTop;
    const next = Math.max(0, Math.min(this.maxScrollTop, current + delta));
    this.scrollTop = next;
    this.followTail = next === this.maxScrollTop;
  }
}
