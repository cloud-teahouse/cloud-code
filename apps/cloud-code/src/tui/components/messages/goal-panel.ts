...
import type { GoalSnapshot, GoalStatus } from '@cloud-code/sdk';
...
export class GoalSetMessageComponent implements Component {
  ...
  render(width: number): string[] {
    return renderLifecycleLine(t('panels.goal.set'), width);
  }
}
...
export class UpcomingGoalAddedMessageComponent implements Component {
  ...
  render(width: number): string[] {
    return renderLifecycleLine(t('panels.goal.upcomingAdded'), width);
  }
}
...
export function goalPanelTitle(goal: GoalSnapshot): string {
  return t('panels.goal.title', { status: goalStatusText(goal.status) });
}

/** Localised goal status label (active / complete / blocked / paused). */
function goalStatusText(status: GoalStatus): string {
  switch (status) {
    case 'active':
      return t('panels.goal.status.active');
    case 'complete':
      return t('panels.goal.status.complete');
    case 'blocked':
      return t('panels.goal.status.blocked');
    case 'paused':
      return t('panels.goal.status.paused');
  }
}
...
export function buildGoalReportLines(goal: GoalSnapshot, wrapWidth: number = WRAP_WIDTH): string[] {
  ...
  const row = (label: string, val: string): string => `${muted(padEndVisible(label, LABEL_WIDTH))}${val}`;

  if (showReason) {
    lines.push(
      row(
        t('panels.goal.label.status'),
        currentTheme.fg(statusColor, goalStatusText(goal.status)) +
          (reason !== undefined ? muted(` — ${reason}`) : ''),
      ),
    );
  }
  lines.push(row(t('panels.goal.label.running'), value(formatGoalElapsed(goal.wallClockMs))));
  lines.push(row(t('panels.goal.label.turns'), value(`${goal.turnsUsed}`)));
  lines.push(row(t('panels.goal.label.tokens'), value(formatTokenCount(goal.tokensUsed))));
  if (!isComplete) {
    const stop = formatStopRow(goal);
    lines.push(
      stop !== null
        ? row(t('panels.goal.label.stop'), value(stop))
        : muted(t('panels.goal.noStop')),
    );
  }
  return lines;
}
...
function formatStopRow(goal: GoalSnapshot): string | null {
  ...
  if (budget.turnBudget !== null) {
    parts.push(
      t('panels.goal.stop.turns', { budget: budget.turnBudget, used: goal.turnsUsed }),
    );
  }
  if (budget.tokenBudget !== null) {
    parts.push(t('panels.goal.stop.tokens', { tokens: formatTokenCount(budget.tokenBudget) }));
  }
  if (budget.wallClockBudgetMs !== null) {
    parts.push(t('panels.goal.stop.elapsed', { elapsed: formatGoalElapsed(budget.wallClockBudgetMs) }));
  }
