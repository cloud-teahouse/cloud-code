...
import { Spacer, Text, applyBackgroundToLine, truncateToWidth, visibleWidth, type Component } from '@cloud-code/pi-tui';
...
export class UserMessageComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    // Spacer — stays unpainted; the background block starts at the bullet row.
    const spacerLines = this.spacerComponent.render(safeWidth);
    for (const line of spacerLines) {
      lines.push(line);
    }
    ...
    const rendered = lines.map((line, index) => {
      ...
      if (index < spacerLines.length) return truncated;
      // Full-width background block behind the echo (Claude's
      // userMessageBackground analogue). The ✨ bullet and the bold roleUser
      // text keep their own colours — only the row's underlay is painted.
      return applyBackgroundToLine(truncated, safeWidth, (s) =>
