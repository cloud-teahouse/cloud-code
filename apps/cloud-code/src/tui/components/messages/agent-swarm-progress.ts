import { truncateToWidth, visibleWidth, type Component } from '@cloud-code/pi-tui';
import chalk from 'chalk';

import { agentSwarmResultStructuredSchema } from '@cloud-code/sdk';
...
const ORCHESTRATING_LABEL = 'swarm.status.orchestrating';
const PROMPTING_LABEL = 'swarm.status.prompting';
const WORKING_LABEL = 'swarm.status.working';
const COMPLETED_LABEL = 'swarm.status.completed';
const FAILED_LABEL = 'swarm.status.failed';
const ABORTED_LABEL = 'swarm.status.aborted';
const CANCELLED_LABEL = 'swarm.status.cancelled';
const QUEUED_LABEL = 'swarm.status.queued';
const SUSPENDED_LABEL = 'swarm.status.rateLimited';
const RESUMED_ITEM_LABEL = 'swarm.item.resumed';
...
const PHASE_LABELS: Record<AgentSwarmPhase, string> = {
  ...
  running: 'swarm.phase.running',
  completed: 'swarm.phase.completed',
  failed: 'swarm.phase.failed',
...
export class AgentSwarmProgressComponent implements Component {
  ...
  applyResult(output: string, structured?: unknown): boolean {
    const statuses = parseAgentSwarmResultStatuses(output, structured);
  ...
  private renderHeader(width: number, _summary: AgentSwarmSummary | undefined): string {
    ...
    const title = gradientText(t('swarm.title'), this.colors.primary, this.colors.accent, AGENT_SWARM_TITLE_ACCENT_BIAS);
    const description =
  ...
  private renderProgressStatusLine(width: number, status: TotalStatus): string {
    const labelText = resolveDescription(totalStatusLabel(status));
    // Shimmer the label only while the swarm is working; terminal states
...
/**
 * Summary counts for an AgentSwarm result. New sessions carry them in the
 * result's structured envelope; older records are still parsed from the
 * XML output via {@link agentSwarmResultSummaryFromOutput}.
 */
export function agentSwarmResultSummary(result: ToolResultBlockData): AgentSwarmResultSummary {
  const structured = agentSwarmResultStructuredSchema.safeParse(result.structured);
  if (structured.success) {
    const { completed, failed, aborted, members } = structured.data;
    if (completed !== undefined && failed !== undefined && aborted !== undefined) {
      return { completed, failed, aborted, parsed: members !== undefined && members.length > 0 };
    }
    if (members !== undefined) {
      return {
        completed: members.filter((member) => member.outcome === 'completed').length,
        failed: members.filter((member) => member.outcome === 'failed').length,
        aborted: members.filter((member) => member.outcome === 'aborted').length,
        parsed: members.length > 0,
      };
    }
  }
  return agentSwarmResultSummaryFromOutput(result.output);
}

/**
 * True when an AgentSwarm result reports a deliberate user interruption
 * (the whole batch aborted). Prefers the structured error kind; older
 * records are detected by matching the English cancellation text.
 */
export function agentSwarmResultIsUserCancelled(result: ToolResultBlockData): boolean {
  const structured = agentSwarmResultStructuredSchema.safeParse(result.structured);
  if (structured.success && structured.data.errorKind !== undefined) {
    return structured.data.errorKind === 'user_cancelled';
  }
  return /\b(?:aborted|cancelled)\b/i.test(result.output);
}
...
function parseAgentSwarmResultStatuses(
  ...
  const parsed = agentSwarmResultStructuredSchema.safeParse(structured);
  if (parsed.success && parsed.data.members !== undefined) {
    const fromEnvelope = agentSwarmStatusesFromEnvelope(output, parsed.data.members);
...
/**
 * Build per-member statuses from the structured envelope. The envelope
 * carries outcomes positionally (in `<subagent>` tag order); the member
 * *texts* are the subagents' own content and still come out of the XML
 * bodies. Returns undefined when the tag walk disagrees with the envelope
 * (truncated or hand-edited output) so the caller can fall back to the
 * full legacy parse.
 */
function agentSwarmStatusesFromEnvelope(
  output: string,
  members: readonly { outcome: 'completed' | 'failed' | 'aborted' }[],
): AgentSwarmResultStatus[] | undefined {
  const bodies = forEachSubagentTag(output, (_attrs, body) => body);
  if (bodies.length !== members.length) return undefined;
  return members.map((member, index) => ({
    index: index + 1,
    status: member.outcome === 'aborted' ? 'cancelled' : member.outcome,
    completedText: member.outcome === 'completed' ? bodies[index] : undefined,
    failureText: member.outcome === 'failed' ? bodies[index] : undefined,
  }));
}
...
function renderCellLabel(
  ...
  if (snapshot.phase === 'running') {
    return truncateWithColor(resolveDescription(runningCellLabelText(member)), width, colors.textDim);
  }
  ...
  return truncateWithColor(
    resolveDescription(PHASE_LABELS[snapshot.phase]),
    width,
...
function renderCancelledCellLabel(
  ...
  const labelText = resolveDescription(member.cancelledLabelText ?? ABORTED_LABEL);
...
function renderPendingCell(
  ...
  const prefix = `${id} `;
  const itemText = collapseWhitespace(member.itemText);
  const label = resolveDescription(itemText.length > 0 ? itemText : QUEUED_LABEL);
  const labelWidth = Math.max(1, width - visibleWidth(prefix));
...
function renderQueuedCell(
  ...
  const prefix = `${id} `;
  const labelWidth = Math.max(1, width - visibleWidth(prefix));
  return prefix + truncateWithColor(resolveDescription(QUEUED_LABEL), labelWidth, colors.textDim);
}
