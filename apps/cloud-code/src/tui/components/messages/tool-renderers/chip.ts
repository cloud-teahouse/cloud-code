...
export function formatWriteChip(stats: WriteStats): string {
  return t(stats.lines === 1 ? 'messages.chip.lines.one' : 'messages.chip.lines.other', {
    count: stats.lines,
  });
}
...
const readChip: ChipProvider = (_toolCall, result) => {
  ...
  return t(lines === 1 ? 'messages.chip.lines.one' : 'messages.chip.lines.other', {
    count: lines,
  });
};
...
const grepChip: ChipProvider = (_toolCall, result) => {
  ...
  if (matches === 0) return t('messages.chip.noMatches');
  return t(matches === 1 ? 'messages.chip.matches.one' : 'messages.chip.matches.other', {
    count: matches,
  });
};
...
const globChip: ChipProvider = (_toolCall, result) => {
  ...
  if (files === 0) return t('messages.chip.noFiles');
  return t(files === 1 ? 'messages.chip.files.one' : 'messages.chip.files.other', {
    count: files,
  });
};
...
const webSearchChip: ChipProvider = (_toolCall, result) => {
  ...
  if (count === 0) {
    return lines.length === 0 ? t('messages.chip.noResults') : t('messages.chip.webResult');
  }
  return t(count === 1 ? 'messages.chip.results.one' : 'messages.chip.results.other', { count });
};

const goalStatusOutputChip: ChipProvider = (_toolCall, result) =>
  result.is_error ? '' : goalStatusChip(result);
