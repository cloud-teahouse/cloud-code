...
export function renderDiffLines(
  ...
  if (hidden > 0) {
    output.push(s.meta(`     … ${moreChangesHiddenText(hidden, 'ctrl+o')}`));
  }

  return output;
}

/** Truncation footer text: singular/plural follows the count, like `approval.hiddenLines`. */
function moreChangesHiddenText(hidden: number, hint: string): string {
  return hidden === 1
    ? t('messages.diff.moreChangesHidden.one', { count: hidden, hint })
    : t('messages.diff.moreChangesHidden.other', { count: hidden, hint });
}
      ...
      if (gap > 0) {
        ...
        output.push(
          s.meta(
            `     … ${
              gap === 1
                ? t('messages.diff.unchangedLines.one', { count: gap })
                : t('messages.diff.unchangedLines.other', { count: gap })
            } …`,
          ),
        );
        body++;
