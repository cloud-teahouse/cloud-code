 ...
 *
 * Mounted by `cloud-code-tui.ts` via the same nested-takeover pattern as
...
export class ApprovalPreviewViewer extends Container implements Focusable {
  ...
  /** Hover-to-scroll: the wheel pans the viewer three rows per tick. */
  ...
  private renderFooter(width: number, bodyHeight: number): string {
    ...
    const keys =
      `${key('↑↓')} ${dim(t('approval.preview.line'))}  ` +
      `${key('PgUp/PgDn')} ${dim(t('approval.preview.page'))}  ` +
      `${key('g/G')} ${dim(t('approval.preview.topBot'))}  ` +
      `${key('Q/Esc/Ctrl+E')} ${dim(t('approval.preview.cancel'))}`;
    const left = ` ${keys}`;
