...
export interface UndoSelectorOptions {
  ...
  /** Header line; defaults to the /undo wording. /rewind overrides it. */
...
export class UndoSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const frameLines = this.frame.render(width, {
      title: this.opts.title ?? t('dialogs.undo.title'),
      hintParts,
      content: lines,
      footer: [''],
    });
  ...
  /**
   * The content region (between the header blank and the footer): a window of
   * at most MAX_VISIBLE_CHOICES rows, one per choice, positioned with the
   * PREFERRED_SELECTED_OFFSET math. Returns the lines plus the
   * content-relative hit zones (row 0 = first content line; one full-width
   * zone per visible choice). Once a selection was submitted the dialog
   * ignores the mouse, so no zones are declared.
   */
  private renderContent(
    width: number,
    view: SearchableListView<UndoChoice>,
  ): { lines: string[]; zones: HitZone[] } {
    const lines: string[] = [];
    const zones: HitZone[] = [];
    if (view.items.length === 0) {
      lines.push(currentTheme.fg('textMuted', `   ${t('dialogs.undo.empty')}`));
      return { lines, zones };
    }
