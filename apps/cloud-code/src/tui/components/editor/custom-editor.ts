...
import { ensureAppKeybindings } from '#/tui/keybindings/manager';
...
export class CustomEditor extends Editor {
  ...
  constructor(tui: TUI, options: CustomEditorOptions = {}) {
    ...
    // mouseScroll: let the wheel pan the composer when the input overflows
    // its visible window (pi-tui keeps it opt-in; the transcript still owns
    // the wheel everywhere else). mouseClickToPosition: a left press inside
    // the text box moves the cursor to the clicked grapheme (press-only —
    // no drag selection; autocomplete-open presses still go to the menu).
    super(tui, theme, {
      paddingX: 4,
    ...
    // Make sure the global keybindings manager knows the app.chat.* actions
    // (see tui/keybindings/). Startup installs the user-aware manager first,
    // in which case this is a no-op.
    ensureAppKeybindings();
  ...
  private cancelAutocompleteActivity(): void {
    ...
    // Cloud Code needs Esc to win over app-level cancel while the slash menu request is active.
    (this as unknown as AutocompleteInternals).cancelAutocomplete();
  }

  // Repaint the viewport when the autocomplete dropdown closes, so the editor
  // snaps back to the bottom instead of sitting where the taller dropdown left
  // it. The collapse repaint rewrites the visible region in place, keeping the
  // native scrollback intact. Only worthwhile when the session content already
  // overflows one screen; below that a top-aligned repaint would pull the
  // editor up and leave a blank tail. Always skipped inside tmux, whose own
  // reflow handles the shrink.
  private requestFullRenderOnAutocompleteClose(): void {
    if (isInsideTmux()) return;
    const { columns, rows } = this.tui.terminal;
    // Repaint when content fills or overflows the viewport. An exact fill (==
    // rows) is safe (no blank tail) and still needs the repaint: the
    // differential renderer keeps the old viewport offset after a shrink.
    if (this.tui.render(columns).length < rows) return;
    this.tui.requestCollapseRender();
  }
  ...
  override render(width: number): string[] {
    ...
    // Empty-buffer placeholder (render-only; the buffer is untouched). Bash
    // mode shows its `! shell mode` border label instead of a placeholder.
    // Synced every frame so language hot-switch and mode flips just work.
    this.placeholderText = this.inputMode === 'bash' ? '' : t('editor.placeholder');
    ...
    return wrapWithSideBorders(lines, (s) => this.borderColor(s), {
      ...
      label: isBash ? ` ${currentTheme.boldFg('shellMode', t('editor.shellMode.label'))} ` : undefined,
    });
  }
  ...
  override handleInput(data: string): void {
    ...
    // Global keybindings manager (pi-tui tui.* + app.chat.* actions). The
    // rebindable chat shortcuts below dispatch through it; only the reserved
    // keys (ctrl+c/ctrl+d) and the stateful input guards (Escape, Tab,
    // empty-buffer ↑/↓, `!` bash mode) stay hardcoded — see the boundary
    // notes inline and in tui/keybindings/default-bindings.ts.
    ...
    // Paste image binding — the key is platform-aware (ctrl+v on Unix,
    // alt+v on Windows; see APP_KEYBINDINGS) and user-rebindable. When the
    // host reports no image available, we fall through to pi-tui's normal
    // paste path so text from the clipboard still works.
    if (kb.matches(normalized, 'app.chat.pasteImage')) {
    ...
    // ctrl+c / ctrl+d are RESERVED (interrupt/exit, same rule as Claude
    // Code's NON_REBINDABLE list): intentionally not in the rebindable
    // keybindings table. Their double-press state machines live in
    // controllers/editor-keyboard.ts.
    ...
    if (kb.matches(normalized, 'app.chat.openExternalEditor')) {
      this.onOpenExternalEditor?.();
      return;
    }

    if (kb.matches(normalized, 'app.chat.toggleToolExpand')) {
      this.onToggleToolExpand?.();
      return;
    }

    if (kb.matches(normalized, 'app.chat.steer')) {
      this.onCtrlS?.();
      return;
    }

    if (kb.matches(normalized, 'app.chat.backgroundTask')) {
    ...
    if (kb.matches(normalized, 'app.chat.toggleTodoExpand')) {
    ...
    if (kb.matches(normalized, 'app.chat.planModeToggle')) {
      this.onShiftTab?.();
      return;
    }
    ...
    // Escape stays hardcoded by design: it is a state machine (autocomplete
    // cancel here; stream cancel / double-Esc undo in
    // controllers/editor-keyboard.ts), not a rebindable action.
    if (matchesKey(normalized, Key.escape)) {
      if (this.hasAutocompleteActivity()) {
        this.cancelAutocompleteActivity();
        return;
      }
      // Vim mode: an INSERT-mode Escape switches to NORMAL and is consumed by
      // the vim engine (useVimInput parity — vim owns this mode switch, so it
      // never reaches the app-level cancel below). A NORMAL-mode Escape only
      // cancels a pending vim command and falls through to the app's own
      // cancel / double-Esc-undo semantics.
      if (this.vimRouteInput(normalized)) {
        return;
      }
      this.onEscape?.();
      return;
    ...
    // Skipped in vim NORMAL mode: bash entry is an insert-mode gesture, and
    // NORMAL-mode keys belong to the vim state machine.
    if (
      this.inputMode === 'prompt' &&
