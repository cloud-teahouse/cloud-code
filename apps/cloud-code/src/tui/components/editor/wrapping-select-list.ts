 ...
 *
 * Rendering is replaced, and `itemRowCount` reports the wrapped heights back
 * to pi-tui so its mouse hit-test walks the same rows this class paints —
 * clicks track wrapped descriptions exactly. Selection, filtering, and key
 * handling stay in pi-tui; the mouse-hover row is painted here with the
 * theme's `hoverText` (a background highlight, matching pi-tui's own list).
 * pi-tui keeps the row state private, so the
...
export class WrappingSelectList extends SelectList {
  /** Width of the last render() frame — item row counts are width-dependent. */
  private lastWidth = 80;

  override render(width: number): string[] {
    this.lastWidth = width;
    const { filteredItems, selectedIndex, hoveredIndex, theme } = this.internals();
    if (filteredItems.length === 0) {
      return [theme.noMatch(t('editor.autocomplete.noMatch'))];
    }
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (!item) continue;
      // Hover affordance (mouse motion only; the keyboard flow never sets
      // hoveredIndex, so no-mouse renders stay byte-identical).
      const hover = i === hoveredIndex ? (line: string) => (theme.hoverText?.(line) ?? line) : null;
      lines.push(
        ...this.renderItemLines(item, i === selectedIndex, width, primaryColumnWidth).map((line) =>
          hover === null ? line : hover(line),
        ),
      );
    }
  ...
  /**
   * Rows one item occupies at the last render width. The count comes from
   * the same renderItemLines path that paints the menu — pi-tui's row-hit
   * walk consumes it, so both sides share a single source of truth.
   */
  protected override itemRowCount(item: SelectItem): number {
    return this.renderItemLines(item, false, this.lastWidth, this.primaryColumnWidth()).length;
  }
