...
import { wrapWords } from '#/tui/components/dialogs/frame/dialog-frame';
...
export class DeviceCodeBoxComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const promptLine = truncateToWidth(
      currentTheme.fg('textDim', t('status.login.prompt')),
      innerWidth,
      '…',
    );
    const urlLine = truncateToWidth(currentTheme.fg('primary', url), innerWidth, '…');

    const codeLabel = currentTheme.boldFg('textDim', t('status.login.codeLabel'));
    ...
    if (hint !== undefined && hint.length > 0) {
      contentLines.push('');
      // Prose hint: wrap at word boundaries so the cancel instruction
      // survives narrow widths.
      for (const hintLine of wrapWords(hint, innerWidth)) {
        contentLines.push(currentTheme.fg('textDim', hintLine));
      }
    }
