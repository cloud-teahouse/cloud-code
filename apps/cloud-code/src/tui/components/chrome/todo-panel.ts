...
export interface TodoItem {
  ...
  /** Present-continuous phrase shown in place of the title while in_progress. */
...
export class TodoPanelComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(c.primary).bold(t('panels.todo.title')),
    ];
    ...
    if (this.expanded) {
      ...
      if (this.todos.length > MAX_VISIBLE) {
        lines.push(
          chalk.hex(c.textDim)(t('panels.todo.footerExpanded', { count: this.todos.length })),
        );
      }
    } else {
      ...
      if (hidden > 0) {
        ...
        lines.push(
          chalk.hex(c.textDim)(t('panels.todo.footerCollapsed', { count: hidden, suffix })),
        );
      }
    }
...
function renderRow(todo: TodoItem, colors: ColorPalette): string {
  ...
  const label = todo.status === 'in_progress' ? (todo.activeForm ?? todo.title) : todo.title;
  const titleStyled = styleTitle(label, todo.status, colors);
...
const STATUS_LABELS: readonly { status: TodoStatus; labelKey: MessageKey }[] = [
  { status: 'done', labelKey: 'panels.todo.status.done' },
  { status: 'in_progress', labelKey: 'panels.todo.status.inProgress' },
  { status: 'pending', labelKey: 'panels.todo.status.pending' },
];
...
export function formatHiddenCounts(counts: Record<TodoStatus, number>): string {
  return STATUS_LABELS.filter(({ status }) => counts[status] > 0)
    .map(({ status, labelKey }) => `${counts[status]} ${t(labelKey)}`)
    .join(' · ');
}
