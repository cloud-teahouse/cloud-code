...
import type { MarkdownTheme, EditorTheme, ScrollbarStyle } from '@cloud-code/pi-tui';
...
export function createEditorTheme(): EditorTheme {
  return {
    ...
    placeholderColor: (s) => currentTheme.dim(s),
    selectList: {
      ...
      hoverText: (s) => chalk.bgHex(currentTheme.color('hoverBackground'))(s),
    },
  };
}
...
/**
 * Transcript-scrollbar style adapter for `TUI.setScrollbarStyle`. Chrome
 * like the scroll badge: the track takes the muted text tone, the thumb the
 * primary accent so the draggable part stands out. Reads the palette through
 * `currentTheme` at call time, so a theme switch repaints on the next frame.
 */
export function createScrollbarStyle(): ScrollbarStyle {
  return {
    track: (glyph) => chalk.hex(currentTheme.color('textMuted'))(glyph),
    thumb: (glyph) => chalk.hex(currentTheme.color('primary'))(glyph),
  };
}

/** Linear RGB blend of two `#RRGGBB` colours; `amount` 1 yields `b` in full. */
