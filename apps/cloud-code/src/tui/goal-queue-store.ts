...
function normalizeObjective(value: string): string {
  ...
  if (objective.length === 0) {
    throw new CloudCodeError(ErrorCodes.GOAL_OBJECTIVE_EMPTY, t('selectors.goalQueue.errorEmpty'));
  }
  if (objective.length > MAX_GOAL_OBJECTIVE_LENGTH) {
    throw new CloudCodeError(
      ErrorCodes.GOAL_OBJECTIVE_TOO_LONG,
      t('selectors.goalQueue.errorTooLong', { max: MAX_GOAL_OBJECTIVE_LENGTH }),
    );
  }
  return objective;
}
...
function findGoalIndex(file: GoalQueueFile, goalId: string): number {
  ...
  if (index === -1) {
    throw new CloudCodeError(ErrorCodes.GOAL_NOT_FOUND, t('selectors.goalQueue.errorNotFound'));
  }
  return index;
}
