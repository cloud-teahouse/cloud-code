...
export function renderDanceWelcomeHeader(
  ...
  const rightRow0 = truncateToWidth(
    rainbowText(t('welcome.title'), palette, phase + 2, true),
    textWidth,
    '…',
  );
  ...
  return [
    ...
    ...logo.slice(2).map((row, i) => rainbowText(row.padEnd(logoWidth), palette, phase + 6 + i * 3)),
  ];
}
...
export function tryHandleDanceCommand(host: SlashCommandHost, parsed: ParsedSlashInput): boolean {
  ...
  if (sub === 'off') {
    ...
    host.showStatus(t('welcome.dance.statusOn', { cmd: cmd('/dance off') }));
  } else {
    currentDanceController.start({ hold: false });
    host.showStatus(t('welcome.dance.statusHint', { cmd: cmd('/dance on') }));
  }
  return true;
}
