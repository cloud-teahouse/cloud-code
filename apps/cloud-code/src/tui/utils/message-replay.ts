...
import { limitAgentReplayByTurns } from '@cloud-code/sdk';
...
function formatHookResultBlock(event: string, body: string, blocked: boolean): string {
  const title = blocked
    ? t('utils.hookResult.titleBlocked', { event })
    : t('utils.hookResult.title', { event });
  return `*${title}*\n\n${body.trim() || t('utils.hookResult.empty')}`;
}
