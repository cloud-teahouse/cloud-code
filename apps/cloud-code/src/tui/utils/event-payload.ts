...
export function isTodoItemShape(
  value: unknown,
): value is { title: string; status: 'pending' | 'in_progress' | 'done'; activeForm?: unknown } {
...
/** Extract the optional `activeForm` label from a raw todo payload. */
export function todoActiveForm(value: { activeForm?: unknown }): string | undefined {
  return typeof value.activeForm === 'string' && value.activeForm.length > 0
    ? value.activeForm
    : undefined;
}
...
function formatProviderFilteredMessage(
  ...
  return t('utils.eventPayload.providerFiltered', { finishReason: normalizedFinishReason, raw });
}
