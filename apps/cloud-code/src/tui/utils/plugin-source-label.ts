...
/** Trust label → badge text key; resolved at render time like other chrome. */
const TRUST_BADGE_KEY: Readonly<Record<PluginTrustLabel, MessageKey>> = {
  official: 'panels.plugins.trustBadge.official',
  curated: 'panels.plugins.trustBadge.curated',
  'third-party': 'panels.plugins.trustBadge.thirdParty',
};

/** Localized badge word for a trust label (`official` / `curated` / `third-party`). */
export function localizedTrustLabel(label: PluginTrustLabel): string {
  return t(TRUST_BADGE_KEY[label]);
}
