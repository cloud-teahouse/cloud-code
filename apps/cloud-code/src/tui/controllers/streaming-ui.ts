...
import { ToolGroupComponent } from '../components/messages/tool-group';
...
/**
 * Tools whose consecutive same-step calls merge into one `ToolGroupComponent`
 * card. Picked for one-line summarizability: every call compresses to its key
 * argument plus a status tail without losing the information the transcript
 * is scanned for. Edit/Write are deliberately excluded — the content/diff
 * body is the point of those cards — and Read/Agent keep their specialized
 * groups (Read sums result lines, Agent tracks subagent lifecycle).
 */
const GROUPABLE_TOOL_NAMES: ReadonlySet<string> = new Set(['Bash', 'Glob', 'Grep']);
...
export class StreamingUIController {
  ...
  /** Wall-clock start of the active compaction, for its completion line. */
  private compactionStartedAtMs: number | undefined = undefined;
  ...
  // ---------------------------------------------------------------------------
  // Stall detection — flips the activity spinner to the warning color when the
  // token stream goes quiet while tokens are expected (waiting / composing).
  // ---------------------------------------------------------------------------

  private lastTokenAtMs = 0;
  private stallWatchTimer: ReturnType<typeof setInterval> | undefined;
  private stalledApplied = false;
  ...
  hasPendingToolGroup(): boolean {
    return this._pendingToolGroup !== null;
  }
  ...
  /**
   * The resolved display name of a streaming tool call (same fallback chain
   * as {@link getStreamingToolCallPreview}) without parsing the accumulated
   * arguments. Callers that only need the name to decide whether a full
   * preview parse is warranted (e.g. per-delta AgentSwarm routing) use this
   * to keep the per-delta path allocation-free.
   */
  getStreamingToolCallName(id: string): string {
    return (
      this._streamingToolCallArguments.get(id)?.name ??
  ...
  // ---------------------------------------------------------------------------
  // Dispose helpers (moved from CloudCodeTUI)
  // ---------------------------------------------------------------------------
  ...
  /**
   * Called on every fresh token signal (assistant/thinking/tool-call deltas,
   * plus turn and step begins as the baseline). Restarts the watchdog and
   * clears the stalled state immediately so the spinner recovers its normal
   * color on the first new token rather than the next check tick.
   */
  noteTokenActivity(): void {
    this.lastTokenAtMs = Date.now();
    if (this.stalledApplied) this.applyStalled(false);
    this.ensureStallWatch();
  }

  private ensureStallWatch(): void {
    if (this.stallWatchTimer !== undefined) return;
    this.stallWatchTimer = setInterval(() => {
      this.checkStall();
  ...
  private checkStall(): void {
    ...
    // Turn over (or never started): self-terminate — no one restarts the
    // watchdog except noteTokenActivity.
    ...
    const stalled =
      ...
      this.isTokenStreamExpected() &&
      Date.now() - this.lastTokenAtMs > STREAM_STALL_THRESHOLD_MS;
    this.applyStalled(stalled);
  }

  /**
   * Red means "the model stream went quiet", so it only applies where tokens
   * are actually expected: the waiting moon (TTFT / between steps) and the
   * composing braille. A running tool legitimately produces no tokens (its
   * card shows elapsed time), and 'thinking' replaces the spinner with the
   * ThinkingComponent — neither should burn the user's attention.
   */
  private isTokenStreamExpected(): boolean {
    const { state } = this.host;
    // `!` shell commands run under the 'shell' phase and compaction hides the
    // activity spinner entirely — neither involves a model token stream.
    if (state.appState.streamingPhase === 'shell' || state.appState.isCompacting) return false;
    if (state.livePane.mode === 'waiting') return true;
    return state.livePane.mode === 'idle' && state.appState.streamingPhase === 'composing';
  }

  private applyStalled(stalled: boolean): void {
    this.stalledApplied = stalled;
    // Re-assert on every call: updateActivityPane recreates the MoonLoader on
    // style switches, and the fresh instance starts unstalled.
    const spinner = this.host.state.activitySpinner?.instance;
    if (spinner !== undefined && spinner.isStalled() !== stalled) {
      spinner.setStalled(stalled);
    }
  }

  private stopStallWatch(): void {
    if (this.stallWatchTimer !== undefined) {
      clearInterval(this.stallWatchTimer);
      this.stallWatchTimer = undefined;
    }
    this.lastTokenAtMs = 0;
    if (this.stalledApplied) this.applyStalled(false);
  }
  ...
  finalizeTurn(sendQueued: (item: QueuedMessage) => void): void {
    ...
    notifyTerminalOnce(state, `turn-complete:${completedTurnKey}`, {
      title: t('footer.notify.taskComplete'),
  ...
  onToolCallStart(toolCall: ToolCallBlockData): void {
    ...
    if (this._pendingToolGroup !== null && this._pendingToolGroup.toolName !== toolCall.name) {
      this._pendingToolGroup = null;
    }

    let handled = this.tryAttachAgentToolCall(toolCall, tc);
    if (!handled) handled = this.tryAttachReadToolCall(toolCall, tc);
    if (!handled) handled = this.tryAttachToolGroupCall(toolCall, tc);
    if (!handled) {
  ...
  beginCompaction(instruction?: string, tokensBefore?: number): void {
    const { state } = this.host;
    if (this._activeCompactionBlock !== undefined) {
      this._activeCompactionBlock.markDone();
      this._activeCompactionBlock = undefined;
    }
    this.compactionStartedAtMs = Date.now();
    const workingTip = currentWorkingTip()?.text;
    const block = new CompactionComponent(
      state.ui,
      instruction,
      workingTip === undefined ? undefined : resolveDescription(workingTip),
      tokensBefore,
    );
  ...
  endCompaction(tokensBefore?: number, tokensAfter?: number, summary?: string): void {
    ...
    this._activeCompactionBlock = undefined;
    // A completed compaction is a long task node: leave the same quiet gray
    // completion line a finished turn would.
    const startedAtMs = this.compactionStartedAtMs;
    this.compactionStartedAtMs = undefined;
    if (startedAtMs !== undefined) {
      state.transcriptContainer.addChild(
        new TurnCompletionComponent(formatTurnCompletionLine(Date.now() - startedAtMs)),
      );
    }
    state.ui.requestRender();
  }
  ...
  /**
   * Same-tool grouping, mirroring the read-group mechanics: the first call
   * mounts standalone and is upgraded to a group in place when a second call
   * of the same tool follows within the same step, so a run grows live as new
   * calls stream in. A run breaks on assistant text/thinking (the pending
   * group is cleared when those start) and on step boundaries (live resets
   * pending state on step begin; replay bumps the step per assistant
   * message), so replayed history forms the same groups the live view did.
   */
  private tryAttachToolGroupCall(toolCall: ToolCallBlockData, tc: ToolCallComponent): boolean {
    const { state } = this.host;
    if (!GROUPABLE_TOOL_NAMES.has(toolCall.name)) {
      this._pendingToolGroup = null;
      return false;
    }
    ...
    const step = toolCall.step ?? this._currentStep;
    const turnId = toolCall.turnId ?? this._currentTurnId;
    const pending = this._pendingToolGroup;

    if (
      pending !== null &&
      (pending.step !== step || pending.turnId !== turnId || pending.toolName !== toolCall.name)
    ) {
      this._pendingToolGroup = null;
    }

    const cur = this._pendingToolGroup;
    if (cur === null) {
      this._pendingToolGroup = { step, turnId, toolName: toolCall.name, solo: tc };
      state.transcriptContainer.addChild(tc);
      state.ui.requestRender();
      return true;
    }
    ...
    const group = this.upgradeSoloToToolGroup(solo, toolCall.name);
    group.attach(toolCall.id, tc);
    this._pendingToolGroup = { step, turnId, toolName: toolCall.name, group };
    state.ui.requestRender();
    return true;
  }

  private upgradeSoloToToolGroup(solo: ToolCallComponent, toolName: string): ToolGroupComponent {
    const { state } = this.host;
    const group = new ToolGroupComponent(toolName, state.ui);
    const children = state.transcriptContainer.children;
    const idx = children.indexOf(solo);
    if (idx >= 0) {
      // In-place replacement is picked up by the container's ref-checked
