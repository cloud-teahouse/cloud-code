...
export class SubAgentEventHandler {
  ...
  clearAgentSwarmProgress(): void {
    ...
    // Optional: controller unit tests hand-roll partial state without the zone.
    this.host.state.swarmContainer?.clear();
  ...
  handleAgentSwarmToolResult(
    ...
    if (isError && isUserCancelledSubagentResult(resultData)) {
      ...
      progress.markToolCallEnded();
      if (!progress.applyResult(resultData.output, resultData.structured)) {
        progress.markSwarmFailed(resultData.output);
      }
    } else {
      progress.markToolCallEnded();
      progress.applyResult(resultData.output, resultData.structured);
    }
    // The swarm's tool call ended: the finished grid leaves the pinned slot
    // and becomes a transcript record.
    this.finalizeAgentSwarmProgress(progress);
    this.host.updateActivityPane();
    this.requestRender();
  }
  ...
  private handleForegroundSubagentFailed(
    ...
    const { parentToolCallId } = info;
    if (this.updateAgentSwarmProgress(parentToolCallId, (progress) => {
      this.markAgentSwarmFailedOrCancelled(progress, event.subagentId, event.error, event.errorKind);
    })) {
      this.host.streamingUI.removeToolComponentIfInactive(parentToolCallId);
      return;
    }
  ...
  private ensureAgentSwarmProgress(
    ...
    // While the swarm runs, its grid is pinned in the slot right above the
    // editor; completion moves it into the transcript as the history record.
    this.host.state.swarmContainer.addChild(progress);
    this.host.updateActivityPane();
    this.requestRender();
    return progress;
  }

  /** Move a finished swarm grid from the pinned slot into the transcript. */
  private finalizeAgentSwarmProgress(progress: AgentSwarmProgressComponent): void {
    const container = this.host.state.swarmContainer;
    const index = container.children.indexOf(progress);
    if (index >= 0) {
  ...
  private removeAgentSwarmProgress(
    ...
    progress.dispose();
    const children = this.host.state.swarmContainer.children;
  ...
  private agentSwarmGridHeight(): number | undefined {
    ...
    const rowsAfterSwarm = state.slotContainer.children
      .filter((child) => child !== state.swarmContainer)
  ...
  private markAgentSwarmFailedOrCancelled(
    ...
    // The event's structured error kind wins; the English sentence match is
    // the fallback for events from older cores.
    if (errorKind === 'user_cancelled' || isUserCancelledSubagentError(error)) {
...
/**
 * Cancellation detection for a whole-result AgentSwarm error. New sessions
 * mark the result with a structured error kind; older records are matched
 * by the English cancellation sentences via
 * {@link isUserCancelledSubagentError}.
 */
function isUserCancelledSubagentResult(result: ToolResultBlockData): boolean {
  ...
  return isUserCancelledSubagentError(result.output);
}
