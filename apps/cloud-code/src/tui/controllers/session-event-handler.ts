...
import { TurnCompletionComponent } from '../components/messages/turn-completion';
import {
...
import { translateErrorPayload } from '../utils/error-i18n';
import {
  readGoalQueue,
  removeGoalQueueItem,
...
import { formatCountdownMmSs, formatTurnCompletionLine } from '../utils/turn-completion';
...
import { TeamTracker } from './teams-tracker';
import type {
  AppState,
  LivePaneState,
  QueuedMessage,
...
/**
 * T2 interrupt recall payload: the submitted text to restore into the editor,
 * plus the transcript entries created for it (the optimistic echo), which the
 * caller drops so the recalled message no longer renders as a normal message.
 */
export interface InterruptRecall {
  readonly text: string;
  readonly transcriptEntryIds: readonly string[];
}
...
export class SessionEventHandler {
  ...
  /**
   * Every transcript component currently showing MCP server status (spinners
   * and finalized rows, including rows orphaned by a later status transition).
   * MCP status rows are live session diagnostics, not history: a runtime reset
   * that KEEPS the transcript (/reload) removes them, so the post-reset
   * snapshot sync renders each server's current status exactly once instead of
   * duplicating the stale rows.
   */
  private mcpServerStatusComponents: Set<Component> = new Set();
  ...
  /**
   * T2: whether the in-flight turn has produced any user-visible assistant
   * output (assistant text delta, hook result content, or a tool call).
   * Drives Esc / Ctrl+C "interrupt before output → recall input for
   * re-editing". Thinking deltas intentionally do not count.
   */
  private currentTurnHasVisibleOutput = false;
  /**
   * T2: text of the user input that started the in-flight turn, recorded by
   * the host's submit paths. Recalled into the editor when the turn is
   * cancelled before any visible output; cleared on turn end so a stale
   * prompt can never leak into a later (e.g. cron-fired) turn.
   */
  recallableTurnInput: string | undefined;
  /**
   * T2: transcript entries created for `recallableTurnInput` (the optimistic
   * prompt echo / steer echo). Removed from the transcript when the recall
   * fires, matching the server-side withdrawal of the interrupted input.
   */
  recallableTranscriptEntryIds: readonly string[] | undefined;
  /**
   * Per-turn token usage summed from `turn.step.completed` events (a turn can
   * span several steps). Reset on `turn.started`; mirrored into AppState so
   * the footer can render the in/cache/out breakdown.
   */
  private currentTurnUsage: TokenUsage | null = null;
  /** Wall-clock start of the in-flight turn, for the turn completion line. */
  private currentTurnStartedAtMs = 0;
  private currentTurnFirstTokenRecorded = false;

  /**
   * First-token latency (turn start → first visible delta), recorded once per
   * turn into the rolling last-10 window shown in the footer.
   */
  private noteFirstTokenLatency(): void {
    if (this.currentTurnStartedAtMs === 0 || this.currentTurnFirstTokenRecorded) return;
    this.currentTurnFirstTokenRecorded = true;
    const latencies = (this.host.state.appState.recentFirstTokenLatencies ??= []);
    latencies.push(Date.now() - this.currentTurnStartedAtMs);
  ...
  /** 1s ticker driving the rate-limit pause countdown notice. */
  private rateLimitPauseTicker: ReturnType<typeof setInterval> | null = null;
  ...
  resetRuntimeState(): void {
    ...
    this.currentTurnHasVisibleOutput = false;
    this.recallableTurnInput = undefined;
    this.recallableTranscriptEntryIds = undefined;
    this.currentTurnUsage = null;
    this.currentTurnStartedAtMs = 0;
    this.currentTurnFirstTokenRecorded = false;
    this.host.state.appState.recentFirstTokenLatencies = [];
    ...
    this.clearRateLimitPause();
    this.stopAllMcpServerStatusSpinners();
    this.removeMcpServerStatusComponents();
  }
  ...
  /**
   * T2 "interrupt before output → recall for re-edit": returns the submitted
   * input to restore into the editor when the in-flight turn has not produced
   * any visible assistant output yet, consuming the record. Returns undefined
   * once output has streamed, or when nothing was recorded (bash `!` commands
   * and skill/plugin invocations are never recorded). The carried transcript
   * entry ids let the caller drop the recalled echo from the transcript, so
   * it no longer renders as a normal message.
   */
  consumeInterruptRecall(): InterruptRecall | undefined {
    if (this.currentTurnHasVisibleOutput) return undefined;
    const text = this.recallableTurnInput;
    if (text === undefined) return undefined;
    const transcriptEntryIds = this.recallableTranscriptEntryIds ?? [];
    this.recallableTurnInput = undefined;
    this.recallableTranscriptEntryIds = undefined;
    return { text, transcriptEntryIds };
  }
  ...
  async syncMcpServerStatusSnapshot(session: Session): Promise<void> {
    ...
    try {
      ...
      host.showError(t('session.mcp.syncFailed', { error: message }));
      return;
    }
  ...
  handleEvent(event: Event, sendQueued: (item: QueuedMessage) => void): void {
    ...
    // a straggler carrying a mandatory turnId (e.g. tool.progress) must not
    // resurrect the finished turn — hasActiveTurn() gates finishCompaction's
    // queue-dispatch branch on this staying clean.
    if (
      'turnId' in event &&
      event.turnId !== undefined &&
      (event.type === 'turn.started' || this.host.streamingUI.hasActiveTurn())
    ) {
    ...
    switch (event.type) {
      ...
      case 'turn.rate_limit_paused': this.handleRateLimitPaused(event); break;
      case 'turn.rate_limit_resuming': this.handleRateLimitResuming(event); break;
      ...
      // The local echo appended at submit time is the optimistic entry for
      // this event; confirm it instead of rendering a second user bubble.
      case 'prompt.submitted': this.host.confirmUserEcho(); break;
  ...
  /**
   * Splice the rendered MCP status rows out of the transcript. Only meaningful
   * when the transcript itself survives the reset (/reload): the rows on screen
   * were rendered from the pre-reset connection state and are stale. Session
   * switches clear the whole transcript right after the reset, so the splice
   * is a no-op there. Replay never re-renders these rows from history — the
   * snapshot sync after subscribe shows the last-known status once.
   */
  private removeMcpServerStatusComponents(): void {
    if (this.mcpServerStatusComponents.size === 0) return;
    const children = this.host.state.transcriptContainer.children;
    for (const component of this.mcpServerStatusComponents.values()) {
      const idx = children.indexOf(component);
      if (idx >= 0) children.splice(idx, 1);
    }
    this.mcpServerStatusComponents.clear();
  }
  ...
  private handleTurnBegin(event: TurnStartedEvent): void {
    // Any new turn (a resumed auto-retry, or user input that cancelled the
    // parked timer core-side) retires the pause countdown line.
    this.clearRateLimitPause();
    this.currentTurnHasAssistantText = false;
    this.currentTurnHasVisibleOutput = false;
    this.currentTurnUsage = null;
    this.currentTurnStartedAtMs = Date.now();
    this.currentTurnFirstTokenRecorded = false;
    ...
    // Baseline for stall detection: the TTFT wait counts as "tokens expected".
    this.host.streamingUI.noteTokenActivity();
  ...
  private handleTurnEnd(event: TurnEndedEvent, sendQueued: (item: QueuedMessage) => void): void {
    ...
    if (event.reason === 'failed' && event.error?.code === 'provider.filtered') {
      this.host.showStatus(t('session.turn.stoppedFiltered'), 'error', { transcript: true });
    }
    if (event.reason === 'blocked') {
      this.host.showStatus(t('session.turn.stoppedBlocked'), 'error', { transcript: true });
    }
    ...
    this.renderTurnCompletionLine(event);
  ...
  /**
   * One quiet "✻ Cogitated for 10s"-style gray line in the transcript flow
   * when a turn completes successfully — gives the turn a sense of landing.
   * Cancelled/failed/blocked turns already leave explicit status lines, and
   * replayed history is not news, so both are skipped.
   */
  private renderTurnCompletionLine(event: TurnEndedEvent): void {
    if (event.reason !== 'completed') return;
    if (this.host.state.appState.isReplaying) return;
    if (this.currentTurnStartedAtMs === 0) return;
    const durationMs = Date.now() - this.currentTurnStartedAtMs;
    this.currentTurnStartedAtMs = 0;
    this.host.state.transcriptContainer.addChild(
      new TurnCompletionComponent(formatTurnCompletionLine(durationMs)),
    );
  ...
  private handleStepBegin(event: TurnStepStartedEvent): void {
    ...
    // A new step re-arms the stall baseline: the wait for its first token
    // counts as "tokens expected" again.
    this.host.streamingUI.noteTokenActivity();
    this.host.patchLivePane({
      mode: 'waiting',
      pendingApproval: null,
  ...
  private handleStepCompleted(event: TurnStepCompletedEvent): void {
    ...
    if (event.providerFinishReason === 'filtered') {
      this.host.showNotice(
        t('session.turn.filteredTitle'),
        t('session.turn.filteredDetail', { reason: event.rawFinishReason ?? 'content_filter' }),
        { transcript: true },
      );
      return;
    }
    ...
    const title =
      truncatedCount > 0
        ? t('session.turn.maxTokensTruncated')
        : t('session.turn.maxTokensNoToolCall');
    const detail = this.isAnthropicSessionActive()
      ? t('session.turn.maxTokensAnthropicDetail')
      : undefined;
    this.host.showNotice(title, detail, { transcript: true });
  }

  /**
   * In-backoff presence: the retry loop is sleeping between attempts;
   * show that in the transient notice slot instead of looking hung. Each
   * `turn.step.retrying` replaces the previous notice.
   */
  private handleStepRetrying(event: TurnStepRetryingEvent): void {
    this.host.showStatus(
      t('session.turn.retrying', {
  ...
  /**
   * Rate-limit pause: the turn ended as a pause and the core parked
   * an auto-resume timer. Render the countdown line ("Rate limited —
   * auto-retry in mm:ss (Esc to cancel)") in the notice slot, ticking every
   * second; cleared by `turn.rate_limit_resuming`, a new turn, or Esc.
   */
  private handleRateLimitPaused(event: TurnRateLimitPausedEvent): void {
    this.host.setAppState({
      rateLimitPause: { resumeAtMs: event.resumeAtMs, attempt: event.attempt },
    });
    this.stopRateLimitPauseTicker();
    const render = (): void => {
      const remainingMs = event.resumeAtMs - Date.now();
      if (remainingMs <= 0) {
        // `turn.rate_limit_resuming` (or a cancel) normally retires the line;
        // stop the ticker at zero so it never shows a negative wait.
        this.stopRateLimitPauseTicker();
        return;
      }
      this.host.showStatus(
        t('session.turn.rateLimitPaused', { time: formatCountdownMmSs(remainingMs) }),
        'warning',
      );
    };
    render();
    this.rateLimitPauseTicker = setInterval(render, 1000);
    // The countdown is a nicety, never a reason to keep the process alive.
    if (typeof this.rateLimitPauseTicker === 'object') this.rateLimitPauseTicker.unref?.();
  }

  private handleRateLimitResuming(event: TurnRateLimitResumingEvent): void {
    this.clearRateLimitPause();
    this.host.showStatus(t('session.turn.rateLimitResuming', { attempt: event.attempt }));
  }

  /**
   * Retire the pause countdown line (ticker + AppState marker). Idempotent;
   * also the Esc-cancel hook from the host's `cancelRateLimitPause`.
   */
  clearRateLimitPause(): void {
    this.stopRateLimitPauseTicker();
    const pause = this.host.state.appState.rateLimitPause;
    if (pause !== null && pause !== undefined) {
      this.host.setAppState({ rateLimitPause: null });
    }
  }

  private stopRateLimitPauseTicker(): void {
    if (this.rateLimitPauseTicker === null) return;
    clearInterval(this.rateLimitPauseTicker);
    this.rateLimitPauseTicker = null;
  }

  /**
   * Sum a step's usage into the per-turn accumulator and mirror it into
   * AppState for the footer breakdown. Steps without usage (provider did not
   * report) leave the accumulator untouched.
   */
  private accumulateTurnUsage(usage: TokenUsage | undefined): void {
    if (usage === undefined) return;
    const prev = this.currentTurnUsage;
    this.currentTurnUsage = {
      inputOther: (prev?.inputOther ?? 0) + usage.inputOther,
      inputCacheRead: (prev?.inputCacheRead ?? 0) + usage.inputCacheRead,
      inputCacheCreation: (prev?.inputCacheCreation ?? 0) + usage.inputCacheCreation,
      output: (prev?.output ?? 0) + usage.output,
    };
    this.host.setAppState({ turnUsage: this.currentTurnUsage });
  }

  private maybeShowDebugTiming(event: TurnStepCompletedEvent): void {
    if (process.env['CLOUD_CODE_DEBUG'] !== '1') return;
  ...
  private handleStepInterrupted(event: TurnStepInterruptedEvent): void {
    ...
    if (reason === 'aborted' || reason === undefined || reason === '') {
      ...
      if (event.message === undefined || event.message === '') {
        this.host.showStatus(t('session.turn.interrupted'), 'error', { transcript: true });
      } else {
        this.host.showError(event.message, { transcript: true });
      }
      return;
    }
    this.host.showError(
      reason === 'max_steps'
        ? t('session.turn.maxSteps')
        : t('session.turn.stepInterrupted', { reason }),
      { transcript: true },
    );
  }
  ...
  private handleAssistantDelta(event: AssistantDeltaEvent): void {
    ...
    if (event.delta.trim().length > 0) {
      ...
      this.currentTurnHasVisibleOutput = true;
      this.pendingModelBlockedFallback = undefined;
    }
  ...
  private handleToolCall(event: ToolCallStartedEvent): void {
    ...
    // A started tool call is visible assistant output (T2).
    this.currentTurnHasVisibleOutput = true;
    ...
    if (matchedCall !== undefined && matchedCall.name === 'TodoList' && !event.isError) {
      ...
      if (Array.isArray(rawTodos)) {
        const sanitized = rawTodos
          .filter((todo): todo is { title: string; status: 'pending' | 'in_progress' | 'done'; activeForm?: unknown } =>
            isTodoItemShape(todo),
          )
          .map((t) => {
            const activeForm = todoActiveForm(t);
            return {
              title: t.title,
              status: t.status,
  ...
  private handleStatusUpdate(event: AgentStatusUpdatedEvent): void {
    ...
    if (event.coordinatorMode !== undefined) patch.coordinatorMode = event.coordinatorMode;
  ...
  private async promoteNextQueuedGoal(): Promise<boolean> {
    ...
    try {
      ...
      host.showError(t('session.goal.queueReadFailed', { error: formatErrorMessage(error) }));
      return false;
    }
      ...
      next.objective,
      {
        beforeSend: async () => {
          ...
          try {
            ...
            host.showError(
              t('session.goal.queueRemoveFailed', { error: formatErrorMessage(error) }),
            );
            await this.cancelStartedQueuedGoal(session);
            return false;
          }
  ...
  private async restoreAndCancelStartedQueuedGoal(
    ...
    try {
      ...
      this.host.showError(t('session.goal.restoreFailed', { error: formatErrorMessage(error) }));
    }
    await this.cancelStartedQueuedGoal(session);
  }
  ...
  private async cancelStartedQueuedGoal(session: Session): Promise<void> {
    try {
      ...
      this.host.showError(t('session.goal.cancelFailed', { error: formatErrorMessage(error) }));
    }
  }
  ...
  private async notifyQueuedGoalWaitingOnBlocked(): Promise<void> {
    ...
    host.showNotice(
      t('session.goal.blockedTitle'),
      t('session.goal.blockedDetail'),
    );
  }
  ...
  private handleSessionError(event: ErrorEvent): void {
    ...
    this.host.streamingUI.resetToolUi();
    this.host.streamingUI.finalizeLiveTextBuffers('idle');
    if (event.code === 'provider.rate_limit' && event.details?.['autoResume'] === true) {
      // A rate-limit pause presents as the countdown line driven by
      // `turn.rate_limit_paused`, not as a transcript error dump.
      return;
    }
    if (event.code === OAUTH_LOGIN_REQUIRED_CODE) {
      this.host.showError(resolveDescription(OAUTH_LOGIN_REQUIRED_STARTUP_NOTICE), {
        transcript: true,
      });
      return;
    }
    this.host.showError(translateErrorPayload(event) ?? formatErrorPayload(event), {
      transcript: true,
    });
    const sessionId = this.host.state.appState.sessionId;
    if (sessionId.length > 0) {
      this.host.showStatus(errorReportHintLine(), undefined, { transcript: true });
    }
  }

  private handleSessionWarning(event: WarningEvent): void {
    this.host.showStatus(t('session.warning', { message: event.message }), 'warning', {
      transcript: true,
    });
  }
  ...
  private renderMcpServerStatus(server: McpServerStatusSnapshot): void {
    ...
    switch (server.status) {
      case 'connected': {
        const toolStr =
          server.toolCount === 1
            ? t('session.mcp.tools.one', { count: server.toolCount })
            : t('session.mcp.tools.other', { count: server.toolCount });
        const message = t('session.mcp.connected', {
          name: server.name,
          tools: toolStr,
      ...
      case 'failed': {
        const message =
          server.error !== undefined
            ? t('session.mcp.failedWithError', { name: server.name, error: server.error })
            : t('session.mcp.failed', { name: server.name });
        this.finalizeMcpServerStatusRow(server.name, message, 'error');
        return;
      }
      case 'needs-auth': {
        const message = t('session.mcp.needsAuth', { name: server.name });
      ...
      case 'disabled':
        this.finalizeMcpServerStatusRow(
          server.name,
          t('session.mcp.disabled', { name: server.name }),
          'textMuted',
        );
        return;
      case 'pending':
  ...
  private showMcpServerStatusSpinner(name: string): void {
    const { state } = this.host;
    const label = t('session.mcp.connecting', { name });
    ...
    this.mcpServerStatusComponents.add(spinner);
    state.ui.requestRender();
  }
  ...
  private finalizeMcpServerStatusRow(name: string, message: string, color: ColorToken): void {
    ...
    this.mcpServerStatusComponents.add(status);
    const spinner = this.mcpServerStatusSpinners.get(name);
    if (spinner === undefined) {
      // MCP server status rows are append-only, dedup'd session diagnostics
      // (failures are worth keeping) — they belong to the transcript, not the
      // single-slot notice area.
  ...
  private handleSkillActivated(event: SkillActivatedEvent): void {
    ...
    this.host.appendTranscriptEntry({
      ...
      turnId: undefined,
      renderMode: 'plain',
      content: t('session.skill.activated', { name: event.skillName }),
  ...
  private handleCompactionBegin(event: CompactionStartedEvent): void {
    ...
    this.host.streamingUI.beginCompaction(event.instruction, event.tokensBefore);
  }
