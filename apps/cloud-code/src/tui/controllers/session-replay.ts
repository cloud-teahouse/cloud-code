...
import { formatErrorMessage, isTodoItemShape, todoActiveForm } from '../utils/event-payload';
...
export class SessionReplayRenderer {
  ...
  async hydrateFromReplay(session: Session): Promise<boolean> {
    ...
    try {
      ...
      if (main === undefined) {
        this.host.showError(t('controllers.replay.historyUnavailable'));
        return false;
      }
      ...
      this.host.showError(t('status.replayFailed', { message }));
      return false;
    } finally {
  ...
  private hydrateTodoPanel(agent: ResumedAgentState): void {
    ...
    const todos = rawTodos
      ...
      .map((todo) => {
        const activeForm = todoActiveForm(todo);
        return {
          title: todo.title,
          status: todo.status,
      ...
      case 'plan_updated':
        ...
        this.host.appendTranscriptEntry(
          replayEntry(
            context,
            'status',
            record.enabled ? t('commands.plan.on') : t('commands.plan.off'),
            'notice',
          ),
        );
        return;
  ...
  private renderSkillActivation(
    ...
    this.host.appendTranscriptEntry({
      ...replayEntry(context, 'skill_activation', t('session.skill.activated', { name: skill.skillName }), 'plain'),
  ...
  private renderCompaction(context: ReplayRenderContext, record: CompactionReplayRecord): void {
    ...
    if (record.result === 'cancelled') {
      this.host.appendTranscriptEntry({
        ...replayEntry(context, 'status', t('selectors.compaction.cancelled'), 'plain'),
        compactionData: {
          result: 'cancelled',
    ...
    this.host.appendTranscriptEntry({
      ...replayEntry(context, 'status', t('selectors.compaction.complete'), 'plain'),
      compactionData: {
  ...
  private renderGoalReplayRecord(context: ReplayRenderContext, record: GoalReplayRecord): void {
    ...
    switch (change.kind) {
      case 'created':
        this.host.appendTranscriptEntry({
          ...replayEntry(context, 'goal', t('panels.goal.set'), 'plain'),
  ...
  private renderPermissionUpdate(context: ReplayRenderContext, mode: PermissionMode): void {
    if (mode === 'yolo') {
      this.host.appendTranscriptEntry(
        replayEntry(context, 'status', t('commands.yolo.on'), 'notice', {
          detail: t('commands.yolo.onDetail'),
        }),
      );
      return;
    }
    this.host.appendTranscriptEntry(
      replayEntry(
        context,
        'status',
        mode === 'manual' ? t('commands.yolo.off') : t('commands.permission.mode', { mode }),
        'notice',
      ),
    );
  }
      ...
      case 'rejected':
        content =
          result.selectedLabel === 'Revise'
            ? t('controllers.replay.planSentBackForRevision')
            : t('controllers.replay.planReviewRejected');
        break;
      case 'cancelled':
        content = t('controllers.replay.planReviewCancelled');
        break;
    }
    const detail =
      result.feedback !== undefined && result.feedback.length > 0
        ? t('controllers.replay.feedbackDetail', { feedback: result.feedback })
        : undefined;
  ...
  private renderBackgroundTaskNotification(
    ...
    // Map the terminal origin onto headline variants via `failureKind` —
    // locale-safe (the previous English-fragment headline.replace(...) silently
    // no-ops once the headline is translated).
    const failureKind =
      origin.status === 'lost'
        ? ('lost' as const)
        : origin.status === 'killed'
          ...
          : origin.status === 'timed_out'
            ? ('timedOut' as const)
            : undefined;
    const status = formatBackgroundAgentTranscript(
      origin.status === 'completed' ? 'completed' : 'failed',
      meta,
      failureKind === undefined ? undefined : { failureKind },
    );
    this.host.appendTranscriptEntry({
      ...replayEntry(context, 'status', status.headline, 'plain'),
      detail: status.detail,
...
function isResumeNormalizationGoalPause(change: GoalReplayLifecycleChange): boolean {
  // New records carry the machine reason code; the English reason set is
  // the fallback for records written before it existed (including the
  // legacy 'Paused after session resume', which has no live emitter).
  if (change.reasonCode !== undefined) {
    return (
      change.status === 'paused' &&
      (change.reasonCode === 'agent_resume' || change.reasonCode === 'session_resume')
    );
  }
  return (
    change.status === 'paused' &&
...
function goalLifecycleReplayContent(change: GoalReplayLifecycleChange): string {
  switch (change.status) {
    ...
    case 'blocked':
      return t('panels.goal.marker.blocked');
    case 'complete':
    case undefined:
      return t('controllers.replay.goalUpdated');
  }
}
