...
import {
  CLOUD_CODE_PLUGIN_MARKETPLACE_URL,
  CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
export interface PluginMarketplaceEntry {
  ...
  /** Name of the registered marketplace this entry came from. Set by the
   * caller when several catalogs are merged (the parser leaves it undefined);
   * rendered as an `@<name>` badge in the plugins panel. */
  readonly marketplace?: string;
}

export interface PluginMarketplace {
  readonly source: string;
  /** Declared marketplace name (Claude Code manifests require it; kimi-market
   * manifests usually omit it). */
...
export async function loadPluginMarketplace(
  ...
  const configuredSource = options.source ?? process.env[CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV];
  const location = resolveMarketplaceLocation(
    configuredSource ?? CLOUD_CODE_PLUGIN_MARKETPLACE_URL,
    options.workDir,
  );
...
export function parsePluginMarketplace(raw: string, location: MarketplaceLocation): PluginMarketplace {
  ...
  return {
    ...
    description:
      ...
      (isRecord(parsed['metadata']) ? stringField(parsed['metadata'], 'description') : undefined),
    version:
      stringField(parsed, 'version') ??
      (isRecord(parsed['metadata']) ? stringField(parsed['metadata'], 'version') : undefined),
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV} cannot be empty.`);
  }
...
function parseMarketplaceEntry(
  ...
  // kimi-market entries use "id"; Claude Code marketplace entries use "name".
  const id = stringField(value, 'id') ?? stringField(value, 'name');
  if (id === undefined) {
    throw new Error(`Plugin marketplace entry ${index + 1} must define "id" (or "name").`);
  }
  validateMarketplaceEntryType(value, id);
  const resolvedSource = parseEntrySource(value, id, location);
  return {
    id,
...
/**
 * Resolve an entry's plugin source. The kimi-market form is a plain string
 * (zip/github URL or path, relative to the manifest). Claude Code marketplace
 * entries may instead use an object — `{source: "github", repo, ref?, sha?}`
 * or `{source: "url", url}` — which maps onto the install sources the plugin
 * manager understands; kinds it cannot install (npm, pip, git-subdir,
 * non-GitHub git URLs) are rejected with an explicit error.
 */
function parseEntrySource(
  ...
  const raw = value['source'] ?? value['url'] ?? value['downloadUrl'];
  if (typeof raw === 'string' && raw.trim().length > 0) {
    return resolveEntrySource(raw, location);
  }
  if (isRecord(raw)) {
    return resolveObjectEntrySource(raw, id);
  }
  throw new Error(`Plugin marketplace entry ${id} must define "source".`);
}

function resolveObjectEntrySource(source: Record<string, unknown>, id: string): string {
  const kind = source['source'];
  if (kind === 'github') {
    const repo = stringField(source, 'repo');
    if (repo === undefined) {
      throw new Error(`Plugin marketplace entry ${id}: github source must define "repo".`);
    }
    const base = `https://github.com/${repo}`;
    // `/commit/<sha>` pins an exact commit; `/tree/<ref>` covers both branches
    // and tags (the install resolver lets codeload pick). See plugin/source.ts.
    const sha = stringField(source, 'sha');
    if (sha !== undefined) return `${base}/commit/${sha}`;
    const ref = stringField(source, 'ref');
    return ref === undefined ? base : `${base}/tree/${ref}`;
  }
  if (kind === 'url') {
    const url = stringField(source, 'url');
    if (url === undefined) {
      throw new Error(`Plugin marketplace entry ${id}: url source must define "url".`);
    }
    // Claude Code's "url" source is a git repository to clone; the plugin
    // manager installs GitHub repos and plain zip URLs, so only github.com
    // git URLs map onto a supported install source.
    if (!/^https:\/\/(www\.)?github\.com\//i.test(url)) {
      throw new Error(
        `Plugin marketplace entry ${id}: git URL sources are only supported for github.com repositories.`,
      );
    }
    return url;
  }
  throw new Error(
    `Plugin marketplace entry ${id}: source kind "${String(kind)}" is not supported ` +
      '(supported: relative paths, github, url, or a plain source string).',
  );
}
...
function resolveEntrySource(source: string, location: MarketplaceLocation): string {
  ...
  // Claude Code resolves relative plugin sources against the marketplace root
  // (the directory containing .claude-plugin/, not .claude-plugin/ itself);
  // kimi-market manifests sit at the root, so their dirname is already right.
  const manifestDir = dirname(location.resolved);
  const baseDir =
...
export function resolveLocalPath(input: string, workDir: string): string {
