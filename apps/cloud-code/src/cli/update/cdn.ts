...
export interface FetchLatestResult {
  /** Raw newest version — what `cloud-code upgrade` installs, never rollout-gated. */
...
/**
 * Fetch the latest published Cloud Code version from the CDN.
 *
...
export async function fetchLatestVersionFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, CLOUD_CODE_CDN_LATEST_URL);
  if (!response.ok) {
...
async function fetchUpdateManifestFromCdn(fetchImpl: typeof fetch): Promise<UpdateManifest> {
  const response = await fetchWithTimeout(fetchImpl, CLOUD_CODE_CDN_LATEST_JSON_URL);
  if (!response.ok) {
