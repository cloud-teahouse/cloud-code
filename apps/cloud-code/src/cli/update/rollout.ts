...
import { readKimiDeviceId } from '@cloud-code/oauth';
...
export type PassiveUpdateReason =
  ...
  | 'eligible'
  /** CLOUD_CODE_EXPERIMENTAL_FLAG is on: rollout skipped, newest always visible. */
  | 'experimental';
 ...
 *
 * `cloud-code upgrade` must NOT go through this gate — it selects directly from the
 * raw latest version.
 */
...
/**
 * Stable per-installation id used for bucketing when the OAuth layer has
 * already minted one (for its X-Msh-Device-Id header). Missing ids stay
 * ephemeral here so update preflight never creates the device_id file itself.
 */
export function resolveUpdateDeviceId(): string {
  return readKimiDeviceId(resolveCloudCodeHome()) ?? randomUUID();
}
 ...
 * truthy values as `CLOUD_CODE_NO_AUTO_UPDATE`) — the update preflight runs
 * before the harness exists, so the core flag registry is not consulted.
 * `CLOUD_CODE_NO_AUTO_UPDATE` still wins: disabling updates beats opting in.
 */
export function isRolloutBypassedByExperimentalEnv(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  const value = (env['CLOUD_CODE_EXPERIMENTAL_FLAG'] ?? '').trim().toLowerCase();
