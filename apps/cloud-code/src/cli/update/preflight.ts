...
export function installCommandFor(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return 'brew upgrade cloud-code';
    case 'native':
...
export function spawnForSource(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return { cmd: 'brew', args: ['upgrade', 'cloud-code'] };
    case 'native':
...
function renderBackgroundInstallSuccessNotice(version: string): string {
  ...
  return `Cloud Code CLI updated to ${displayVersion}\nChangelog: ${CHANGELOG_URL}\n`;
}
...
/**
 * `CLOUD_CODE_NO_AUTO_UPDATE` (or the legacy `KIMI_CLI_NO_AUTO_UPDATE` alias)
...
function isAutoUpdateDisabledByEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  return truthy(env['CLOUD_CODE_NO_AUTO_UPDATE']) || truthy(env['KIMI_CLI_NO_AUTO_UPDATE']);
}
