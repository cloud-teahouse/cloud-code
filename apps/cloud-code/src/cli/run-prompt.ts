...
import { resolveOutputFormat, selectHarnessTransport } from './options';
 ...
 * timer stays ref'd so a cleanup step that suspends on an unref'd handle can't
 * drain the event loop and exit 0 before the rejection propagates — the timer
 * keeps the loop alive until it fires, then gives the rejection a chance to
 * surface. A wedged cleanup is still bounded by `timeoutMs`, so this can't hang
 * the run forever.
 */
...
export async function runPrompt(
  ...
  const harness = await createPromptHarness({
    ...
    transport: selectHarnessTransport(opts),
  });
  log.info('cloud-code starting', {
    version,
    uiMode: PROMPT_UI_MODE,
  ...
  const cleanupPromptRun = async (): Promise<void> => {
    ...
    // the background if it overruns; the caller (`cloud-code -p`) force-exits shortly
  ...
  try {
    ...
    // Headless goal mode: `cloud-code -p "/goal <objective>"`. The goal driver keeps
...
async function createPromptHarness(
  options: Parameters<typeof createCloudCodeHarness>[0],
): Promise<PromptHarness> {
  // The v2 engine is dispatched earlier in `runPrompt` (see the
  // `isCloudCodeV2Enabled()` branch) and never reaches here; this is the v1 path.
  return createCloudCodeHarness(options);
}
      ...
      stderr.write(
        `${chalk.hex('#E8A838')(
          `Session "${opts.session}" was created under a different directory.\n` +
            `  cd "${target.workDir}" && cloud-code -r ${opts.session}`,
        )}\n\n`,
      );
      throw new Error(
        `Session "${opts.session}" was created under a different directory.`,
...
export function requireConfiguredModel(...models: readonly (string | undefined)[]): string {
  ...
  if (model === undefined) {
    throw new Error(
      'No model configured. Run `cloud-code` and use /login to sign in, then retry; or set default_model in config.toml.',
    );
  }
  return model;
}
...
function runPromptTurn(
  ...
  // A `cloud-code -p` run is not done just because the model ended a turn: an active
