/**
 * `cloud-code provider` sub-command — non-interactive provider management.
 *
 * Mirrors the TUI `/provider` flow (apps/cloud-code/src/tui/commands/provider.ts)
 ...
 * (apps/cloud-code/src/tui/utils/refresh-providers.ts) groups by URL, retries
...
export interface ProviderDeps {
  readonly getHarness: () => CloudCodeHarness;
...
export async function handleProviderAdd(
  ...
  // `core-impl.ts removeCloudCodeProvider`), so calling it inside the apply loop
...
export async function handleCatalogAdd(
  ...
  if (opts.defaultModel !== undefined && !models.some((m) => m.id === opts.defaultModel)) {
    deps.stderr.write(
      `Model "${opts.defaultModel}" is not in provider "${providerId}". Run "cloudcode provider catalog list ${providerId}" to see available ids.\n`,
    );
    deps.exit(1);
  }
  ...
  // `core-impl.ts removeCloudCodeProvider`). Without this, re-importing an
...
function resolveDeps(overrides: Partial<ProviderDeps> = {}): ProviderDeps {
  ...
  return {
    getHarness:
      ...
      (() => {
        harness ??= createCloudCodeHarness({ identity });
        return harness;
      }),
...
function asManaged(config: CloudCodeConfig): ManagedKimiConfigShape {
  return config as unknown as ManagedKimiConfigShape;
}

function providerSourceLabel(provider: CloudCodeConfig['providers'][string]): string {
