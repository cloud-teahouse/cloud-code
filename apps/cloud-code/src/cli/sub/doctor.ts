...
export interface DoctorDeps {
  ...
  readonly configRpc?: CloudCodeConfigRpc;
...
export function registerDoctorCommand(parent: Command, deps?: Partial<DoctorDeps>): void {
  const doctor = parent
    .command('doctor')
    .description('Validate Cloud Code CLI configuration files.')
    .action(async () => {
...
function resolveDeps(deps: Partial<DoctorDeps> | DoctorDeps | undefined): ResolvedDoctorDeps {
  ...
  const getConfigRpc = (): CloudCodeConfigRpc => {
    configRpc ??= createCloudCodeConfigRpc();
    return configRpc;
  };
...
function formatFailure(results: readonly CheckResult[], issueCount: number): string {
  return [
    `Cloud Code CLI doctor found ${String(issueCount)} ${issueCount === 1 ? 'issue' : 'issues'}.`,
    '',
    ...formatResults(results),
    '',
  ].join('\n');
...
function findValidationIssues(error: unknown): readonly CloudCodeConfigValidationIssue[] | undefined {
...
function isValidationIssueArray(value: unknown): value is readonly CloudCodeConfigValidationIssue[] {
  return Array.isArray(value) && value.every(isValidationIssue);
}

function isValidationIssue(value: unknown): value is CloudCodeConfigValidationIssue {
