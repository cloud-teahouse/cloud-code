...
function currentTarget(): string {
  return CLOUD_CODE_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  const cacheDirEnv = optionalEnvValue(env, 'CLOUD_CODE_CACHE_DIR');
  if (cacheDirEnv !== null) return cacheDirEnv;

  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'cloud-code');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'cloud-code')
      : pathWin32.join(home, 'AppData', 'Local', 'cloud-code', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'cloud-code');
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? CLOUD_CODE_BUILD_INFO.version ?? 'dev');
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.cloud-code-native-entry.cjs');
  ensureFile(
    entryPath,
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = CLOUD_CODE_BUILD_INFO.version ?? 'dev';
