...
const mocks = vi.hoisted(() => ({
  ...
  resolveCloudCodeHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/cloud-code-export-home'),
}));
...
vi.mock('@cloud-code/sdk', async (importOriginal) => {
  ...
  return {
    ...
    createCloudCodeHarness: (...args: unknown[]) => {
      ...
      const homeDir = options?.homeDir ?? '/tmp/cloud-code-export-home';
      return {
        homeDir,
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'cloud-code-export-'));
});
...
afterEach(() => {
  ...
  mocks.resolveCloudCodeHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/cloud-code-export-home',
  );
});
