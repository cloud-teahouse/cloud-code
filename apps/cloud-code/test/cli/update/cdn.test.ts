...
import { CLOUD_CODE_CDN_LATEST_JSON_URL, CLOUD_CODE_CDN_LATEST_URL } from '#/constant/app';
...
describe('fetchLatestFromCdn', () => {
  it('parses latest.json and returns the manifest', async () => {
    const f = mockRoutedFetch({ [CLOUD_CODE_CDN_LATEST_JSON_URL]: { body: MANIFEST_BODY } });
    ...
    expect(f).toHaveBeenCalledWith(
      CLOUD_CODE_CDN_LATEST_JSON_URL,
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  ...
  it('ignores unknown manifest fields (lenient parsing)', async () => {
    ...
    const f = mockRoutedFetch({ [CLOUD_CODE_CDN_LATEST_JSON_URL]: { body } });
  ...
  for (const [name, route] of fallbackCases) {
    it(`falls back to plain /latest when ${name}`, async () => {
      const f = mockRoutedFetch({
        [CLOUD_CODE_CDN_LATEST_JSON_URL]: route,
        [CLOUD_CODE_CDN_LATEST_URL]: { body: '1.9.0\n' },
      });
      await expect(fetchLatestFromCdn(f)).resolves.toEqual({
        latest: '1.9.0',
        manifest: null,
  ...
  it('throws when both latest.json and plain /latest fail', async () => {
    const f = mockRoutedFetch({
      [CLOUD_CODE_CDN_LATEST_JSON_URL]: { status: 500 },
      [CLOUD_CODE_CDN_LATEST_URL]: { status: 500 },
    });
  ...
  it('propagates the plain /latest error when the fallback also breaks', async () => {
    const f = mockRoutedFetch({
      [CLOUD_CODE_CDN_LATEST_JSON_URL]: new Error('json down'),
      [CLOUD_CODE_CDN_LATEST_URL]: { body: 'not-a-version' },
    });
  ...
  it('falls back to plain /latest when latest.json hangs past the request timeout', async () => {
    ...
    try {
      const f = vi.fn(async (input: string | URL, init?: RequestInit) => {
        if (String(input) === CLOUD_CODE_CDN_LATEST_JSON_URL) {
        ...
        if (String(input) === CLOUD_CODE_CDN_LATEST_URL) {
