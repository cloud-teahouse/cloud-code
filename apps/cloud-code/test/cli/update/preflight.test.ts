...
vi.mock('../../../src/tui/config', () => ({
  ...
  TuiConfigParseError: class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfig) {
      super('Invalid client preferences in ~/.cloud-code/tui.toml; using defaults.');
...
describe('runUpdatePreflight', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('CLOUD_CODE_EXPERIMENTAL_FLAG', '');
  ...
  it('skips all update work when CLOUD_CODE_NO_AUTO_UPDATE is set', async () => {
    vi.stubEnv('CLOUD_CODE_NO_AUTO_UPDATE', '1');
  ...
  it('refreshes a stale cached target before showing the foreground install prompt', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        installCommand: 'npm install -g @cloud-teahouse/cloudcode-cli@0.7.0',
      }),
    );
    expect(mocks.spawn).toHaveBeenCalledWith(
      expect.stringMatching(/^npm(\.cmd)?$/),
      ['install', '-g', '@cloud-teahouse/cloudcode-cli@0.7.0'],
      { stdio: 'inherit' },
    );
    expect(stdout.join('')).toContain('Updated @cloud-teahouse/cloudcode-cli to 0.7.0');
  });
    ...
    try {
      ...
      expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          installCommand: 'npm install -g @cloud-teahouse/cloudcode-cli@0.6.0',
        }),
      );
    } finally {
      vi.useRealTimers();
  ...
  it('pnpm-global: spawns pnpm add -g', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      ['add', '-g', '@cloud-teahouse/cloudcode-cli@0.5.0'],
      { stdio: 'inherit' },
    );
  });
  ...
  it('yarn-global: spawns yarn global add', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      ['global', 'add', '@cloud-teahouse/cloudcode-cli@0.5.0'],
      { stdio: 'inherit' },
    );
  });
  ...
  it('homebrew: prints manual brew upgrade command, does not spawn', async () => {
    ...
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('brew upgrade cloud-code');
    expect(stdout.join('')).toContain('Third-party sources may lag behind the official release.');
    expect(stdout.join('')).toContain('https://github.com/cloud-teahouse/cloud-code');
    expect(promptForInstallChoice).not.toHaveBeenCalled();
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('native on darwin: spawns bash -c with pipefail-guarded curl|bash', async () => {
    ...
    try {
      ...
      expect(script).toContain(
        'curl -fsSL https://raw.githubusercontent.com/cloud-teahouse/cloud-code/main/scripts/install.sh',
      );
  ...
  it('native on win32: prints manual WSL command, does not spawn', async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.detectInstallSource.mockResolvedValue('native');
    const originalPlatform = process.platform;
    ...
    try {
      ...
      expect(stdout.join('')).toContain(
        'wsl bash -c "curl -fsSL https://raw.githubusercontent.com/cloud-teahouse/cloud-code/main/scripts/install.sh | bash"',
      );
      expect(promptForInstallChoice).not.toHaveBeenCalled();
      expect(mocks.spawn).not.toHaveBeenCalled();
    } finally {
  ...
  it('unsupported: prints fallback npm command', async () => {
    ...
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('npm install -g @cloud-teahouse/cloudcode-cli@0.5.0');
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('warns and continues when spawn exits non-zero, without claiming success', async () => {
    ...
    expect(stdout.join('')).not.toContain('Updated @cloud-teahouse/cloudcode-cli');
  });
  ...
  it('logs successful background update installs', async () => {
    ...
    await expect(runUpdatePreflight('0.4.0', { ...options, logger })).resolves.toBe('continue');
    await flushBackgroundInstall();
  ...
  it('shows a one-shot notice after a background update succeeds and the new version starts', async () => {
    ...
    await expect(runUpdatePreflight('0.5.0', { ...options, logger })).resolves.toBe('continue');

    const rendered = stdout.join('');
    expect(rendered).toContain('Cloud Code CLI updated to v0.5.0');
    expect(rendered).toContain(
      'https://github.com/cloud-teahouse/cloud-code/blob/dev/apps/cloud-code/CHANGELOG.md',
    );
  ...
  it('infers a background update success notice when the active install version is now running', async () => {
    ...
    expect(stdout.join('')).toContain('Cloud Code CLI updated to v0.5.0');
    expect(writeUpdateInstallState).toHaveBeenCalledWith(expect.objectContaining({
      active: null,
      lastFailure: null,
      lastSuccess: expect.objectContaining({
  ...
  describe('rollout gating', () => {
    ...
    it('prompts when eligible and auto-install is disabled', async () => {
      disableAutoInstall();
      const released = cacheWithManifest(releasedForEveryone('0.5.0'));
      mocks.readUpdateCache.mockResolvedValue(released);
      mocks.refreshUpdateCache.mockResolvedValue(released);
    ...
    it('uses the refreshed manifest when the prompt target changes', async () => {
      disableAutoInstall();
    ...
    it('CLOUD_CODE_EXPERIMENTAL_FLAG bypasses the rollout: held devices still update', async () => {
    ...
    it('CLOUD_CODE_NO_AUTO_UPDATE still wins over the experimental flag', async () => {
      vi.stubEnv('CLOUD_CODE_EXPERIMENTAL_FLAG', '1');
      vi.stubEnv('CLOUD_CODE_NO_AUTO_UPDATE', '1');
