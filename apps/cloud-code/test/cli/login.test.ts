...
describe('kimi login', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(createCloudCodeHarness).mockClear();
  ...
  it('registers a `login` subcommand on the program', () => {
    const program = new Command('cloud-code');
  ...
  it('invokes harness.auth.login and exits 0 on success', async () => {
    ...
    const program = new Command('cloud-code').exitOverride();
    registerLoginCommand(program);

    await expect(program.parseAsync(['node', 'cloud-code', 'login'])).rejects.toThrow(ExitCalled);
