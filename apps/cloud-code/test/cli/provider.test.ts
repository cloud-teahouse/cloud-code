...
interface FakeHarness {
  ...
  setConfig: (patch: Partial<CloudCodeConfig>) => Promise<CloudCodeConfig>;
  removeProvider: (providerId: string) => Promise<CloudCodeConfig>;
}

function makeHarness(initial: CloudCodeConfig): {
  harness: FakeHarness;
  current: () => CloudCodeConfig;
  setConfigCalls: Array<Partial<CloudCodeConfig>>;
  removeCalls: string[];
} {
  // `persisted` simulates the on-disk config; the real RPC's `removeProvider`
  // reads from / writes to disk on every call (see
  // `packages/agent-core/src/rpc/core-impl.ts removeCloudCodeProvider`). Tests must
  ...
  // `removeProvider`.
  let persisted: CloudCodeConfig = structuredClone(initial);
  const setConfigCalls: Array<Partial<CloudCodeConfig>> = [];
  ...
  const harness: FakeHarness = {
    ...
    setConfig: async (patch) => {
      ...
      // Mirror the real `setCloudCodeConfig`: deep-merge with undefined keys
      ...
      persisted = next as CloudCodeConfig;
...
describe('kimi provider add', () => {
  it('imports providers and models from a custom registry, persisting source on each provider', async () => {
    ...
    const { harness, current, setConfigCalls } = makeHarness({ providers: {} } as CloudCodeConfig);
  ...
  it('preserves newly-imported providers when a later registry entry replaces an existing id', async () => {
    // Regression test: `harness.removeProvider` re-reads
  ...
  it('reads the api key from KIMI_REGISTRY_API_KEY when --api-key is omitted', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness } = makeHarness({ providers: {} } as CloudCodeConfig);
    const { deps, exitCodes } = makeDeps(harness, {
...
describe('registerProviderCommand', () => {
  it('describes the user-facing subcommand and routes flags through commander', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness, current } = makeHarness({ providers: {} } as CloudCodeConfig);
  ...
  it('reports write failures on stderr and exits 1 instead of crashing', async () => {
    ...
    harness.removeProvider = async () => {
      throw new Error(
        'Cannot change settings while config.toml is invalid — fix it first (run `cloudcode doctor` for details).',
      );
    };
    const { deps, stderr, exitCodes } = makeDeps(harness);
...
describe('cloud-code provider catalog list', () => {
...
describe('cloud-code provider catalog add', () => {
  ...
  it('preserves thinking.enabled when --default-model is supplied to a thinking-capable model', async () => {
    // Regression test: `applyCatalogProvider` always
  ...
  it('drops a stale default_model when the catalog refresh no longer contains it', async () => {
    // Regression test: when the user previously chose
