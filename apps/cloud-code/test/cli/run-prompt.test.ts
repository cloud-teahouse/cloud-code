 ...
 * Run: pnpm -C apps/cloud-code exec vitest run test/cli/run-prompt.test.ts
 */
...
import type { createKimiDeviceId as createKimiDeviceIdFn } from '@cloud-code/oauth';
...
describe('runPrompt', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('CLOUD_CODE_MODEL_OUTPUT_FORMAT', '');
  });
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: cloudcode -r ses_prompt\n');
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: cloudcode -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"cloudcode -r ses_prompt","content":"To resume this session: cloudcode -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
