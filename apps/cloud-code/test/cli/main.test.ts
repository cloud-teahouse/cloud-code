...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    CloudCodeHarness: vi.fn(),
    createCloudCodeHarness: vi.fn(),
  };
});

vi.mock('@cloud-code/sdk', async () => {
  const actual = await vi.importActual<typeof import('@cloud-code/sdk')>(
    '@cloud-code/sdk',
  );
  class MockCloudCodeHarness {
...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      ...
      expect(process.title).toBe('cloud-code');
    } finally {
  ...
  it('runs the upgrade command and closes the harness', async () => {
    const exitCode = await runHandleUpgradeCommand();

    expect(exitCode).toBe(0);
    expect(mocks.createCloudCodeHarness).toHaveBeenCalledTimes(1);
  ...
  it('formats Kimi startup errors with structured fields', () => {
    const error = new CloudCodeError(
