...
const executableName = process.platform === 'win32' ? 'cloud-code.exe' : 'cloud-code';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `cloud-code-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `cloud-code-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, CLOUD_CODE_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `cloud-code-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  cloud-code-${target}.zip\n`,
    );
  });
  ...
  it('produces a manifest from zip archive checksums', async () => {
    ...
    await writeFile(join(releaseDir, 'cloud-code-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'cloud-code-darwin-arm64.zip.sha256'),
      `${checksum}  cloud-code-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, 'cloudcode-cli@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: 'cloudcode-cli@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'cloud-code-darwin-arm64.zip',
          checksum,
        },
      },
    });
