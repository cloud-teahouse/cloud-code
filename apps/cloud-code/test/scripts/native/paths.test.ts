...
describe('targetTriple', () => {
  ...
  it('honors CLOUD_CODE_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { CLOUD_CODE_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns cloud-code.exe on win32', () => {
    expect(executableName('win32')).toBe('cloud-code.exe');
  });

  it('returns cloud-code on other platforms', () => {
    expect(executableName('darwin')).toBe('cloud-code');
    expect(executableName('linux')).toBe('cloud-code');
  });
});
...
describe('path helpers', () => {
  ...
  it('returns absolute bin path with executable name', () => {
    expect(nativeBinPath('darwin-arm64', 'darwin')).toBe(
      p('dist-native/bin/darwin-arm64/cloud-code'),
    );
    expect(nativeBinPath('win32-x64', 'win32')).toBe(
      p('dist-native/bin/win32-x64/cloud-code.exe'),
    );
  });

  it('returns intermediate artifact paths', () => {
    expect(nativeJsBundlePath()).toBe(p('dist-native/intermediates/main.cjs'));
    expect(nativeBlobPath()).toBe(p('dist-native/intermediates/cloud-code.blob'));
