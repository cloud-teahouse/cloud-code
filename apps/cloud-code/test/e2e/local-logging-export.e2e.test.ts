...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    ...
    try {
      ...
      await runCloudCodeExport([session.id, '-o', defaultZip]);
      ...
      await runCloudCodeExport([session.id, '-o', noGlobalZip, '--no-include-global-log']);
...
async function runCloudCodeExport(args: string[]): Promise<void> {
