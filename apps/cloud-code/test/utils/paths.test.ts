...
describe('getDataDir', () => {
  it('returns ~/.cloud-code when CLOUD_CODE_HOME is not set', () => {
    expect(getDataDir()).toBe(join(homedir(), '.cloud-code'));
  });

  it('returns CLOUD_CODE_HOME when set', () => {
    process.env['CLOUD_CODE_HOME'] = '/tmp/cloud-code-test-data';
    expect(getDataDir()).toBe('/tmp/cloud-code-test-data');
  });

  it('returns CLOUD_CODE_HOME even if it is a relative path', () => {
    process.env['CLOUD_CODE_HOME'] = 'relative/path';
...
describe('getLogDir', () => {
  it('returns <dataDir>/logs', () => {
    expect(getLogDir()).toBe(join(homedir(), '.cloud-code', 'logs'));
  });

  it('respects CLOUD_CODE_HOME', () => {
    process.env['CLOUD_CODE_HOME'] = '/z';
...
describe('getBinDir', () => {
  it('returns <dataDir>/bin', () => {
    expect(getBinDir()).toBe(join(homedir(), '.cloud-code', 'bin'));
  });

  it('respects CLOUD_CODE_HOME', () => {
    process.env['CLOUD_CODE_HOME'] = '/custom-bin-home';
...
describe('getUpdateStateFile', () => {
  it('returns <dataDir>/updates/latest.json', () => {
    expect(getUpdateStateFile()).toBe(join(homedir(), '.cloud-code', 'updates', 'latest.json'));
  });

  it('respects CLOUD_CODE_HOME', () => {
    process.env['CLOUD_CODE_HOME'] = '/updates-home';
...
describe('getUpdateInstallStateFile', () => {
  it('returns <dataDir>/updates/install.json', () => {
    expect(getUpdateInstallStateFile()).toBe(
      join(homedir(), '.cloud-code', 'updates', 'install.json'),
    );
  });
...
describe('getInputHistoryFile', () => {
  it('returns <dataDir>/user-history/<md5(workDir)>.jsonl', () => {
    ...
    expect(getInputHistoryFile(workDir)).toBe(
      join(homedir(), '.cloud-code', 'user-history', `${hash}.jsonl`),
    );
  });

  it('respects CLOUD_CODE_HOME', () => {
    process.env['CLOUD_CODE_HOME'] = '/custom/data';
