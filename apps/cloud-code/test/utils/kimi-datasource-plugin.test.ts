...
import { resolveKimiCodeOAuthKey } from '@cloud-code/oauth';
...
describe('kimi-datasource MCP server', () => {
  ...
  it('uses env-scoped credentials and derives the datasource URL from CLOUD_CODE_BASE_URL', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'kimi-datasource-plugin-'));
    const kimiHome = join(tempDir, 'kimi-home');
    const requests: unknown[] = [];
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          CLOUD_CODE_HOME: kimiHome,
          CLOUD_CODE_BASE_URL: baseUrl,
          CLOUD_CODE_OAUTH_HOST: oauthHost,
  ...
  it('returns the complete data-source routing contract when tools are listed', async () => {
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        cwd: REPO_ROOT,
        env: { ...process.env, CLOUD_CODE_HOME: kimiHome },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
...
// truth in @cloud-code/oauth. The credential file name is the OAuth
