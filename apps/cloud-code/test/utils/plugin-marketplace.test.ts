...
describe('loadPluginMarketplace', () => {
  ...
  it('loads the default CDN marketplace with injectable fetch', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(CLOUD_CODE_PLUGIN_MARKETPLACE_URL);
  ...
  it('falls back to the source checkout marketplace when the default CDN cannot be fetched', async () => {
    const previous = process.env[CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV];
    delete process.env[CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV];
    const fetchImpl = vi.fn(async () => {
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env[CLOUD_CODE_PLUGIN_MARKETPLACE_URL_ENV] = previous;
      }
    }
  });
