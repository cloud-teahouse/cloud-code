...
describe('getNativeCacheBase precedence', () => {
  ...
  it('uses CLOUD_CODE_CACHE_DIR when set (highest precedence)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { CLOUD_CODE_CACHE_DIR: '/custom/cache' },
    });
    expect(got).toBe('/custom/cache');
  });

  it('ignores CLOUD_CODE_HOME (no longer affects native cache)', () => {
    const got = getNativeCacheBase({
      ...baseOptions,
      env: { CLOUD_CODE_HOME: '/legacy' },
      platform: 'darwin',
    });
    expect(got).toBe('/home/u/Library/Caches/cloud-code');
  });
  ...
  it('uses XDG_CACHE_HOME on Linux when set', () => {
    ...
    expect(got).toBe('/xdg/cloud-code');
  });
  ...
  it('uses LOCALAPPDATA on Windows when set', () => {
    ...
    expect(got).toBe('C:\\Users\\u\\AppData\\Local\\cloud-code');
  });
});
