 ...
 * Run: pnpm -C apps/cloud-code exec vitest run test/tui/commands/secondary-model.test.ts
 */
import type { ModelAlias, ThinkingEffort } from '@cloud-code/sdk';
...
describe('handleSecondaryModelCommand', () => {
  ...
  it('warns with the env-overridden effective binding instead of the picked model', async () => {
    // CLOUD_CODE_SECONDARY_MODEL / CLOUD_CODE_SECONDARY_EFFORT win over the
    // persisted recipe: the reloaded config carries the overlaid values, and
    // the status message must name them rather than echo the pick.
    const { host } = makeHost({
    ...
    expect(message).toContain('CLOUD_CODE_SECONDARY_MODEL=cheap');
    expect(message).toContain('CLOUD_CODE_SECONDARY_EFFORT=low');
