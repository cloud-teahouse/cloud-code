...
describe('ApprovalPreviewViewer', () => {
  ...
  it('pans with the mouse wheel', () => {
    ...
    const before = viewer.render(100).join('\n');
    expect(before).toContain('row-1 ');
    expect(before).not.toContain('row-26');

    viewer.handleMouse(wheel(65));
    viewer.handleMouse(wheel(65));
    const scrolled = viewer.render(100).join('\n');
    expect(scrolled).toContain('row-26');
    expect(scrolled).not.toContain('row-1 ');

    for (let i = 0; i < 100; i++) viewer.handleMouse(wheel(65));
    expect(viewer.render(100).join('\n')).toContain('row-200');

    for (let i = 0; i < 100; i++) viewer.handleMouse(wheel(64));
    expect(viewer.render(100).join('\n')).toContain('row-1');
  });
});
