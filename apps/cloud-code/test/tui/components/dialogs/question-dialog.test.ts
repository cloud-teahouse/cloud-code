...
describe('QuestionDialogComponent', () => {
  ...
  it('escape while editing an Other answer exits the input first, keeping the draft', () => {
    const pending = makePending([
      {
        question: 'Pick toppings?',
        multi_select: true,
    ...
    dialog.handleInput('3'); // jump to the synthetic Other option → edit mode
    dialog.handleInput('x');
    dialog.handleInput(ESC); // first Esc: leaves the input only
    expect(collected).toEqual([]);
    const out = strip(dialog.render(80).join('\n'));
    expect(out).toContain('Other: x');

    dialog.handleInput(ESC); // second Esc: cancels the dialog
    expect(collected).toEqual([[]]);
  });

  it('shift+tab moves to the previous tab', () => {
    const pending = makePending([
      {
        question: 'Q1?',
        header: 'First',
...
describe('QuestionDialogComponent mouse support', () => {
  const press = (dialog: QuestionDialogComponent, row: number, col = 1, button = 0): void => {
    dialog.handleMouse({ type: 'press', button, col, row, slotRelative: false });
  };
  const motion = (dialog: QuestionDialogComponent, row: number, col = 1): void | boolean =>
    dialog.handleMouse({ type: 'motion', button: 3, col, row, slotRelative: false });
  ...
  function colOf(dialog: QuestionDialogComponent, marker: string): number {
    const lines = dialog.render(80).map(strip);
  ...
  it('activates a single-select option on click and auto-advances', () => {
    const pending = makePending([
    ...
    press(dialog, rowOf(dialog, '[2] B1'));
    expect(collected).toEqual([]);
    // Single-select answered Q1 and advanced to Q2.
    expect(strip(dialog.render(80).join('\n'))).toContain('? Q2?');

    press(dialog, rowOf(dialog, '[1] A2'));
    expect(collected).toEqual([]);
    // All questions answered → advanced to the submit/review tab.
    expect(strip(dialog.render(80).join('\n'))).toContain('Review');
  });

  it('toggles a multi-select option on click without advancing', () => {
    const pending = makePending([
      { question: 'Q1?', multi_select: true, options: [{ label: 'A1' }, { label: 'B1' }] },
    ]);
    const { dialog } = makeDialog(pending);

    press(dialog, rowOf(dialog, 'A1'));
    let out = strip(dialog.render(80).join('\n'));
    expect(out).toContain('[✓] A1');
    expect(out).toContain('? Q1?'); // still on the same tab
    press(dialog, rowOf(dialog, 'A1'));
    out = strip(dialog.render(80).join('\n'));
    expect(out).toContain('[ ] A1');
  });
  ...
  it('switches tabs when a tab cell is clicked', () => {
    ...
    press(dialog, 3, colOf(dialog, 'Q2'));
    expect(strip(dialog.render(80).join('\n'))).toContain('? Q2?');

    // Clicking the Submit tab opens the review.
    press(dialog, 3, colOf(dialog, 'Submit'));
    expect(strip(dialog.render(80).join('\n'))).toContain('Review');

    // And back to Q1.
    press(dialog, 3, colOf(dialog, 'Q1'));
    expect(strip(dialog.render(80).join('\n'))).toContain('? Q1?');
  });

  it('highlights a submit action on click and executes it on re-click', () => {
    const pending = makePending([
      { question: 'Q1?', multi_select: false, options: [{ label: 'A1' }] },
    ]);
    const { dialog, collected } = makeDialog(pending);
    press(dialog, rowOf(dialog, '[1] A1')); // answer → submit tab

    const cancelRow = rowOf(dialog, '[2]');
    press(dialog, cancelRow); // highlights Cancel
    expect(collected).toEqual([]);
    press(dialog, cancelRow); // re-click executes: empty answers
    expect(collected).toEqual([[]]);
  });

  it('underlines the hovered option/tab and clears on leave', () => {
    const pending = makePending([
      { question: 'Q1?', multi_select: false, options: [{ label: 'A1' }, { label: 'B1' }] },
    ]);
    const { dialog } = makeDialog(pending);
    ...
    const optionRow = rowOf(dialog, '[2] B1');
    expect(motion(dialog, optionRow)).not.toBe(false);
    expect(dialog.render(80).join('\n')).toContain('[4m');
    expect(motion(dialog, optionRow)).toBe(false); // unchanged → frame skipped

    // Hovering a tab underlines the tab cell instead.
    expect(motion(dialog, 3, colOf(dialog, 'Submit'))).not.toBe(false);
    expect(dialog.render(80).join('\n')).toContain('[4m');

    // Leaving the interactive rows clears the underline completely.
    motion(dialog, 0);
    expect(dialog.render(80).join('\n')).toBe(baseline);
  });
});
