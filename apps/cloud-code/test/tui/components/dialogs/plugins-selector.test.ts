...
function makePanel(opts: {
  ...
  return { panel, onSelect, onCancel, onRequestMarketplace };
}
...
describe('plugins selector dialogs', () => {
  ...
  it('routes D / M / R / Enter to remove / mcp / reload / details on the Installed tab (transition aliases)', () => {
    const { panel, onSelect } = makePanel({ installed: [superpowers] });
    panel.handleInput('d');
    panel.handleInput('m');
    panel.handleInput('r');
  ...
  it('routes the canonical Alt+D / Alt+M / Alt+R / Alt+I to the Installed-tab row actions', () => {
    const esc = String.fromCodePoint(27);
    const { panel, onSelect } = makePanel({ installed: [superpowers] });
    panel.handleInput(`${esc}[100;3u`); // alt+d
    panel.handleInput(`${esc}[109;3u`); // alt+m
    panel.handleInput(`${esc}[114;3u`); // alt+r
    panel.handleInput(`${esc}[105;3u`); // alt+i
    expect(onSelect).toHaveBeenCalledWith({ kind: 'remove', id: 'superpowers' });
    expect(onSelect).toHaveBeenCalledWith({ kind: 'mcp', id: 'superpowers' });
    expect(onSelect).toHaveBeenCalledWith({ kind: 'reload' });
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'superpowers' });
  ...
  it('fires the Alt row actions even while the search box is focused', () => {
    const esc = String.fromCodePoint(27);
    const { panel, onSelect } = makePanel({ installed: [superpowers] });
    panel.handleInput('/'); // focus the search box
    panel.handleInput(`${esc}[100;3u`); // alt+d — not a query character
    expect(onSelect).toHaveBeenCalledWith({ kind: 'remove', id: 'superpowers' });
    // The query stays empty: the Alt key never landed in the search box.
    expect(strip(renderRaw(panel))).toContain('1 installed');
  });

  it('PgUp/PgDn/Home/End jump the highlight on the Installed tab', () => {
    const esc = String.fromCodePoint(27);
    ...
    panel.handleInput(`${esc}[F`); // End → last row
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'plugin-9' });

    panel.handleInput(`${esc}[H`); // Home → first row
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'plugin-0' });

    panel.handleInput(`${esc}[6~`); // PgDn → one page down
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'plugin-8' });

    panel.handleInput(`${esc}[5~`); // PgUp → back to the top
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenLastCalledWith({ kind: 'details', id: 'plugin-0' });
  });
...
describe('plugins panel search', () => {
  ...
  const webtools = { ...superpowers, id: 'web-tools', displayName: 'Web Tools' };
  const esc = String.fromCodePoint(27);

  it('renders the search box on the list tabs but not on Custom', () => {
    const { panel } = makePanel({ installed: [superpowers] });
    expect(strip(renderRaw(panel))).toContain('Search…');
    for (let i = 0; i < 3; i++) panel.handleInput('\t'); // installed → … → custom
    expect(strip(renderRaw(panel))).not.toContain('Search…');
  });

  it('filters the Installed tab via / and typed characters; Enter activates the match', () => {
    const { panel, onSelect } = makePanel({ installed: [superpowers, datasource, webtools] });
    panel.handleInput('/');
    panel.handleInput('d');
    panel.handleInput('a');
    const out = strip(renderRaw(panel));
    ...
    // The count line still describes the full installed set, not the matches.
    expect(out).toContain('3 installed');

    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'kimi-datasource' });
  });

  it('shows a no-matches notice when the filter empties the Installed tab', () => {
    const { panel, onSelect } = makePanel({ installed: [superpowers] });
    panel.handleInput('/');
    panel.handleInput('z');
    panel.handleInput('z');
    expect(strip(renderRaw(panel))).toContain('No matches');
    // Enter is inert with no rows.
    panel.handleInput('\r');
    expect(onSelect).not.toHaveBeenCalled();
  });

  it('focuses the search box with ↑ from the first row', () => {
    const { panel } = makePanel({ installed: [superpowers, datasource] });
    panel.handleInput(`${esc}[A`); // ↑ on the first row moves focus into the box
    expect(strip(renderRaw(panel))).toContain('Esc back to list');
  });

  it('layers Esc on a searchable tab: clear query → unfocus the box → close', () => {
    const { panel, onCancel } = makePanel({ installed: [superpowers, datasource] });
    panel.handleInput('/');
    panel.handleInput('d');
    expect(strip(renderRaw(panel))).toContain('Esc back to list');

    panel.handleInput(esc); // clears the query; the box stays focused
    let out = strip(renderRaw(panel));
    expect(out).toContain('Superpowers'); // the full list is back
    expect(out).toContain('Esc back to list');
    expect(onCancel).not.toHaveBeenCalled();

    panel.handleInput(esc); // unfocuses the box
    out = strip(renderRaw(panel));
    expect(out).toContain('/ ↑ search');
    expect(onCancel).not.toHaveBeenCalled();

    panel.handleInput(esc); // closes the panel
    expect(onCancel).toHaveBeenCalledTimes(1);
  });

  it('trims the query with Backspace while the box is focused', () => {
    const { panel } = makePanel({ installed: [superpowers, datasource] });
    panel.handleInput('/');
    panel.handleInput('d');
    panel.handleInput('a');
    expect(strip(renderRaw(panel))).not.toContain('? Superpowers');
    panel.handleInput('\u007F'); // Backspace trims the 'a'
    panel.handleInput('\u007F'); // …and the 'd' — the full list returns
    expect(strip(renderRaw(panel))).toContain('? Superpowers');
  });

  it('treats Space and action letters as query text while the search box is focused', () => {
    const { panel, onSelect } = makePanel({ installed: [superpowers, datasource] });
    panel.handleInput('/');
    panel.handleInput('d'); // would remove the row when the box is unfocused
    panel.handleInput(' '); // would toggle the row when the box is unfocused
    panel.handleInput('x');
    expect(onSelect).not.toHaveBeenCalled();
    // The query holds "d x" — a no-match state, not a removed/toggled row.
    expect(strip(renderRaw(panel))).toContain('No matches');

    panel.handleInput(esc); // clear the query
    panel.handleInput(esc); // unfocus the box
    panel.handleInput(' '); // the toggle fires again with the list focused
    expect(onSelect).toHaveBeenCalledWith({ kind: 'toggle', id: 'superpowers', enabled: false });
  });

  it('keeps each tab’s query across tab switches', () => {
    ...
    panel.handleInput('d');
    panel.handleInput('a');
    // The Official query filters catalog rows and the pinned Web Bridge row.
    let out = strip(renderRaw(panel));
    expect(out).toContain('Kimi Datasource');
    expect(out).not.toContain('Web Bridge');

    // Third-party has its own, still empty query: the full list shows.
    panel.handleInput('\t');
    out = strip(renderRaw(panel));
    expect(out).toContain('Superpowers  install');
    panel.handleInput('/');
    panel.handleInput('z');
    panel.handleInput('z');
    expect(strip(renderRaw(panel))).toContain('No matches');

    // Back on Official the "da" query (and its match) is still there.
    panel.handleInput(`${esc}[Z`); // Shift+Tab
    out = strip(renderRaw(panel));
    expect(out).toContain('Kimi Datasource');
    expect(out).not.toContain('Web Bridge');

    // Enter installs the highlighted match.
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
      entry: expect.objectContaining({ id: 'kimi-datasource' }),
  ...
  it('keeps each tab’s cursor across tab switches', () => {
    const { panel } = makePanel({ installed: [superpowers, datasource] });
    panel.handleInput(`${esc}[B`); // ↓ → Kimi Datasource
    expect(strip(renderRaw(panel))).toContain('? Kimi Datasource');
    panel.handleInput('\t'); // → Official
    panel.handleInput(`${esc}[Z`); // Shift+Tab → back to Installed
    expect(strip(renderRaw(panel))).toContain('? Kimi Datasource');
  });

  it('keeps the typed query when the marketplace catalog lands', () => {
    const { panel } = makePanel({ installed: [], initialTab: 'third-party' });
    // The catalog is still loading; the query is typed first.
    panel.handleInput('/');
    panel.handleInput('s');
    panel.handleInput('u');
    panel.setMarketplace(marketplaceEntries, '/tmp/marketplace.json');
  ...
  it('shows a no-matches notice when the filter empties a marketplace tab', () => {
    const { panel, onSelect } = makePanel({ installed: [], initialTab: 'third-party' });
    panel.setMarketplace(marketplaceEntries, '/tmp/marketplace.json');
    panel.handleInput('/');
    panel.handleInput('z');
    panel.handleInput('z');
    ...
    // The footer counts follow the matches.
    expect(out).toContain('0 installed · 0 available');
    panel.handleInput('\r');
    expect(onSelect).not.toHaveBeenCalled();
  });

  it('keeps the Custom tab search-free: / types into the URL input', () => {
    const { panel } = makePanel({ initialTab: 'custom' });
    expect(strip(renderRaw(panel))).not.toContain('Search…');
    for (const ch of '/abc') panel.handleInput(ch);
    const out = strip(renderRaw(panel));
    expect(out).not.toContain('Search…');
    expect(out).toContain('/abc');
  });

  it('cancels flat with Esc on the Custom tab (a text input)', () => {
    const { panel, onCancel } = makePanel({ initialTab: 'custom' });
