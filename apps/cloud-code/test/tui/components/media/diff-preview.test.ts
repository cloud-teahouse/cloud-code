...
describe('renderDiffLinesClustered', () => {
  ...
  it('inflects the truncation footer by hidden count and localizes it in zh-CN', () => {
    // Eight changed lines, all but one hidden → singular.
    const one = stripAnsi(
      renderDiffLines('A\nB\nC\nD', 'W\nX\nY\nZ', 'f.ts', false, 1, 1, 7).join('\n'),
    );
    expect(one).toContain('1 more change hidden (ctrl+o to expand)');
    // Two or more hidden → plural.
    const many = stripAnsi(
      renderDiffLines('A\nB\nC\nD', 'W\nX\nY\nZ', 'f.ts', false, 1, 1, 1).join('\n'),
    );
    expect(many).toContain('7 more changes hidden (ctrl+o to expand)');

    setLocalePreference('zh-CN');
    const zh = stripAnsi(
      renderDiffLines('A\nB\nC\nD', 'W\nX\nY\nZ', 'f.ts', false, 1, 1, 1).join('\n'),
    );
    expect(zh).toContain('还有 7 处修改未显示（ctrl+o 展开）');
    expect(zh).not.toContain('more change');
    setLocalePreference('en');
  });

  it('renders the elision separator in zh-CN', () => {
    setLocalePreference('zh-CN');
    const oldLines: string[] = [];
    for (let i = 1; i <= 30; i++) oldLines.push(`L${String(i)}`);
    const newLines = oldLines.slice();
