...
import {
  ...
  agentSwarmResultIsUserCancelled,
...
describe('AgentSwarmProgressComponent', () => {
  ...
  it('shimmers the status label only while the swarm is working', () => {
    ...
    try {
      ...
      const workingLine = component
        .render(100)
        .find((line) => strip(line).includes('Working...'));
      expect(workingLine).toBeDefined();
      expect(rgbRuns(workingLine!)).toBeGreaterThan(5);

      component.markCompleted('agent-1');
      component.markCompleted('agent-2');

      const completedLine = component
        .render(100)
        .find((line) => strip(line).includes('Completed.'));
      expect(completedLine).toBeDefined();
      // Terminal: a static status color for the label, no per-character wave.
      expect(rgbRuns(completedLine!)).toBeLessThan(rgbRuns(workingLine!));
    } finally {
      chalk.level = previousLevel;
    }
  });
  ...
  it('applies per-member outcomes from the structured envelope', () => {
    const component = createComponent();
    ...
    const applied = component.applyResult(
      [
        ...
        '<subagent agent_id="agent-1" item="src/a.ts" outcome="completed">' +
          'Reviewed src/a.ts.</subagent>',
        '<subagent agent_id="agent-2" item="src/b.ts" outcome="failed">' +
      ...
      ].join('\n'),
      {
        ...
        aborted: 0,
        members: [
          { outcome: 'completed', agentId: 'agent-1', item: 'src/a.ts' },
          { outcome: 'failed', agentId: 'agent-2', item: 'src/b.ts' },
        ],
      },
    );
  ...
  it('reads swarm summary counts from the structured envelope', () => {
    const summary = agentSwarmResultSummary({
      tool_call_id: 'call_swarm',
      output: 'unparseable output',
      structured: { completed: 2, failed: 1, aborted: 1, members: [] },
    });

    expect(summary).toEqual({ completed: 2, failed: 1, aborted: 1, parsed: false });
  });

  it('falls back to the XML output for the summary when no envelope exists', () => {
    const summary = agentSwarmResultSummary({
      tool_call_id: 'call_swarm',
      output: [
        '<agent_swarm_result>',
        '<summary>completed: 1</summary>',
        '<subagent agent_id="agent-1" outcome="completed">done</subagent>',
        '</agent_swarm_result>',
      ].join('\n'),
    });

    expect(summary).toEqual({ completed: 1, failed: 0, aborted: 0, parsed: true });
  });

  it('detects user cancellation from the structured error kind', () => {
    expect(
      agentSwarmResultIsUserCancelled({
        tool_call_id: 'call_swarm',
        output: 'provider exploded',
        is_error: true,
        structured: { errorKind: 'user_cancelled' },
      }),
    ).toBe(true);
    expect(
      agentSwarmResultIsUserCancelled({
        tool_call_id: 'call_swarm',
        output: 'Aborted by the user',
        is_error: true,
        structured: { errorKind: 'error' },
      }),
    ).toBe(false);
    // Legacy fallback: no envelope, English text match.
    expect(
      agentSwarmResultIsUserCancelled({
        tool_call_id: 'call_swarm',
        output: 'Aborted by the user',
