...
describe('AgentGroupComponent', () => {
  ...
  it('shimmers the group header while any member runs and freezes once all finish', () => {
    ...
    try {
      ...
      const header = (): string => group.render(120)[1] ?? '';
      ...
      expect(header()).not.toBe(waved);

      // All members terminal: the header freezes into the finished form.
      a.onSubagentCompleted({ resultSummary: 'done' });
      b.onSubagentCompleted({ resultSummary: 'done' });
      const finished = header();
      expect(strip(finished)).toContain('2 agents finished');
      vi.advanceTimersByTime(2000);
      expect(header()).toBe(finished);
