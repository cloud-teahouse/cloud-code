...
describe('FooterComponent goal badge shimmer', () => {
  const goalBase = {
    goalId: 'g1',
  ...
  function badgeOf(line: string): string {
    const start = line.indexOf('[goal ');
  ...
  it('shimmers the badge label while the goal is active, not while paused', () => {
    const previousLevel = chalk.level;
    chalk.level = 3;
    try {
      const active = new FooterComponent({
        ...baseState,
        goal: { ...goalBase, status: 'active' } as AppState['goal'],
      });
      const activeBadge = badgeOf(active.render(120)[0]!);
      // The per-character wave styles each label character individually.
      expect(rgbRuns(activeBadge)).toBeGreaterThan(5);

      const paused = new FooterComponent({
        ...baseState,
        goal: { ...goalBase, status: 'paused' } as AppState['goal'],
      });
      const pausedBadge = badgeOf(paused.render(120)[0]!);
      // Paused is a static state: brackets + status dot + one muted label run.
      expect(rgbRuns(pausedBadge)).toBeLessThan(rgbRuns(activeBadge));
      expect(plain(activeBadge)).toBe(plain(pausedBadge).replace('paused', 'active'));
    } finally {
