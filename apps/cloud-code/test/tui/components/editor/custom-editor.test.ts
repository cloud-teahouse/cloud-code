...
describe('CustomEditor paste marker expansion', () => {
  ...
  it('does not leak an unhandled rejection when the image paste handler rejects', async () => {
    const editor = makeEditor();
...
describe('CustomEditor collapse re-render on autocomplete close', () => {
  function makeEditorWithRenderSpy(contentLines: number): {
    editor: CustomEditor;
    requestRender: ReturnType<typeof vi.fn>;
    requestCollapseRender: ReturnType<typeof vi.fn>;
  } {
    const requestRender = vi.fn();
    const requestCollapseRender = vi.fn();
    const tui = {
      requestRender,
      requestCollapseRender,
      terminal: { rows: 40, cols: 120 },
      render: vi.fn(() => Array.from({ length: contentLines }, () => '')),
    } as unknown as TUI;
    return { editor: new CustomEditor(tui), requestRender, requestCollapseRender };
  }
  ...
  it('forces a full re-render on the render frame after Escape closes the menu (content overflows)', async () => {
    vi.stubEnv('TMUX', '');
    const { editor, requestCollapseRender } = makeEditorWithRenderSpy(50);
    editor.setAutocompleteProvider(providerReturning([{ value: 'help', label: 'help' }]));
  ...
  it('keeps differential rendering when the content fits on one screen', async () => {
    vi.stubEnv('TMUX', '');
    const { editor, requestCollapseRender } = makeEditorWithRenderSpy(10);
    editor.setAutocompleteProvider(providerReturning([{ value: 'help', label: 'help' }]));
    ...
    renderFrame(editor);
    await flushAutocomplete();
    expect(requestCollapseRender).not.toHaveBeenCalled();
  });

  it('forces a full re-render when the content exactly fills one screen', async () => {
    vi.stubEnv('TMUX', '');
    const { editor, requestCollapseRender } = makeEditorWithRenderSpy(40);
    editor.setAutocompleteProvider(providerReturning([{ value: 'help', label: 'help' }]));
...
describe('CustomEditor prompt symbol', () => {
  // oxlint-disable-next-line no-control-regex -- ESC (\u001B) is required to match ANSI SGR escape sequences
  const stripAnsi = (s: string): string => s.replaceAll(/\u001B\[[0-9;]*m/g, '');

  it('renders ❯ as the prompt symbol in prompt mode', () => {
    const editor = makeEditor();
    for (const char of 'hi') {
      editor.handleInput(char);
    }

    const contentLine = stripAnsi(editor.render(90)[1] ?? '');
    expect(contentLine).toContain('❯ hi');
    expect(contentLine).not.toContain('> hi');
  });

  it('keeps ! as the prompt symbol in bash mode', () => {
    const editor = makeEditor();
    editor.inputMode = 'bash';
    for (const char of 'ls') {
      editor.handleInput(char);
    ...
    const contentLine = stripAnsi(editor.render(90)[1] ?? '');
    expect(contentLine).toContain('! ls');
    expect(contentLine).not.toContain('❯');
  });
});

describe('CustomEditor placeholder', () => {
  const stripAnsiLocal = (s: string): string => s.replaceAll(/\u001B\[[0-9;]*m/g, '');

  afterEach(() => {
    setLocalePreference('en');
  });

  it('renders the dim placeholder on an empty buffer', () => {
    ...
    try {
      ...
      expect(plain).toContain('Type a message (? for shortcuts)');
      // Painted with the dim SGR, not as ordinary text.
  ...
  it('hides the placeholder as soon as the buffer has content', () => {
    const editor = makeEditor();
    editor.handleInput('h');

    const plain = editor.render(90).map(stripAnsiLocal).join('\n');

    expect(plain).not.toContain('Type a message');
    expect(plain).toContain('h');
  });

  it('suppresses the placeholder in bash mode', () => {
    const editor = makeEditor();
    ...
    expect(plain).not.toContain('Type a message');
    expect(plain).toContain('! shell mode');
  });

  it('renders the zh-CN placeholder', () => {
    setLocalePreference('zh-CN');
    const editor = makeEditor();

    const plain = editor.render(90).map(stripAnsiLocal).join('\n');

    expect(plain).toContain('输入消息（? 查看快捷键）');
  });
});

// oxlint-disable-next-line no-control-regex -- ESC (\x1b) is required to match ANSI SGR escape sequences
const stripSgrForTest = (s: string): string => s.replace(/\u001B\[[0-9;]*m/g, '');

describe('CustomEditor slash menu mouse click with wrapped descriptions', () => {
  // At width 120 the menu content is 112 columns; with SLASH layout the
  // primary column is 13, leaving ~95 for descriptions — LONG wraps to two
  // rows, so menu rows are: model=0-1, coordinator=2, help=3.
  const LONG =
    'lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt '.repeat(
      2,
    );
  const items = [
    { value: 'model', label: 'model', description: LONG },
    { value: 'coordinator', label: 'coordinator', description: 'Delegates work' },
    { value: 'help', label: 'help', description: 'Shows help' },
  ];
  const press = (row: number): MouseEvent => ({ type: 'press', button: 0, col: 10, row, slotRelative: false });

  async function openSlashMenu(): Promise<CustomEditor> {
    const editor = makeEditor();
    editor.setAutocompleteProvider(providerReturning(items));
    editor.handleInput('/');
    await flushAutocomplete();
    expect(editor.isShowingAutocomplete()).toBe(true);
    return editor;
  ...
  it('selects the clicked item, not the item N rows down — with wrapped rows above', async () => {
    const editor = await openSlashMenu();
    const frame = editor.render(120).map(stripSgrForTest);
    expect(frame.some((line) => line.includes('→ model'))).toBe(true);

    // Click 'coordinator': its frame row sits two painted rows below 'model'
    // because model's description wraps. One-row-per-item math would hit 'help'.
    const coordinatorRow = frame.findIndex((line) => line.includes('coordinator'));
    expect(coordinatorRow).toBeGreaterThan(0);
    editor.handleMouse(press(coordinatorRow));

    const after = editor.render(120).map(stripSgrForTest);
    expect(after.some((line) => line.includes('→ coordinator'))).toBe(true);
  });

  it('maps a click on a wrapped description row to the item it belongs to', async () => {
    const editor = await openSlashMenu();
    const frame = editor.render(120).map(stripSgrForTest);
    const modelRow = frame.findIndex((line) => line.includes('model'));

    // 'help' starts unselected; clicking model's second (wrapped) row must not
    // move the selection onto 'coordinator'.
    editor.handleMouse(press(modelRow + 1));
    const after = editor.render(120).map(stripSgrForTest);
    expect(after.some((line) => line.includes('→ model'))).toBe(true);
  });
});

describe('CustomEditor mouse wheel scroll (mouseScroll opt-in)', () => {
  const wheel = (button: 64 | 65): MouseEvent => ({ type: 'wheel', button, col: 5, row: 2, slotRelative: false });

  it('claims the vertical wheel over an overflowing composer and pans its window', () => {
    const editor = makeEditor(); // terminal.rows = 40 → 12 visible content rows
    editor.setText(Array.from({ length: 30 }, (_, i) => `line ${i + 1}`).join('\n'));
    const initial = stripSgrForTest(editor.render(120).join('\n'));
    expect(initial).toContain('↑ 18 more');

    expect(editor.wantsMouseEvent?.(wheel(64))).toBe(true);
    editor.handleMouse(wheel(64));
    const panned = stripSgrForTest(editor.render(120).join('\n'));
    expect(panned).toContain('↑ 15 more');
    expect(panned).toContain('↓ 3 more');
  });

  it('declines the wheel while the input fits the visible window', () => {
    const editor = makeEditor();
    editor.setText('short');
    editor.render(120);
    expect(editor.wantsMouseEvent?.(wheel(64))).toBe(false);
    expect(editor.handleMouse(wheel(64))).toBe(false);
  });

  it('snaps back to the cursor on the next keystroke after panning', () => {
    const editor = makeEditor();
    editor.setText(Array.from({ length: 30 }, (_, i) => `line ${i + 1}`).join('\n'));
    editor.render(120);
    editor.handleMouse(wheel(64));
    editor.handleMouse(wheel(64));
    expect(stripSgrForTest(editor.render(120).join('\n'))).toContain('↑ 12 more');

    editor.handleInput('x');
    const out = stripSgrForTest(editor.render(120).join('\n'));
