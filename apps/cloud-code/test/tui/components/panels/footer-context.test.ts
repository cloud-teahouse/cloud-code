...
describe('FooterComponent — turn token breakdown', () => {
  afterEach(() => {
    setLocalePreference('en');
  });

  it('renders in/cache/out segments when turn usage carries cache data', () => {
    const fc = new FooterComponent(
      baseState({
        contextUsage: 0.5,
        turnUsage: {
  ...
  it('renders the cache segment with a zero placeholder when the provider reports no cache activity', () => {
    const fc = new FooterComponent(
      baseState({
        turnUsage: { inputOther: 2048, inputCacheRead: 0, inputCacheCreation: 0, output: 100 },
      }),
    );
    const [, line2] = fc.render(200);
    const out = strip(line2 ?? '');
  ...
  it('renders the breakdown with zero placeholders before the first step reports usage', () => {
    const fc = new FooterComponent(baseState({ contextUsage: 0.25 }));
    const [, line2] = fc.render(200);
    const out = strip(line2 ?? '');
