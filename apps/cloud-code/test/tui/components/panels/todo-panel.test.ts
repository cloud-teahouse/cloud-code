...
describe('activeForm', () => {
  it('renders activeForm in place of the title for in_progress rows', () => {
    const panel = new TodoPanelComponent();
    panel.setTodos([
      { title: 'Run the test suite', status: 'in_progress', activeForm: 'Running the test suite' },
    ]);
    const out = strip(panel.render(80).join('\n'));
    expect(out).toMatch(/● Running the test suite/);
    expect(out).not.toMatch(/Run the test suite/);
  });

  it('ignores activeForm for pending and done rows', () => {
    const panel = new TodoPanelComponent();
    panel.setTodos([
      { title: 'Open PR', status: 'pending', activeForm: 'Opening PR' },
      { title: 'Ship it', status: 'done', activeForm: 'Shipping it' },
    ]);
    const out = strip(panel.render(80).join('\n'));
    expect(out).toMatch(/○ Open PR/);
    expect(out).toMatch(/✓ Ship it/);
  });

  it('falls back to the title when activeForm is absent', () => {
    const panel = new TodoPanelComponent();
