...
beforeEach(() => {
  dir = join(tmpdir(), `cloud-code-tui-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('TUI config', () => {
  it('creates the default config when the file does not exist', async () => {
    ...
    expect(text).toContain('Client preferences for Cloud Code CLI.');
...
`);
  ...
  it('defaults editor.vim_mode to false when omitted', () => {
    const config = parseTuiConfig(`
[editor]
command = "code --wait"
`);
...
`);
  ...
  it('falls back to "auto" for an invalid language value (single-key catch)', () => {
    const config = parseTuiConfig(`
theme = "dark"
language = "fr"
`);
    ...
    await saveTuiConfig(
      {
        ...
        fullscreen: DEFAULT_TUI_CONFIG.fullscreen,
        editorCommand: null,
        vimMode: DEFAULT_TUI_CONFIG.vimMode,
