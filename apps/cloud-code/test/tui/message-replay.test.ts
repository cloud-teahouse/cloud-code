...
describe('CloudCodeTUI resume message replay', () => {
  ...
  it('groups replayed Bash calls from one assistant message like the live view', async () => {
    const replay: AgentReplayRecord[] = [
      message('user', [{ type: 'text', text: 'run commands' }]),
      message('assistant', [], {
        toolCalls: [
          toolCall('call_bash_1', 'Bash', { command: 'ls -la' }),
          toolCall('call_bash_2', 'Bash', { command: 'npm test' }),
        ],
      }),
      message('tool', [{ type: 'text', text: 'file a\n' }], {
        toolCallId: 'call_bash_1',
      }),
      message('tool', [{ type: 'text', text: 'pass\n' }], {
    ...
    const driver = await replayIntoDriver(replay);
    const group = driver.state.transcriptContainer.children.find(
      (child) => child instanceof ToolGroupComponent,
    );

    expect(group).toBeInstanceOf(ToolGroupComponent);
    expect((group as ToolGroupComponent).size()).toBe(2);
    const output = stripAnsi((group as ToolGroupComponent).render(120).join('\n'));
    expect(output).toContain('Bash ×2');
    expect(output).toContain('$ ls -la');
    expect(output).toContain('$ npm test');
    expect(driver.streamingUI.hasPendingToolGroup()).toBe(false);
    expect(driver.streamingUI.getToolComponent('call_bash_1')).toBeUndefined();
    expect(driver.streamingUI.getToolComponent('call_bash_2')).toBeUndefined();
  });

  it('breaks a replayed tool run when the assistant speaks between calls', async () => {
    const replay: AgentReplayRecord[] = [
      message('user', [{ type: 'text', text: 'run commands' }]),
      message('assistant', [], {
        toolCalls: [
          toolCall('call_bash_1', 'Bash', { command: 'ls' }),
          toolCall('call_bash_2', 'Bash', { command: 'pwd' }),
        ],
      }),
      message('tool', [{ type: 'text', text: 'file a\n' }], {
        toolCallId: 'call_bash_1',
      }),
      message('tool', [{ type: 'text', text: '/tmp\n' }], {
        toolCallId: 'call_bash_2',
      }),
      message('assistant', [{ type: 'text', text: 'one more check' }], {
        toolCalls: [toolCall('call_bash_3', 'Bash', { command: 'whoami' })],
      }),
      message('tool', [{ type: 'text', text: 'tsukiko\n' }], {
    ...
    const group = children.find((child) => child instanceof ToolGroupComponent);

    expect(group).toBeInstanceOf(ToolGroupComponent);
    expect((group as ToolGroupComponent).size()).toBe(2);
    // The post-text call renders standalone, after the assistant message.
    const standalone = children.filter((child) => child instanceof ToolCallComponent);
    expect(standalone).toHaveLength(1);
    expect((standalone[0] as ToolCallComponent).toolCallView.id).toBe('call_bash_3');
  });

  it('renders replayed tool results through their display ref (localized)', async () => {
    const replay: AgentReplayRecord[] = [
      message('user', [{ type: 'text', text: 'clean up cron' }]),
      message('assistant', [], {
        toolCalls: [toolCall('call_cron_delete', 'CronDelete', { id: 'abc12345' })],
      }),
      message('tool', [{ type: 'text', text: 'Deleted cron job abc12345.' }], {
        toolCallId: 'call_cron_delete',
    ...
    try {
      const transcript = stripAnsi(driver.state.transcriptContainer.render(140).join('\n'));
      expect(transcript).toContain('已删除定时任务 abc12345。');
      expect(transcript).not.toContain('Deleted cron job abc12345.');
    } finally {
      setLocalePreference('en');
    }
  });

  it('renders replayed tool results raw when no display ref rides the message', async () => {
    const replay: AgentReplayRecord[] = [
      message('user', [{ type: 'text', text: 'clean up cron' }]),
      message('assistant', [], {
        toolCalls: [toolCall('call_cron_delete', 'CronDelete', { id: 'abc12345' })],
    ...
    const driver = await replayIntoDriver(replay);
    const transcript = stripAnsi(driver.state.transcriptContainer.render(140).join('\n'));
    expect(transcript).toContain('Deleted cron job abc12345.');
  });
...
// mergeAllTurnSteps runs once after replay hydration (session-replay.ts); it
// must mirror mergeCurrentTurnSteps' zero-count guard so replayed history
// keeps the same non-counted cards (approval records, background-task finals,
// goal markers) the live view showed.
describe('mergeAllTurnSteps after replay', () => {
  function mountTurn(
    driver: ReplayDriver,
    steps: { readonly statusCards: number; readonly thinking: number },
  ): void {
    const container = driver.state.transcriptContainer;
    container.children.splice(0);
    driver.state.transcriptEntries.splice(0);
    ...
    for (let i = 0; i < steps.statusCards; i++) {
      container.addChild(new StatusMessageComponent(`Approved call ${String(i)}`));
    }
    for (let i = 0; i < steps.thinking; i++) {
      container.addChild(new ThinkingComponent(`thinking ${String(i)}`));
    }
  }

  function countInstances<T>(children: readonly unknown[], ctor: new (...args: never[]) => T): number {
    return children.filter((child) => child instanceof ctor).length;
  }

  it('keeps non-counted cards when the merge window has no thinking/tool steps', async () => {
    const driver = await makeDriver(makeSession([]));
    // 30+ steps whose oldest window (KEEP_RECENT overflow) is all cards.
    mountTurn(driver, { statusCards: TRANSCRIPT_KEEP_RECENT_STEPS + 1, thinking: 5 });
    const before = driver.state.transcriptContainer.children.length;

    driver.mergeAllTurnSteps();

    const children = driver.state.transcriptContainer.children;
    expect(children.length).toBe(before);
    expect(countInstances(children, StatusMessageComponent)).toBe(TRANSCRIPT_KEEP_RECENT_STEPS + 1);
    expect(countInstances(children, ThinkingComponent)).toBe(5);
    expect(countInstances(children, StepSummaryComponent)).toBe(0);
  });

  it('still merges overflow thinking steps into a visible summary', async () => {
    const driver = await makeDriver(makeSession([]));
    mountTurn(driver, { statusCards: 0, thinking: TRANSCRIPT_KEEP_RECENT_STEPS + 5 });

    driver.mergeAllTurnSteps();

    const children = driver.state.transcriptContainer.children;
    expect(countInstances(children, ThinkingComponent)).toBe(TRANSCRIPT_KEEP_RECENT_STEPS);
    expect(children.length).toBe(1 + 1 + TRANSCRIPT_KEEP_RECENT_STEPS);
    const summary = children.find((child) => child instanceof StepSummaryComponent);
    expect(summary).toBeDefined();
    expect((summary as StepSummaryComponent).isEmpty).toBe(false);
    expect((summary as StepSummaryComponent).render(120).join('\n')).toContain('thinking 5 times');
  });
});
