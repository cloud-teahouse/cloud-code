...
function makeRefreshHost(initial: CloudCodeConfig): {
  current: () => CloudCodeConfig;
  removeProvider: ReturnType<typeof vi.fn<(providerId: string) => Promise<CloudCodeConfig>>>;
  setConfig: ReturnType<typeof vi.fn<(patch: Partial<CloudCodeConfig>) => Promise<CloudCodeConfig>>>;
} {
  ...
  const setConfig = vi.fn(async (patch: Partial<CloudCodeConfig>) => {
...
describe('refreshAllProviderModels', () => {
  ...
  it('refreshes managed Cloud Code against environment endpoints over persisted config', async () => {
    ...
    expect(resolveOAuthToken).toHaveBeenCalledWith(CLOUD_CODE_PROVIDER_NAME, envOauthRef);
  });
  ...
  it('refreshes a hand-configured API-key provider pointing at the managed endpoint', async () => {
    ...
    vi.stubEnv('CLOUD_CODE_BASE_URL', baseUrl);
    const userAliasModel = {
      provider: 'my-kimi',
      model: 'kimi-for-coding',
      maxContextSize: 131072,
  ...
  it('matches the managed endpoint even with a trailing slash on the configured baseUrl', async () => {
    vi.stubEnv('CLOUD_CODE_BASE_URL', 'https://api.managed.example.test/coding/v1');
    const host = makeRefreshHost({
      providers: {
        'my-kimi': {
          type: 'kimi',
  ...
  it('refreshes a hand-written managed:kimi-code provider that uses an API key instead of OAuth', async () => {
    ...
    expect(host.current().providers[CLOUD_CODE_PROVIDER_NAME]).toEqual({
      type: 'kimi',
      baseUrl,
      apiKey: 'sk-distributed-key',
    });
