...
describe('error payload formatting', () => {
  ...
  it('shows concise provider filter text from CloudCodeError details', () => {
    const error = new CloudCodeError(ErrorCodes.PROVIDER_API_ERROR, filteredThinkOnlyMessage, {
      details: {
        finishReason: 'filtered',
        rawFinishReason: 'content_filter',
      },
