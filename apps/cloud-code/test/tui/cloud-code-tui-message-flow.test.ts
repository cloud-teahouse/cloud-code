...
// /status and /usage aggregate local session wire logs; keep the message-flow
// tests off the developer's real sessions dir (slow + nondeterministic).
...
import type { QueuedMessage, StatusNoticeOptions } from '#/tui/types';
...
async function makeDriver(
  ...
  const driver = new CloudCodeTUI(harness as never, makeStartupInput()) as unknown as MessageDriver;
...
// Live Agent Swarm grids pin to the slot right above the editor while running;
// on completion they move into the transcript.
function renderSwarmZone(driver: MessageDriver): string {
  return driver.state.swarmContainer.render(120).join('\n');
}
...
const originalKimiCodeHome = process.env['CLOUD_CODE_HOME'];
const originalPluginMarketplaceUrl = process.env['CLOUD_CODE_PLUGIN_MARKETPLACE_URL'];
const originalVisual = process.env['VISUAL'];
const originalEditor = process.env['EDITOR'];

async function makeTempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cloud-code-tui-'));
  tempDirs.push(dir);
  return dir;
}

async function makeExportedSessionZip(content = 'session zip'): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cloud-code-feedback-export-'));
  tempDirs.push(dir);
...
afterEach(async () => {
  ...
  if (originalKimiCodeHome === undefined) {
    ...
    process.env['CLOUD_CODE_HOME'] = originalKimiCodeHome;
  }
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['CLOUD_CODE_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['CLOUD_CODE_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('CloudCodeTUI message flow', () => {
  it('creates a new session via /clear as the alias for /new', async () => {
  ...
  it('applies theme changes from slash commands', async () => {
    process.env['CLOUD_CODE_HOME'] = await makeTempHome();
...
`,
  ...
  it('submits feedback successfully and shows the feedback id', async () => {
    ...
    expect(harness.auth.submitFeedback).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        sessionId: 'ses-1',
        version: 'cloud-code-0.0.0-test',
        model: 'k2',
      }),
    );
    expect(stripSgr(renderNoticeArea(driver))).toContain('Feedback ID: 3');
  });
  ...
  it('uploads session logs when codebase scanning fails but the session directory is available', async () => {
    ...
    const notice = stripSgr(renderNoticeArea(driver));
    expect(notice).toContain('Feedback ID: 3');
    expect(notice).toContain('attachment upload failed');
  });
  ...
  it('shows feedback API error messages without replacing them with HTTP status text', async () => {
    ...
    const transcript = stripSgr(renderTranscript(driver));
    // The spinner's final ✗ line stays in the transcript…
    expect(transcript).toContain('backend says no');
    // …while the fallback reason + URL go to the transient notice slot.
    const notice = stripSgr(renderNoticeArea(driver));
    expect(notice).toContain('Opening GitHub Issues as fallback');
    expect(notice).not.toContain('Failed to submit feedback (HTTP 500).');
  });

  it('does not submit feedback when the dialog is cancelled', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it('keeps the new session subscribed when post-create setup fails', async () => {
    ...
    await vi.waitFor(() => {
      expect(stripSgr(renderNoticeArea(driver))).toContain(
  ...
  it('switches plan mode through the editor Shift-Tab handler', async () => {
    const { driver, session } = await makeDriver();
  ...
  it('routes /yolo through session permission state', async () => {
    const { driver, session } = await makeDriver();
  ...
  it('keeps the editor draft and shows an error when media materialization fails', async () => {
    // A pasted video whose source file vanished before submit:
    // materializeVideoToCache's copyFileSync throws ENOENT synchronously.
    // The main submit path must catch it like onCtrlS does — report a status,
    // keep the process alive, and restore the draft pi-tui cleared on submit.
    process.env['CLOUD_CODE_HOME'] = await makeTempHome();
    const session = makeSession();
    const { driver } = await makeDriver(session);
    driver.state.appState.model = 'k2';
    const imageStore = (driver as unknown as { imageStore: ImageAttachmentStore }).imageStore;
    const attachment = imageStore.addVideo('video/mp4', '/nonexistent/clip-source.mp4', 'clip.mp4');
    const text = `watch ${attachment.placeholder}`;
    ...
    expect(stripSgr(renderNoticeArea(driver))).toContain('Failed to prepare media attachment');
  });

  it('confirms the optimistic echo on prompt.submitted without duplicating it', async () => {
    const { driver } = await makeDriver();

    driver.handleUserInput('hello');
    expect(driver.state.transcriptEntries.filter((e) => e.kind === 'user')).toHaveLength(1);
    ...
    // The locally drawn echo becomes authoritative — no second user bubble.
    const userEntries = driver.state.transcriptEntries.filter((e) => e.kind === 'user');
    expect(userEntries).toHaveLength(1);
    expect(userEntries[0]).toEqual(expect.objectContaining({ content: 'hello' }));
    expect(stripSgr(renderTranscript(driver))).toContain('hello');
  });

  it('removes the optimistic echo when the prompt call fails', async () => {
    const session = makeSession({
      prompt: vi.fn(async () => {
    ...
    // Rejected submit: the bubble leaves the entry list and the container,
    // and the failure is reported instead.
    await vi.waitFor(() => {
      expect(driver.state.transcriptEntries.filter((e) => e.kind === 'user')).toHaveLength(0);
    });
    await vi.waitFor(() => {
      expect(stripSgr(renderNoticeArea(driver))).toContain('provider offline');
    });
    expect(stripSgr(renderTranscript(driver))).not.toContain('✨ hello');
    expect(driver.state.appState.streamingPhase).toBe('idle');
  });

  it('draws a ▔ divider above editor-replacement panels and clears it on restore', async () => {
    const { driver } = await makeDriver();

    driver.handleUserInput('/help');

    await vi.waitFor(() => {
      expect(driver.state.editorContainer.topSeparator).toBe(true);
    });
    const firstRow = stripSgr(driver.state.editorContainer.render(80)[0] ?? '');
    expect(firstRow).toContain('▔');

    (driver.state.editorContainer.children[0] as { handleInput(data: string): void }).handleInput(
      '',
    );

    await vi.waitFor(() => {
      expect(driver.state.editorContainer.topSeparator).toBe(false);
    });
  ...
  it('keeps command notices that are not part of the undone context', async () => {
    ...
    await vi.waitFor(() => {
      expect(stripSgr(renderNoticeArea(driver))).toContain('Auto mode: ON');
    });
    ...
    // The command notice lives in the transient notice slot now, so /undo
  ...
  it('removes AgentSwarm progress from undone turns', async () => {
    ...
    // The running grid pins to the slot zone above the editor, not the transcript.
    const swarmZone = stripSgr(renderSwarmZone(driver));
    expect(swarmZone).toContain('Agent Swarm');
    expect(swarmZone).toContain('Review changed files');
  ...
  it('removes debug timing status from undone turns', async () => {
    const { driver, session } = await makeDriver();
    const previousDebug = process.env['CLOUD_CODE_DEBUG'];
    process.env['CLOUD_CODE_DEBUG'] = '1';
    try {
      driver.handleUserInput('hello');
      driver.sessionEventHandler.handleEvent(
        {
      ...
      if (previousDebug === undefined) {
        delete process.env['CLOUD_CODE_DEBUG'];
      } else {
        process.env['CLOUD_CODE_DEBUG'] = previousDebug;
      }
    }
  });
  ...
  it('resets the streaming phase when steering mid-goal input fails', async () => {
    ...
    // Cloud Code routes transient errors to the notice slot, not the transcript.
    expect(stripSgr(renderNoticeArea(driver))).toContain('Failed to steer: session closed');
  });
  ...
  it('collapses click-expanded cards and groups on the second ctrl+o toggle', async () => {
    const { driver } = await makeDriver();

    const card = new ToolCallComponent(
      { id: 'call_bash_solo', name: 'Bash', args: { command: 'seq 1 20' } },
      {
        tool_call_id: 'call_bash_solo',
        output: Array.from({ length: 20 }, (_, i) => `output line ${i + 1}`).join('\n'),
        is_error: false,
      },
      driver.state.ui,
    );
    driver.state.transcriptContainer.addChild(card);

    const group = new ToolGroupComponent('Bash', driver.state.ui);
    const a = new ToolCallComponent(
      { id: 'call_bash_g1', name: 'Bash', args: { command: 'ls' } },
      undefined,
      driver.state.ui,
    );
    const b = new ToolCallComponent(
      { id: 'call_bash_g2', name: 'Bash', args: { command: 'pwd' } },
      undefined,
      driver.state.ui,
    );
    group.attach('call_bash_g1', a);
    group.attach('call_bash_g2', b);
    a.setResult({ tool_call_id: 'call_bash_g1', output: 'group alpha\n', is_error: false });
    b.setResult({ tool_call_id: 'call_bash_g2', output: 'group beta\n', is_error: false });
    driver.state.transcriptContainer.addChild(group);

    card.setClickExpanded(true);
    group.setClickExpanded(true);
    let text = stripSgr(renderTranscript(driver));
    ...
    text = stripSgr(renderTranscript(driver));
    expect(text).not.toContain('output line 20');
    expect(text).not.toContain('group alpha');
  ...
  it('renders an error instead of prompting when no model is selected', async () => {
    ...
    expect(session.prompt).not.toHaveBeenCalled();
    expect(driver.state.noticeContainer.render(120).join('\n')).toContain('LLM not set');
  });
  ...
  it('cancels and closes a running /btw panel on Escape', async () => {
    ...
    const requestCollapseRender = vi.spyOn(driver.state.ui, 'requestCollapseRender').mockImplementation(() => {});
    driver.state.editor.onEscape?.();

    expect(session.cancel).toHaveBeenCalledOnce();
    expect(driver.state.btwPanelContainer.children).toHaveLength(0);
    expect(requestCollapseRender).toHaveBeenCalled();
    const editorTopBorder = stripSgr(driver.state.editor.render(80)[0] ?? '');
    expect(editorTopBorder.startsWith('╭')).toBe(true);
    expect(editorTopBorder.endsWith('╮')).toBe(true);
    expect(driver.state.editor.focused).toBe(true);
  ...
  it('does not run /btw without a selected model', async () => {
    ...
    expect(driver.state.btwPanelContainer.children).toHaveLength(0);
    expect(stripSgr(renderNoticeArea(driver))).toContain('LLM not set');
  ...
  it('pins the running AgentSwarm grid in the slot zone instead of the tool-card body', async () => {
    const { driver } = await makeDriver();
    const sendQueued = vi.fn();
    ...
    let transcript = stripSgr(renderSwarmZone(driver));
    ...
    transcript = stripSgr(renderSwarmZone(driver));
  ...
  it('marks only core user-cancellation subagent failures as cancelled', async () => {
    ...
    const transcript = stripSgr(driver.state.swarmContainer.render(200).join('\n'));
  ...
  it('does not let later transcript entries reduce the AgentSwarm grid height', async () => {
    ...
    const outerChildren = driver.state.rootContainer.children;
    ...
    const swarmProgress = driver.state.swarmContainer.children.find(
  ...
  it('moves the swarm grid from the slot zone into the transcript on completion', async () => {
    const { driver } = await makeDriver();
    const sendQueued = vi.fn();
    ...
    // While running: pinned in the slot zone, absent from the transcript.
    expect(driver.state.swarmContainer.children).toHaveLength(1);
    expect(
      driver.state.transcriptContainer.children.some(
        (child) => child instanceof AgentSwarmProgressComponent,
      ),
    ).toBe(false);
    ...
    // On completion: the grid leaves the slot and becomes a transcript record.
    expect(driver.state.swarmContainer.children).toHaveLength(0);
    expect(
      driver.state.transcriptContainer.children.some(
        (child) => child instanceof AgentSwarmProgressComponent,
      ),
  ...
  it('shows plan review reject on the plan card without an approval notice', async () => {
    ...
    // Plan choices: 1 Approve · 2 Approve+Auto · 3 Approve with feedback ·
    // 4 Reject · 5 Revise.
    (driver.state.editorContainer.children[0] as ApprovalPanelComponent).handleInput('4');
  ...
  it('renders /status as a tabbed dialog with session, account, and MCP facts', async () => {
    const session = makeSession({
      listMcpServers: vi.fn(async () => [
        {
          name: 'local-tools',
    ...
    const { driver, harness } = await makeDriver(session);
    vi.mocked(harness.auth.getAccountSnapshot).mockImplementation(
      async (providerName?: string) =>
        providerName === 'managed:chatgpt-codex'
          ? { state: 'logged-in', email: 'user@example.com', planType: 'plus' }
          : { state: 'logged-in' },
    );
    vi.mocked(harness.auth.getManagedUsage).mockResolvedValue({
      kind: 'ok',
      summary: null,
      limits: [],
      extraUsage: null,
    ...
    await vi.waitFor(() => {
      const dialog = driver.state.editorContainer.children[0];
      if (!(dialog instanceof StatusDialogComponent)) {
        throw new Error(
          `dialog not mounted; notice=${JSON.stringify(renderNoticeArea(driver))}; children=${driver.state.editorContainer.children.length}`,
        );
      }
      expect(listMcpServers.mock.calls.length).toBeGreaterThan(previousCalls);
      const output = stripSgr(dialog.render(120).join('\n'));
      expect(output).toContain('Status');
      expect(output).toContain('Kimi Code');
      expect(output).toContain('ChatGPT');
      expect(output).toContain('Stats');
      expect(output).toContain('>_ Cloud Code CLI');
      expect(output).toContain('Model');
      expect(output).toContain('Session');
      expect(output).toContain('ses-1');
      expect(output).toContain('1 connected');
      // The account email lives on the ChatGPT tab — switch to it.
      dialog.handleInput('\t');
      dialog.handleInput('\t');
      const chatGptOutput = stripSgr(dialog.render(120).join('\n'));
      expect(chatGptOutput).toContain('user@example.com (Plus)');
    });
  });
  ...
  it('renders /mcp list failures as command boundary errors', async () => {
    ...
    await vi.waitFor(() => {
      const output = stripSgr(driver.state.noticeContainer.render(120).join('\n'));
  ...
  it('shows a quota note after installing a quota-consuming official plugin', async () => {
    ...
    await vi.waitFor(() => {
      const notice = stripSgr(renderNoticeArea(driver));
      expect(notice).toContain('Run /new or /reload to apply plugin changes.');
      expect(notice).toContain('Note: This plugin consumes your quota.');
    });
  });
  ...
  it('does not show the quota note for a same-id fork installed from a local path', async () => {
    ...
    await vi.waitFor(() => {
      const notice = stripSgr(renderNoticeArea(driver));
      expect(notice).toContain('Installed Kimi Datasource');
    });
    expect(stripSgr(renderNoticeArea(driver))).not.toContain(
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
    ...
    process.env['CLOUD_CODE_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    await vi.waitFor(() => {
      const notice = stripSgr(renderNoticeArea(driver));
      expect(notice).toContain('Installed Demo');
      expect(notice).toContain('Run /new or /reload to apply plugin changes.');
      expect(notice).not.toContain('Note: This plugin consumes your quota.');
    });
  ...
  it('installs default marketplace entries through plain install', async () => {
    ...
    try {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith(CLOUD_CODE_PLUGIN_MARKETPLACE_URL);
    } finally {
  ...
  it('shows an inline Official error when the marketplace is unreachable, keeping the panel open', async () => {
    const originalFetch = globalThis.fetch;
    process.env['CLOUD_CODE_PLUGIN_MARKETPLACE_URL'] = 'https://example.test/marketplace.json';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('requires confirmation before /plugins remove removes a plugin', async () => {
    ...
    await vi.waitFor(() => {
      expect(stripSgr(renderNoticeArea(driver))).toContain('Remove cancelled: demo.');
    });
    expect(session.removePlugin).not.toHaveBeenCalled();
  });
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(filteredOutput).toContain('⌕ tu');
  ...
  it('uses the effective effort returned after a model-switch fallback', async () => {
    ...
    expect(driver.state.appState.model).toBe('turbo');
    expect(driver.state.appState.thinkingEffort).toBe('mid');
    expect(renderNoticeArea(driver)).toContain('Switched to kimi-turbo with thinking mid.');
  });
  ...
  it('does not write config when re-confirming the current effort in the picker', async () => {
    ...
    await vi.waitFor(() => {
      expect(renderNoticeArea(driver)).toContain('Already using Kimi K2 with thinking high.');
    });
    expect(setConfig).not.toHaveBeenCalled();
    expect(session.setThinking).not.toHaveBeenCalled();
  });
  ...
  it('live-updates the /model picker rows after the OAuth refresh lands', async () => {
    ...
    const tui = driver as unknown as CloudCodeTUI;
  ...
  it('opens the /model picker immediately while the OAuth refresh is still pending', async () => {
    const { driver } = await makeDriver(makeSession(), {
      getConfig: vi.fn(async () => ({
        models: {
          k2: {
    ...
    // The picker mounts on the cached model list without waiting for the
    // network refresh (which never resolves here).
    expect(refreshOAuthProviderModels).toHaveBeenCalledOnce();
    const picker = driver.state.editorContainer.children[0];
    expect(picker).toBeInstanceOf(TabbedModelSelectorComponent);
    const output = stripSgr((picker as TabbedModelSelectorComponent).render(120).join('\n'));
  ...
  it('enables search in the shared model selector helper', async () => {
    ...
    // Layered Esc: clear the query, then unfocus the search box, then cancel.
    (picker as ModelSelectorComponent).handleInput('\u001B');
    (picker as ModelSelectorComponent).handleInput('\u001B');
    (picker as ModelSelectorComponent).handleInput('\u001B');
  ...
  it('forks the active session and switches to the returned session', async () => {
    ...
    try {
      ...
      expect(driver.state.noticeContainer.render(120).join('\n')).toContain(
        'Session forked (ses-fork). To return to the original session: cloudcode -r ses-source',
      );
    } finally {
      process.title = originalTitle;
    }
...
describe('/model status displayName override', () => {
  it('shows the overridden display name in the switch status', async () => {
    ...
    expect(renderNoticeArea(driver)).toContain('Switched to Custom Turbo with thinking on.');
    expect(renderNoticeArea(driver)).not.toContain('Remote Turbo');
  });
});
...
describe('/effort support_efforts override', () => {
  it('warns and applies efforts hidden by an Anthropic support_efforts override', async () => {
    ...
    await vi.waitFor(() => {
      expect(renderNoticeArea(driver)).toContain('Thinking set to max.');
    });
    // The unlisted-effort warning is pinned into the transcript.
    ...
    expect(stripSgr(renderNoticeArea(driver))).toContain('Thinking set to max.');
  });
  ...
  it('keeps rejecting efforts hidden by a Kimi support_efforts override', async () => {
    ...
    await vi.waitFor(() => {
      expect(renderNoticeArea(driver)).toContain(
...
describe('T2 interrupt-before-output input recall', () => {
  beforeEach(() => {
    setLocalePreference('en');
  });

  it('Esc recalls the submitted prompt into the editor when the turn produced no output', async () => {
    const { driver, session } = await makeDriver();

    driver.handleUserInput('recall me please');
    await vi.waitFor(() => {
      expect(session.prompt).toHaveBeenCalledWith('recall me please');
    });
    expect(driver.state.editor.getText()).toBe('');

    driver.state.editor.onEscape?.();

    // The recall arms the server-side withdrawal: the interrupted prompt is
    // pulled out of the model-visible context as the turn unwinds.
    expect(session.cancel).toHaveBeenCalledWith({ withdrawInput: true });
    expect(driver.state.editor.getText()).toBe('recall me please');
    expect(stripSgr(renderNoticeArea(driver))).toContain(
      'Cancelled — your input was restored to the editor for re-editing.',
    );
    // …and the recalled echo leaves the transcript instead of staying as a
    // normal message next to the edited resend.
    expect(
      driver.state.transcriptEntries.some(
        (entry) => entry.kind === 'user' && entry.content === 'recall me please',
      ),
    ).toBe(false);
  });

  it('Ctrl+C recalls the submitted prompt with the same semantics', async () => {
    const { driver, session } = await makeDriver();

    driver.handleUserInput('recall via ctrl-c');
    await vi.waitFor(() => {
      expect(session.prompt).toHaveBeenCalledWith('recall via ctrl-c');
    });

    driver.state.editor.onCtrlC?.();

    expect(session.cancel).toHaveBeenCalledWith({ withdrawInput: true });
    expect(driver.state.editor.getText()).toBe('recall via ctrl-c');
    expect(
      driver.state.transcriptEntries.some(
        (entry) => entry.kind === 'user' && entry.content === 'recall via ctrl-c',
      ),
    ).toBe(false);
  });

  it('does not recall once the assistant streamed visible output', async () => {
    const { driver, session } = await makeDriver();

    driver.handleUserInput('already answered');
    await vi.waitFor(() => {
    ...
    // Output already streamed: plain cancel, no withdrawal, and the user
    // message stays in the transcript.
    expect(session.cancel).toHaveBeenCalledWith({ withdrawInput: false });
    expect(driver.state.editor.getText()).toBe('');
    expect(
      driver.state.transcriptEntries.some(
        (entry) => entry.kind === 'user' && entry.content === 'already answered',
      ),
    ).toBe(true);
  });

  it('does not recall a bash `!` command cancelled during the shell phase', async () => {
    const cancelShellCommand = vi.fn(async () => {});
    const session = makeSession({
      runShellCommand: vi.fn(() => new Promise(() => {})),
      cancelShellCommand,
    });
    const { driver } = await makeDriver(session);
    driver.state.appState.inputMode = 'bash';
    ...
    driver.handleUserInput('sleep 100');
  ...
  it('recalls the latest steer text when the steered turn still has no output', async () => {
    const { driver, session } = await makeDriver();

    driver.handleUserInput('original prompt');
    await vi.waitFor(() => {
      expect(session.prompt).toHaveBeenCalledWith('original prompt');
    });

    driver.state.editor.setText('steer note');
    driver.state.editor.onCtrlS?.();
    await vi.waitFor(() => {
    ...
    // Same withdrawal semantics as the prompt recall: the cancel carries the
    // withdrawal, and the recalled steer's echo leaves the transcript while
    // the original prompt's echo stays.
    expect(session.cancel).toHaveBeenCalledWith({ withdrawInput: true });
    expect(driver.state.editor.getText()).toBe('steer note');
    expect(
      driver.state.transcriptEntries.some(
        (entry) => entry.kind === 'user' && entry.content === 'steer note',
      ),
    ).toBe(false);
    expect(
      driver.state.transcriptEntries.some(
        (entry) => entry.kind === 'user' && entry.content === 'original prompt',
      ),
    ).toBe(true);
  });

  it('does not recall on the compaction cancel path', async () => {
    const { driver, session } = await makeDriver();
    driver.sessionEventHandler.handleEvent(
      {
        type: 'compaction.started',
...
describe('transient notice expiry', () => {
  beforeEach(() => {
    setLocalePreference('en');
  });
  ...
  it('auto-clears a transient notice after the display timeout', async () => {
    const { driver } = await makeDriver();
    vi.useFakeTimers();

    driver.showStatus('temporary note');
    expect(stripSgr(renderNoticeArea(driver))).toContain('temporary note');

    // Still visible just before the 8s window ends, gone right at it.
  ...
  it('resets the expiry timer when a newer notice arrives', async () => {
    const { driver } = await makeDriver();
    vi.useFakeTimers();

    driver.showStatus('first note');
    vi.advanceTimersByTime(5_000);
    driver.showStatus('second note');

    // Half the new window has passed: the replacement stays, and the stale
    // timer for the first notice must not clobber it.
    vi.advanceTimersByTime(5_000);
    const notice = stripSgr(renderNoticeArea(driver));
    expect(notice).toContain('second note');
    expect(notice).not.toContain('first note');

    vi.advanceTimersByTime(3_000);
    expect(stripSgr(renderNoticeArea(driver)).trim()).toBe('');
  });

  it('does not expire recorded (transcript) notices', async () => {
    const { driver } = await makeDriver();
    vi.useFakeTimers();

    driver.showStatus('kept in transcript', undefined, { transcript: true });
