...
import { CLOUD_CODE_HOME_ENV } from '#/constant/app';
...
function setupTempCache(): { cleanup: () => void } {
  ...
  const prev = process.env[CLOUD_CODE_HOME_ENV];
  process.env[CLOUD_CODE_HOME_ENV] = home;
  return {
    cleanup: () => {
      if (prev === undefined) delete process.env[CLOUD_CODE_HOME_ENV];
      else process.env[CLOUD_CODE_HOME_ENV] = prev;
...
describe('extractMediaAttachments', () => {
  ...
  it('inserts a compression caption before an image that was compressed at paste time', () => {
    ...
    const att = store.addImage(new Uint8Array([1, 2, 3]), 'image/png', 2000, 2000, {
      path: '/tmp/cloud-code-original-images/abc.png',
      width: 2600,
      height: 2600,
      byteLength: 123456,
      mime: 'image/png',
    ...
    expect(caption.text).toContain('/tmp/cloud-code-original-images/abc.png');
    expect(r.parts[1]).toEqual({
      type: 'image_url',
