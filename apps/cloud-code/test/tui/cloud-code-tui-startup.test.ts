...
import { log, type GoalSnapshot } from '@cloud-code/sdk';
...
/** Access to private lifecycle helpers on CloudCodeTUI for startup tests. */
interface PrivateDriver extends StartupDriver {
  initMainTui(): Promise<boolean>;
}

function makeStartupInput(
  cliOptions: Partial<CloudCodeTUIStartupInput['cliOptions']> = {},
  tuiConfig: Partial<CloudCodeTUIStartupInput['tuiConfig']> = {},
): CloudCodeTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: CloudCodeTUIStartupInput) {
  const driver = new CloudCodeTUI(harness as never, input) as unknown as StartupDriver;
...
describe('CloudCodeTUI startup', () => {
  ...
  it('clears the sessions picker search query when toggling scope with Ctrl+A', async () => {
    ...
    // The fresh picker shows the search box placeholder instead of the query.
    expect(output).toContain('⌕ Search…');
    expect(output).not.toContain('⌕ cwd');
  });

  it('ignores a second Enter while the picked session switch is still in flight', async () => {
    const targetSession = makeSession({ id: 'ses-target' });
    let resolveResume: ((session: unknown) => void) | undefined;
    ...
    const harness = makeHarness(makeSession({ id: 'ses-current' }), {
      resumeSession,
      listSessions: vi.fn(async () => [listedSession]),
    });
    const driver = makeDriver(harness, makeStartupInput());
    await expect(driver.init()).resolves.toBe(false);
    ...
    // The first Enter is parked inside harness.resumeSession; the second one
    // must have been swallowed by the in-flight latch, not started a second
    // interleaved switch.
    expect(resumeSession).toHaveBeenCalledTimes(1);

    resolveResume?.(targetSession);
    await vi.waitFor(() => {
      expect(driver.state.activeDialog).toBeNull();
    });
    expect(resumeSession).toHaveBeenCalledTimes(1);
  ...
  it('exits after picking another cwd from the startup picker', async () => {
    ...
    await expect((driver as unknown as PrivateDriver).initMainTui()).resolves.toBe(false);
    await (driver as unknown as { bootstrapFromPicker(): Promise<void> }).bootstrapFromPicker();
  ...
  it('logs in again when a token already exists', async () => {
    const session = makeSession();
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({
  ...
  it('logs login failures with session context', async () => {
    ...
    const session = makeSession();
    const loginError = new Error('Failed to list Cloud Code models (HTTP 402).');
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({ providers: [] })),
        login: vi.fn(async () => {
      ...
      expect(warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          error: expect.objectContaining({
            message: 'Failed to list Cloud Code models (HTTP 402).',
          }),
        }),
      );
    } finally {
  ...
  it('clears managed credentials and session state on logout', async () => {
    const session = makeSession();
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
  ...
  it('does not render remote banners (brand independence)', async () => {
