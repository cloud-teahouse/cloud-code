...
User-facing changes absorbed from upstream kimi-code 0.29.2 / 0.30.0 / 0.31.x
(v1 engine only; agent-core-v2, kap-server, kimi-web, and telemetry remain
excluded):

- **Custom agents on the v1 engine**: Markdown agent files from user/project/
  extra/explicit roots with precedence rules, SYSTEM.md main-prompt override,
  `--agent`/`--agent-file` in shell and print modes, and plugin-contributed
  custom agents (an `agents/` dir in the plugin). Project discovery lives at
  `.cloud-code/agents`.
- **Secondary model**: `/secondary_model` command (persist + live-apply),
  `[secondary_model]` recipe with patch fields synthesized as a derived
  `__secondary__` entry, `CLOUD_CODE_SECONDARY_MODEL` /
  `CLOUD_CODE_SECONDARY_EFFORT` env overrides (unflagged, stable feature).
  Legacy `effort =` recipes migrate to `default_effort` on read.
- **Plugins**: manifest `systemPrompt`/`systemPromptPath` contributes
  system-prompt sections (32 KB budget), quota note after installing official
  quota-billed plugins (e.g. Kimi Datasource), and an update notice when an
  official plugin in use has a marketplace update.
- **Footer status line**: `[status_line]` in `tui.toml` — reorder/drop the
  built-in slots (mode/goal/model/tasks/cwd/git/tips) or replace line 1 with
  a custom command fed a JSON snapshot on stdin.
- **Reliability**: quota-exhausted 429 fails fast instead of burning the
  retry budget (Moonshot `exceeded_current_quota_error`, OpenAI
  `insufficient_quota`); TaskOutput drops the blocking wait (snapshot only);
  validation-rejected tool calls count toward the repeat circuit breaker;
  goal pursuit continues across per-turn step-limit boundaries; built-in
  models.dev catalog fallback when the fetch fails; provider-model refresh
  drains briefly at exit so its config writes land atomically.
- **OAuth**: structured managed-usage rows (window/duration/resetAt),
  managed user profile fetch, unified host identity headers (User-Agent +
  X-Msh-*) on OAuth requests.
- **Subagent tool**: `model` accepts `primary`, `secondary`, or an explicit
  alias (profile `model_preference` honored); available secondary models are
  advertised in the tool description when configured.
...
- [#1186](https://github.com/MoonshotAI/kimi-code/pull/1186) [`821847c`](https://github.com/MoonshotAI/kimi-code/commit/821847cb4b88d9128014609aad307ab8d9e9a5f3) - Add `CLOUD_CODE_CUSTOM_HEADERS` for custom outbound LLM request headers and send the `User-Agent` header to non-Kimi providers. Set `CLOUD_CODE_CUSTOM_HEADERS` to newline-separated `Name: Value` lines.

- [#1186](https://github.com/MoonshotAI/kimi-code/pull/1186) [`821847c`](https://github.com/MoonshotAI/kimi-code/commit/821847cb4b88d9128014609aad307ab8d9e9a5f3) - Route managed Cloud Code models on the Anthropic-compatible protocol through the beta Messages API.
...
- [#1170](https://github.com/MoonshotAI/kimi-code/pull/1170) [`cf558cd`](https://github.com/MoonshotAI/kimi-code/commit/cf558cd74267393d6497ddedf25e192eaac4f94b) - Support the Anthropic-compatible protocol for managed Cloud Code, including video input.
...
- [#1006](https://github.com/MoonshotAI/kimi-code/pull/1006) [`60dfb68`](https://github.com/MoonshotAI/kimi-code/commit/60dfb68a2d4c342cfbad5f48d4d269fb6cdd43c0) - Add server authentication and safe `--host` exposure. The local server now
  ...
  public-binding hardening tier: mandatory `CLOUD_CODE_PASSWORD`, TLS (or
...
- [#484](https://github.com/MoonshotAI/kimi-code/pull/484) [`f863127`](https://github.com/MoonshotAI/kimi-code/commit/f863127ab7e8b8e2e9af11c54694c08900e3103a) - Add custom color themes. Define your own palette as a JSON file in `~/.cloud-code/themes/`, or generate one with the built-in `/custom-theme` skill command.
...
- [#334](https://github.com/MoonshotAI/kimi-code/pull/334) [`eeefa98`](https://github.com/MoonshotAI/kimi-code/commit/eeefa98083e9d037d2ba7c59de9e5eb51b19fdd7) - Add a `kimi upgrade` command for manually checking and upgrade Cloud Code CLI.
...
- [#31](https://github.com/MoonshotAI/kimi-code/pull/31) [`475ebad`](https://github.com/MoonshotAI/kimi-code/commit/475ebadc2070e3b878789f6a89ce191b1bd957a9) - Migrate user skills from `~/.kimi/skills/` to `~/.cloud-code/skills/` during the first-launch migration; existing target skills are kept.
