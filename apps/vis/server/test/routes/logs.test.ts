...
describe('logs route (local sessions)', () => {
  ...
  it('reads the session log from the session dir and the global log from CLOUD_CODE_HOME', async () => {
    ...
    await writeFile(join(sessionDir, 'logs', 'cloud-code.log'), '2026-06-01T00:00:00.000Z INFO  session boot  k=v\n');
    // …but the shared global log lives at <home>/logs/cloud-code.log, NOT under
    // the session dir. Before the fix this was reported as unavailable.
    await mkdir(join(home, 'logs'), { recursive: true });
    await writeFile(join(home, 'logs', 'cloud-code.log'), '2026-06-01T00:00:01.000Z WARN  global thing  g=1\n');
  ...
  it('discovers a rotated session log when the active file has rotated away', async () => {
    ...
    cleanup = c;
    await mkdir(join(sessionDir, 'logs'), { recursive: true });
    // Only an archive exists — no active cloud-code.log.
    await writeFile(join(sessionDir, 'logs', 'cloud-code.log.1'), '2026-06-01T00:00:00.000Z INFO  rotated only  r=1\n');
  ...
  it('concatenates rotated + active session logs oldest-first', async () => {
    ...
    cleanup = c;
    await mkdir(join(sessionDir, 'logs'), { recursive: true });
    await writeFile(join(sessionDir, 'logs', 'cloud-code.log.2'), '2026-06-01T00:00:00.000Z INFO  oldest\n');
    await writeFile(join(sessionDir, 'logs', 'cloud-code.log.1'), '2026-06-01T00:00:01.000Z INFO  middle\n');
    await writeFile(join(sessionDir, 'logs', 'cloud-code.log'), '2026-06-01T00:00:02.000Z INFO  newest\n');
