...
import { CLOUD_CODE_HOME } from '../config';
import { discoverLogFiles, readLogs } from '../lib/log-reader';
import { readSessionDetail } from '../lib/session-store';

const SESSION_LOG_REL = ['logs', 'cloud-code.log'] as const;
const GLOBAL_LOG_REL = ['logs', 'global', 'cloud-code.log'] as const;
const HOME_GLOBAL_LOG_REL = ['logs', 'cloud-code.log'] as const;

export function logsRoute(home: string = CLOUD_CODE_HOME): Hono {
  const r = new Hono();
  r.get('/:id/logs', async (c) => {
    ...
    // it is captured under the session dir (logs/global/cloud-code.log); for a
    // live local session it lives at <CLOUD_CODE_HOME>/logs/cloud-code.log
    ...
    // Either log may have rotated (cloud-code.log.1, .2, …); discover the active
