 ...
 *   - Windows resolves Git Bash via `CLOUD_CODE_SHELL_PATH`, `git.exe` on PATH
...
describe('detectEnvironment', () => {
  ...
  it('uses CLOUD_CODE_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { CLOUD_CODE_SHELL_PATH: 'D:\\custom\\bash.exe' },
