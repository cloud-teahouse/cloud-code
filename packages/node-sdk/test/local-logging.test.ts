...
import { createCloudCodeHarness, log } from '#/index';
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    ...
    const globalPath = join(homeDir, 'logs', 'cloud-code.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'cloud-code.log');

    // Trigger an export — this flushes both global and session via CloudCodeCore
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/cloud-code.log')).toBe(true);
    expect(entries.has('logs/global/cloud-code.log')).toBe(false);
    expect(entries.get('logs/cloud-code.log')!.toString('utf-8')).toContain(
      'session export marker',
    );
    expect(result.manifest.sessionLogPath).toBe('logs/cloud-code.log');
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/cloud-code.log');
  ...
  it('default export works when no session log file exists', async () => {
    ...
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/cloud-code.log')).toBe(false);
  ...
  it('default export includes rotated session log files without requiring active cloud-code.log', async () => {
    ...
    try {
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/cloud-code.log' || entry.startsWith('logs/cloud-code.log.'),
      );
      expect(sessionLogEntries.length).toBeGreaterThan(0);
      expect(sessionLogEntries).toContain('logs/cloud-code.log.1');
      expect(entries.has('logs/global/cloud-code.log')).toBe(false);
      if (entries.has('logs/cloud-code.log')) {
        expect(result.manifest.sessionLogPath).toBe('logs/cloud-code.log');
      } else {
        expect(result.manifest.sessionLogPath).toBeUndefined();
      }
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/cloud-code.log')).toBe(true);
    expect(result.manifest.globalLogPath).toBe('logs/global/cloud-code.log');
    // Global log carries entries that don't have a sessionId routed to a sink.
    expect(entries.get('logs/global/cloud-code.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });
  ...
  it('--include-global-log bundles the active root global log path', async () => {
    ...
    const workDir = await makeTempDir('kimi-log-work-');
    const first = createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir: firstHome });
    const firstSession = await first.createSession({ id: 'ses_first_global_export', workDir });
    const second = createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir: secondHome });
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/cloud-code.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'cloud-code.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/cloud-code.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/cloud-code.log')!.toString('utf-8');
  ...
  it('multiple CloudCodeHarness constructions in the same process do not throw', async () => {
    const homeDir = await makeTempDir('kimi-log-home-');
    expect(() => createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
  });
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'cloud-code.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'cloud-code.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    try {
      ...
      expect(logsDir).not.toContain('cloud-code.log');
    } finally {
      restoreLogEnv(env);
    }
  });

  it('CloudCodeHarness.close() flushes the global log', async () => {
    const homeDir = await makeTempDir('kimi-log-home-');
    const harness = createCloudCodeHarness({ identity: TEST_IDENTITY, homeDir });
