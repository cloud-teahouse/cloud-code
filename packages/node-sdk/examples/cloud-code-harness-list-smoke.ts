...
async function main(): Promise<void> {
  const homeDir = await mkdtemp(join(tmpdir(), 'cloud-code-harness-list-home-'));
  const workDirA = await mkdtemp(join(tmpdir(), 'cloud-code-harness-list-work-a-'));
  const workDirB = await mkdtemp(join(tmpdir(), 'cloud-code-harness-list-work-b-'));
