...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/node-sdk/examples/cloud-code-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Cloud Code CLI home dir; defaults to CLOUD_CODE_HOME or ~/.cloud-code
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveCloudCodeHome(options.homeDir);
  const harness = createCloudCodeHarness({
    identity: { userAgentProduct: 'cloud-code-cli', version: 'log-marker', platform: 'kimi_code_cli' },
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'cloud-code.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that KIMI_LOG_LEVEL is not "off" and that the session id exists in this CLOUD_CODE_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
