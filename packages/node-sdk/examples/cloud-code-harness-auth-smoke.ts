...
import { createCloudCodeHarness, type CloudCodeHarness } from '@cloud-code/sdk';

import { smokeIdentityFromEnv, runPromptToEnd } from './runtime-smoke-helpers';

const MANAGED_CLOUD_CODE_PROVIDER = 'managed:kimi-code';
...
async function main(): Promise<void> {
  ...
  const harness = createCloudCodeHarness({ homeDir, identity: smokeIdentityFromEnv() });
  ...
  try {
    if (forceLogin) {
      await harness.auth.logout(MANAGED_CLOUD_CODE_PROVIDER);
    ...
    const status = await harness.auth.status(MANAGED_CLOUD_CODE_PROVIDER);
    const usage = await harness.auth.getManagedUsage(MANAGED_CLOUD_CODE_PROVIDER);
    ...
    if (config.providers[MANAGED_CLOUD_CODE_PROVIDER]?.oauth?.key !== 'oauth/kimi-code') {
    ...
    if (!keepToken) {
      await harness.auth.logout(MANAGED_CLOUD_CODE_PROVIDER).catch(() => {});
    }
    await harness.close();
...
function printUsage(usage: Awaited<ReturnType<CloudCodeHarness['auth']['getManagedUsage']>>): void {
  if (usage.kind === 'error') {
