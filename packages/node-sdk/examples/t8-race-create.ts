...
const harnessA = createCloudCodeHarness({ identity, homeDir });
const harnessB = createCloudCodeHarness({ identity, homeDir });

async function run(label: string, h: CloudCodeHarness): Promise<void> {
  try {
