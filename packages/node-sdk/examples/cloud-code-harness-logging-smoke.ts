...
const SESSION_LOG = 'logs/cloud-code.log';
const GLOBAL_LOG = 'logs/global/cloud-code.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const TEST_HOME = join(homedir(), '.cloud-code-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'cloud-code.log' || file.startsWith('cloud-code.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('cloud-code.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('cloud-code.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['CLOUD_CODE_HOME'] = TEST_HOME;
  ...
  const harness = createCloudCodeHarness({
    identity: { userAgentProduct: 'cloud-code-cli', version: '0.1.1', platform: 'kimi_code_cli' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        'Open these files:',
        '  - logs/cloud-code.log',
        '  - logs/cloud-code.log.1',
        `  - ${summary.sessionDir}/logs/cloud-code.log`,
        `  - ${summary.sessionDir}/logs/cloud-code.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - cloud-code.log.1 exists',
        '  - cloud-code.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/cloud-code.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/cloud-code.log',
        '  - global rotated logs such as ~/.cloud-code-test/logs/cloud-code.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
