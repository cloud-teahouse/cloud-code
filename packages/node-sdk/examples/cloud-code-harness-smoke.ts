...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'cloud-code-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'cloud-code-harness-home-'));
  ...
  try {
    ...
    await harness.renameSession({
      id: session.id,
      title: 'cloud-code-harness-smoke',
    });
