...
import type { Event } from '@cloud-code/sdk';

import { smokeIdentityFromEnv } from './runtime-smoke-helpers';

async function main(): Promise<void> {
  const homeDir = await mkdtemp(join(tmpdir(), 'cloud-code-harness-rename-home-'));
  const workDir = await mkdtemp(join(tmpdir(), 'cloud-code-harness-rename-work-'));
