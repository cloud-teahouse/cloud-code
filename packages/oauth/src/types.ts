 ...
 *
 * Two flows are supported: Device Code Flow (RFC 8628) against
 * `https://auth.kimi.com`, and the ChatGPT authorization-code + PKCE flow
 * against `https://auth.openai.com` (see chatgpt-codex-flow.ts).
 *
...
export interface TokenInfo {
  ...
  /**
   * ChatGPT Codex only: the `chatgpt_account_id` claim extracted from the
   * id_token at login/refresh time. Sent as the `ChatGPT-Account-ID` header
   * on every Codex backend request. Absent for Kimi tokens.
   */
  readonly accountId?: string | undefined;
  /**
   * ChatGPT Codex only: the raw id_token JWT string, kept so account claims
   * remain available after a refresh that does not return a new id_token.
   */
...
export function tokenToWire(token: TokenInfo): TokenInfoWire {
  return {
    ...
    // Optional ChatGPT Codex fields: omitted entirely for Kimi tokens so the
    // on-disk wire format stays byte-identical to what existing readers expect.
    ...(token.accountId !== undefined ? { account_id: token.accountId } : {}),
    ...(token.idToken !== undefined ? { id_token: token.idToken } : {}),
  };
}
...
export function tokenFromWire(wire: Partial<TokenInfoWire>): TokenInfo {
  return {
    ...
    accountId: typeof wire.account_id === 'string' ? wire.account_id : undefined,
    idToken: typeof wire.id_token === 'string' ? wire.id_token : undefined,
  };
}
