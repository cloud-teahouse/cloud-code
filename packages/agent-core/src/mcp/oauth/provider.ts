 ...
 *    state under `<CLOUD_CODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.cloud-code`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("cloud-code (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `cloud-code (${options.serverName})`;
  }
