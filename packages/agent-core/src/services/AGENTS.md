...
In-process service layer for the Cloud Code CLI server, now hosted inside
`@cloud-code/agent-core` (merged from the former `@cloud-code/services`
...
from `index.ts` so importing `@cloud-code/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@cloud-code/agent-core` loads the package barrel, whose impl
