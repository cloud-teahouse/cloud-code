/**
 * Command-prefix arity table for bash commands, ported verbatim from
 * opencode (`packages/opencode/src/permission/arity.ts`).
 *
 * `prefix(tokens)` returns the leading tokens that name the
 * "human-understandable command" (e.g. `git checkout main` →
 * `['git', 'checkout']`). Flags count as tokens here — known inherited
 * defect (do not fix piecemeal, see the F2 design doc):
 *   - `git -C /x log`  → prefix `git -C`      (flag counted as token)
 * The former second defect — `sudo rm x` → prefix `sudo` (over-broad
 * grant `sudo *`) — is fixed upstream of this table:
 * shell-ast/wrappers.ts peels safe wrappers before the arity lookup, so
 * `sudo` only reaches the prefix when the strip was refused.
 */
