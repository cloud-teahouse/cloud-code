---
name: custom-theme
description: Create or edit a Cloud Code CLI custom color theme — a JSON file under the resolved CLOUD_CODE_HOME data directory that recolors the TUI. Use when the user wants their own theme, asks for a specific palette or mood, or wants to tweak an existing custom theme's colors.
---

# Create a Cloud Code CLI custom theme (custom-theme)

Help the user design, write, and apply a custom color theme for the Cloud Code CLI TUI. A theme is a single JSON file; the TUI ships with `dark`, `light`, and `auto`, and any file the user adds becomes selectable alongside them.
...
- **Never assume the data directory is `~/.cloud-code`.** Always resolve `$CLOUD_CODE_HOME` first with the Bash command below.
...
The Cloud Code CLI runtime resolves the data directory as `CLOUD_CODE_HOME` first, falling back to `~/.cloud-code`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `CLOUD_CODE_HOME` is set and fall back to `~/.cloud-code` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<CLOUD_CODE_HOME>` means that resolved data root — **never assume `~/.cloud-code`**. Theme files live at `<CLOUD_CODE_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.

## What a theme is

- A theme lives at `<CLOUD_CODE_HOME>/themes/<name>.json`.
...
## Source of truth: the in-repo token reference

Before choosing colors, read the authoritative token list — every token the loader accepts is defined, with a per-token description, in the theme JSON schema in this repository:

```
apps/cloud-code/src/tui/theme/theme-schema.json
```

Only set tokens from this set — unknown keys are silently ignored at load. The reference table below mirrors the same tokens; when in doubt, trust the schema file.
...
| `userMessageBackground` | Full-width background block behind the user-message echo in the transcript |
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$CLOUD_CODE_HOME"`; if empty, use `echo "$HOME/.cloud-code"`. Use `<root>/themes` for every subsequent step.
   - If tweaking an existing custom theme, **Read** `<CLOUD_CODE_HOME>/themes/<name>.json` first — never overwrite a theme you haven't read.
   - Starting fresh: build a `colors` object from the token table. You can `ls <CLOUD_CODE_HOME>/themes/` and Read one of the user's existing themes as a reference for the format.
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<CLOUD_CODE_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
