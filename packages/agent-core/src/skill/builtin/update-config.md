---
name: update-config
description: Inspect or edit Cloud Code CLI's own config — `config.toml` (model, provider, permission, hooks) and `tui.toml` (theme, editor, notifications, auto-update). Use when the user asks what a setting does or wants to change one.
---

# Configure Cloud Code CLI (update-config)

Help the user inspect, change, and validate Cloud Code CLI's configuration files. The files are **TOML** with **snake_case** keys.

## The two config files

Cloud Code CLI has two TOML config files, both under `<CLOUD_CODE_HOME>/`, both snake_case, but with different ownership — decide which one the user means before doing anything.

The runtime resolves the data directory as `CLOUD_CODE_HOME` first, falling back to `~/.cloud-code`. Before doing anything, resolve the actual directory with Bash so you don't write to the wrong place. Check whether `CLOUD_CODE_HOME` is set and fall back to `~/.cloud-code` when it is empty:

```bash
echo "$CLOUD_CODE_HOME"
echo "$HOME/.cloud-code"
```

Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<CLOUD_CODE_HOME>` means that resolved root — **never assume `~/.cloud-code`**.
...
## Prerequisite 1: the in-repo schema is the single source of truth

Before touching any config, read the authoritative schema for the file you are changing — it defines the fields (key names, types, allowed values, owning section):

- `config.toml` → `packages/agent-core/src/config/schema.ts` (zod schemas such as `ProviderConfigSchema`, `ModelAliasSchema`, `PermissionConfigSchema`, `HookDefSchema`)
- `tui.toml` → the `TuiConfig` type in `apps/cloud-code/src/tui/config.ts`

- Use the **snake_case key names and sections exactly as defined there** — don't invent them, don't guess camelCase.
- If you cannot locate the schema entry for a key, tell the user plainly and ask them to paste the relevant section or confirm whether to proceed from what you already know. **Never edit blindly without an authoritative reference.**
...
- Location: `<CLOUD_CODE_HOME>/config.toml` or `<CLOUD_CODE_HOME>/tui.toml`. For other scopes/files, defer to the schema files above.
...
1. Read the in-repo schema (Prerequisite 1).
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <CLOUD_CODE_HOME>/config.toml`).
4. **Edit the candidate**: use the **Edit** tool on the candidate to **change/add only the target key** — never rewrite the whole file. That way every existing section, entry, comment, and bit of formatting stays exactly as-is; only what should change changes. The candidate is identical to the original, so use the content you read in step 2 to locate the Edit anchor. Check the change against the in-repo schema (key / section / value type / allowed values, snake_case).
5. **Validate the candidate** (see Capability 3, via `cloudcode doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`cloudcode doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs Cloud Code CLI's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `cloudcode doctor config <config-new.toml path>`
- changed `tui.toml` → `cloudcode doctor tui <tui-new.toml path>`
...
Then do two checks `cloudcode doctor` can't:

1. **Cross-check values against the in-repo schema** (single source of truth): are the key / section / enum values as defined there, and snake_case? doctor guarantees "schema-valid", but "valid yet not what the user wanted" (e.g. a misspelled model alias) needs the schema.
...
> To also check whether the currently **active** config is OK overall, run `cloudcode doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
...
Note: `/reload` is available **only when idle** — if a reply is streaming, press Esc / Ctrl-C to stop first. `cloudcode doctor` already validated the schema before the overwrite, so reload should apply cleanly; if it still errors, follow the message to fix it or recover from the most recent timestamped backup. If you don't want to reload now, the **next new session** picks it up automatically.
...
- When you have no authoritative reference for a key, don't edit by guessing.
