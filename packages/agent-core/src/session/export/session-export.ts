...
const SESSION_LOG_REL = 'logs/cloud-code.log';
const GLOBAL_LOG_REL = 'logs/global/cloud-code.log';
...
export async function exportSessionDirectory(input: {
  ...
  if (sessionFiles.length === 0) {
    throw new CloudCodeError(ErrorCodes.SESSION_EXPORT_NOT_FOUND, `Session "${input.summary.id}" has no exportable directory at "${sessionDir}"`, {
...
function defaultExportZipName(sessionId: string, now: Date): string {
  ...
  return `cloud-code-debug-${shortId}-${timestamp}.zip`;
}
