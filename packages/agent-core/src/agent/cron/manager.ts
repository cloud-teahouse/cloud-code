 ...
 * `CLOUD_CODE_CRON_NO_STALE=1` (bench / acceptance tests).
 *
...
export interface CronManagerOptions {
   ...
   * `resolveClockSources(process.env.CLOUD_CODE_CRON_CLOCK)` so production
   * picks up `CLOUD_CODE_CRON_CLOCK=file:...` automatically.
   ...
   * (1000ms unless `CLOUD_CODE_CRON_MANUAL_TICK=1`, which forces `null` here
   ...
   * {@link DEFAULT_PROJECT_PROBE_MS}; `CLOUD_CODE_CRON_MANUAL_TICK=1` forces
...
export class CronManager {
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(cloudCodeEnv('CLOUD_CODE_CRON_CLOCK', 'KIMI_CRON_CLOCK')) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.projectProbeMs =
      cloudCodeEnv('CLOUD_CODE_CRON_MANUAL_TICK', 'KIMI_CRON_MANUAL_TICK') === '1'
        ? null
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => cloudCodeEnv('CLOUD_CODE_DISABLE_CRON', 'KIMI_DISABLE_CRON') === '1',
      onFire: (task, ctx) => {
      ...
      // `CLOUD_CODE_CRON_MANUAL_TICK=1` forces the scheduler into
  ...
  private async initProjectSchedule(): Promise<void> {
    if (this.projectStore === undefined) return;
    if (cloudCodeEnv('CLOUD_CODE_DISABLE_CRON', 'KIMI_DISABLE_CRON') === '1') return;
   ...
   * Stale judgment.
   *
   *   - `CLOUD_CODE_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (cloudCodeEnv('CLOUD_CODE_CRON_NO_STALE', 'KIMI_CRON_NO_STALE') === '1') return false;
   ...
   *
   * Gated on `CLOUD_CODE_CRON_MANUAL_TICK=1` for two reasons:
   *
   ...
   * Set `CLOUD_CODE_CRON_DEBUG=1` to surface the swallowed error to stderr —
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (cloudCodeEnv('CLOUD_CODE_CRON_MANUAL_TICK', 'KIMI_CRON_MANUAL_TICK') !== '1') return;
