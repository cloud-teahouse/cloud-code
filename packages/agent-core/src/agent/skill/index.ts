...
export class SkillManager {
  ...
  activate(input: ActivateSkillPayload): void {
    ...
    if (skill === undefined) {
      throw new CloudCodeError(ErrorCodes.SKILL_NOT_FOUND, `Skill "${input.name}" was not found`);
    }
    if (!isUserActivatableSkillType(skill.metadata.type)) {
      throw new CloudCodeError(ErrorCodes.SKILL_TYPE_UNSUPPORTED, `Skill "${skill.name}" cannot be activated by the user`);
    }
