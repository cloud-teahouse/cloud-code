...
const ROOT_SYMBOL = Symbol.for('cloud-code.logger.root');
const SESSION_LOG_ID = Symbol('cloud-code.logger.sessionLogId');
...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'cloud-code.log'),
 ...
 *   log.info('cloud-code starting', { version });
 *
 ...
 * importing `log` at module load (before `CloudCodeHarness` configures the root)
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'cloud-code.log');
}
