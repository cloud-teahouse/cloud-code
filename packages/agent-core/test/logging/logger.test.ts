...
describe('session routing', () => {
  it('writes sessionId-tagged entries to session sink only', async () => {
    ...
    try {
      ...
      const session = await readFile(join(sessionDir, 'logs', 'cloud-code.log'), 'utf-8');
  ...
  it('writes entries without sessionId only to global (not broadcast)', async () => {
    ...
    try {
      ...
      let sessionText = '';
      try {
        sessionText = await readFile(join(sessionDir, 'logs', 'cloud-code.log'), 'utf-8');
      } catch {}
  ...
  it('keeps same-id sessions in different directories isolated', async () => {
    ...
    try {
      ...
      const firstText = await readFile(join(firstDir, 'logs', 'cloud-code.log'), 'utf-8');
      const secondText = await readFile(join(secondDir, 'logs', 'cloud-code.log'), 'utf-8');
  ...
  it('keeps a reused same-directory session sink open until every handle closes', async () => {
    ...
    try {
      ...
      const text = await readFile(join(sessionDir, 'logs', 'cloud-code.log'), 'utf-8');
