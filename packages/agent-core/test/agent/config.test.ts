import type { ModelCapability, ProviderConfig, ToolCall } from '@cloud-code/kosong';
...
describe('Agent config', () => {
  ...
  it('useProfile emits the rendered system prompt and active tools', async () => {
    ...
    expect(ctx.newEvents()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated     { "model": "mock-model", "contextTokens": 0, "maxContextTokens": 1000000, "contextUsage": 0, "planMode": false, "swarmMode": false, "coordinatorMode": false, "permission": "manual" }
  ...
  it('keeps turn-start config for later steps and applies updates to the next turn', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated                { "model": "changed-model", "thinkingEffort": "off", "contextTokens": 16, "maxContextTokens": 1000000, "contextUsage": 0.000016, "planMode": false, "swarmMode": false, "coordinatorMode": false, "permission": "manual" }
      [wire] config.update                       { "systemPrompt": "Changed system prompt.", "time": "<time>" }
      [emit] agent.status.updated                { "model": "changed-model", "contextTokens": 16, "maxContextTokens": 1000000, "contextUsage": 0.000016, "planMode": false, "swarmMode": false, "coordinatorMode": false, "permission": "manual" }
