 ...
 * `CLOUD_CODE_CRON_MANUAL_TICK=1` env disables the auto-tick interval and,
...
describe('CronManager — P1.8 manual tick + SIGUSR1', () => {
  ...
  describe('CLOUD_CODE_CRON_MANUAL_TICK=1', () => {
    it('does not install setInterval; tick() must be called manually', async () => {
      vi.stubEnv('CLOUD_CODE_CRON_MANUAL_TICK', '1');
  ...
  describe('without CLOUD_CODE_CRON_MANUAL_TICK', () => {
  ...
  describe('SIGUSR1', () => {
    // SIGUSR1 binding is opt-in via CLOUD_CODE_CRON_MANUAL_TICK=1 so that
    ...
    it('logs swallowed tick() throws to stderr when CLOUD_CODE_CRON_DEBUG=1', async () => {
      if (process.platform === 'win32') return;
      vi.stubEnv('CLOUD_CODE_CRON_DEBUG', '1');
    ...
    it('does not write to stderr on tick() throw when CLOUD_CODE_CRON_DEBUG is unset', async () => {
      if (process.platform === 'win32') return;
      // CLOUD_CODE_CRON_DEBUG intentionally NOT set in this test.
    ...
    it('stop() removes the SIGUSR1 listener (no leak)', async () => {
      ...
      // Constructor auto-starts, which binds SIGUSR1 under CLOUD_CODE_CRON_MANUAL_TICK=1.
      const manager = new CronManager(stub.agent, { pollIntervalMs: null });
    ...
    it('does not bind when CLOUD_CODE_CRON_MANUAL_TICK is unset', async () => {
      if (process.platform === 'win32') return;
