...
describe('CronCreateTool', () => {
  ...
  it('documents the session task cap and near-term one-shot guidance, without bench env vars', () => {
    ...
    expect(tool.description).not.toContain('CLOUD_CODE_CRON_NO_STALE');
    expect(tool.description).not.toContain('CLOUD_CODE_CRON_NO_JITTER');
  ...
  it('returns an error when CLOUD_CODE_DISABLE_CRON=1', async () => {
    vi.stubEnv('CLOUD_CODE_DISABLE_CRON', '1');
    const { manager, tool } = makeHarness();
    const msg = assertError(
      await runTool(tool, {
        cron: '*/5 * * * *',
    ...
    expect(msg).toMatchInlineSnapshot(
      `"Cron scheduling is disabled (CLOUD_CODE_DISABLE_CRON=1)."`,
    );
