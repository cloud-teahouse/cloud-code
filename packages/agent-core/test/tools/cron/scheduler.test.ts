 ...
 * `ClockSources`; `CLOUD_CODE_CRON_NO_JITTER=1` pins fire counts on the
 * recurring tests.
 */
...
const ORIGINAL_ENV_NO_JITTER = process.env['CLOUD_CODE_CRON_NO_JITTER'];
...
describe('createCronScheduler — tick behaviour', () => {
  ...
  afterEach(() => {
    if (ORIGINAL_ENV_NO_JITTER === undefined) {
      ...
      process.env['CLOUD_CODE_CRON_NO_JITTER'] = ORIGINAL_ENV_NO_JITTER;
    }
  });
