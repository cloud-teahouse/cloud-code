...
describe('jitteredNextCronRunMs — recurring', () => {
  ...
  it('respects CLOUD_CODE_CRON_NO_JITTER=1 — no offset', () => {
    const prev = process.env['CLOUD_CODE_CRON_NO_JITTER'];
    process.env['CLOUD_CODE_CRON_NO_JITTER'] = '1';
    try {
      const parsed = parseCronExpression('*/5 * * * *');
      const ideal = localDate(2024, 5, 1, 12, 5, 0);
      const jittered = jitteredNextCronRunMs(
      ...
      if (prev === undefined) delete process.env['CLOUD_CODE_CRON_NO_JITTER'];
      else process.env['CLOUD_CODE_CRON_NO_JITTER'] = prev;
    }
  });
});
...
describe('oneShotJitteredNextCronRunMs', () => {
  ...
  it('respects CLOUD_CODE_CRON_NO_JITTER=1', () => {
    const prev = process.env['CLOUD_CODE_CRON_NO_JITTER'];
    process.env['CLOUD_CODE_CRON_NO_JITTER'] = '1';
    try {
      const ideal = localDate(2024, 5, 1, 14, 0, 0);
...
beforeEach(() => {
  delete process.env['CLOUD_CODE_CRON_NO_JITTER'];
});
afterEach(() => {
  delete process.env['CLOUD_CODE_CRON_NO_JITTER'];
});
