...
describe('SkillTool metadata and schema', () => {
  ...
  it('documents the skill and args parameters and the already-loaded guard', () => {
    ...
    // A skill loaded earlier surfaces a <cloud-code-skill-loaded> block; the description
    // must steer the model to follow it rather than re-invoking the tool.
    expect(tool.description).toContain('cloud-code-skill-loaded');
...
describe('SkillTool execution', () => {
  ...
  it('records inline skill content as a loaded skill message', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<cloud-code-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="message text">\nbody of commit\n\nARGUMENTS: message text\n</cloud-code-skill-loaded>',
    );
  ...
  it('keeps plugin instructions adjacent to model-invoked skill content', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<cloud-code-skill-loaded name="brainstorming" trigger="model-tool" source="extra" dir="/skills/brainstorming" args="">\n' +
  ...
  it('expands skill body placeholders for model-invoked inline skills', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<cloud-code-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="-m &quot;fix login&quot;">\nFlag: -m\nCommit message: fix login\nRaw: -m "fix login"\n</cloud-code-skill-loaded>',
    );
  ...
  it('expands session id from the skill registry for model-invoked skills', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<cloud-code-skill-loaded name="session-aware" trigger="model-tool" source="user" dir="/skills/session-aware" args="">\nSession: ses_model_skill\n</cloud-code-skill-loaded>',
    );
  });
  ...
  it('escapes skill name and args in the wrapper boundaries', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<cloud-code-skill-loaded name="a&amp;b" trigger="model-tool" source="user" dir="/skills/a&amp;b" args="&lt;raw &quot;value&quot;&gt;">\nbody of a&b\n\nARGUMENTS: &lt;raw "value"&gt;\n</cloud-code-skill-loaded>',
    );
    expect(methods.recordSkillActivation).toHaveBeenCalledTimes(1);
  });
