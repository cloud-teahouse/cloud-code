...
const DEFAULT_TITLE = 'Cloud Code CLI WebSocket API';
...
export function createAsyncApiDocument(
  ...
  return {
    ...
    info: {
      ...
      version,
      description:
        'WebSocket protocol for Cloud Code CLI daemon control frames, acknowledgements, system frames, and session event streaming.',
    },
    ...
    servers: {
      local: {
        ...
        pathname: wsPath,
        description: 'Cloud Code CLI daemon WebSocket endpoint.',
      },
    },
    channels: {
      [CHANNEL_ID]: {
