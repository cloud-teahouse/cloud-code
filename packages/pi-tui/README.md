...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "@cloud-code/pi-tui";
...
**Fullscreen mouse**: each visible overlay's composed rect is tracked per frame. When the
focused component lives in an overlay, presses and motion inside that rect are translated
into the component's frame (container chrome like separator rows and gutters included, hit
zones composed as usual); presses outside the rect are dropped (no click-through to covered
content), and the wheel follows hover semantics — routed to the overlay only while the
pointer is inside its rect, otherwise to the underlay (transcript scroll / slot panels).
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@cloud-code/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@cloud-code/pi-tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@cloud-code/pi-tui";
...
```typescript
import { matchesKey, Key } from "@cloud-code/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@cloud-code/pi-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@cloud-code/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@cloud-code/pi-tui";
import type { Component } from "@cloud-code/pi-tui";
