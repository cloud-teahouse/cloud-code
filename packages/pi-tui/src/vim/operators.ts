 ...
 * Ported from Claude Code's src/vim/operators.ts; the only adaptation is
 * inlining `countCharInString` (a one-liner there) to keep the dependency
 * surface inside this directory.
 */

import { Cursor } from "./cursor.ts";
import { firstGrapheme, lastGrapheme } from "./intl.ts";
import {
	isInclusiveMotion,
	isLinewiseMotion,
	resolveMotion,
} from "./motions.ts";
import { findTextObject } from "./textObjects.ts";
import type {
	FindType,
	Operator,
	RecordedChange,
...
function countCharInString(text: string, char: string): number {
	let count = 0;
...
export function executeLineOp(
	...
	// cursor.getPosition() returns the wrapped line, which is wrong for this
...
function getOperatorRange(
	...
	// Word motions can land inside a chip ([image #N] / [paste #N ...]);
	// extend the range to cover the whole chip so dw/cw/yw never leave a
	// partial placeholder.
